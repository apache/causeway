    ...
    private void attachInvocationFacet(final ProcessMethodContext processMethodContext) {
        ...
        try {
            ...
            if (interactionWithAction != null) {
                ...
                FacetUtil.addFacet(new InteractionWithActionFacetForAnnotation(actionMethod, typeSpec, returnSpec, holder, getRuntimeContext(), getAdapterManager(), getServicesInjector(), eventType));
            } else if (postsActionInvokedEvent != null) {
                Class<? extends ActionInteractionEvent<?>> eventType = postsActionInvokedEvent.value();
                FacetUtil.addFacet(new InteractionWithActionFacetForPostsActionInvokedEventAnnotation(actionMethod, typeSpec, returnSpec, holder, getRuntimeContext(), getAdapterManager(), getServicesInjector(), eventType));
            } else {
