...
<3> add the arguments of an action to an `ActionDto`.  This is used when the command is actually executed (per xref:system:generated:index/applib/services/iactn/InteractionContext.adoc[`InteractionContext`]) to populate the parameters of the equivalent `ActionInvocationDto`.
<4> add the new value argument of a property to a `PropertyDto`.  This is used when the command is actually executed (per xref:system:generated:index/applib/services/iactn/InteractionContext.adoc[`InteractionContext`]) to set the the new value of the equivalent `PropertyEditDto`.
