...
The optional xref:refguide:applib-svc:presentation-layer-spi/TableColumnOrderService.adoc[`TableColumnOrderService`] SPI service can be used to reorder columns in a table, either for a parented collection (owned by parent domain object) or a standalone collection (returned from an action invocation).

For example, suppose there is a `Customer` and an `Order`:

[plantuml, {imagesdir}layouts/customer-order, png]
....
