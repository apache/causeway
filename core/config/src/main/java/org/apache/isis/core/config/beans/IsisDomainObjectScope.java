...
@Log4j2
class IsisDomainObjectScope implements Scope {
    ...
    public Object get(final String name, final ObjectFactory<?> objectFactory) {
        if (!interactionService().isInInteraction()) {
            throw new IllegalStateException("No Isis Interaction is currently active");
        }
    ...
    public void registerDestructionCallback(final String name, final Runnable runnable) {
        ...
        log.warn("registerDestructionCallback not supported for 'isis-domain-object' scope");
    }
    ...
    public String getConversationId() {
        ...
        return interactionService()
                ...
                .map(UUID::toString)
                .orElseThrow(() -> new IllegalStateException("Could not obtain interactionId from current Isis Interaction"));
    }
