...
The drop-down is hidden by default and can be explicitly enabled alongside the message-of-the-day banner.
...
|  true
| In prototyping mode, a text icon is appeneded to any property that is disabled, with its tool-tip explaining why the property is disabled. This configuration property can be used to suppress the icon, even in prototyping mode, if desired.
