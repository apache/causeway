...
<.> Enables the xref:pjpa:ROOT:about.adoc[JPA/Eclipselink] object store as the persistence mechanism.
<.> Enables the xref:security:spring:about.adoc[Spring Security], which configures Spring filters to perform authentication.
<.> Enables the xref:security:spring-oauth2:about.adoc[] extension, that in particular configures Spring for Oauth2.
<.> Enables the xref:security:secman:about.adoc[Secman] security module, which provides both authorisation (users, roles, permissions) and authentication (although Spring's authentication takes precedence).
<.> Configures Secman to use JPA for persistence.
<.> Configures Secman to auto-create delegated users if authenticated by Spring.
...
There is a similar domain service for xref:pjpa:ROOT:about.adoc[JPA/Eclipselink] (`EclipselinkConfiguration`).
