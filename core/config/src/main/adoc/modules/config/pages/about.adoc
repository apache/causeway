...
<.> Enables JPA/Eclipeslink for persistence.
+
Alternatively, enable JDO/Datanucleus using `IsisModuleJdoDataNucleus5`.
+
Specifying an ORM for a persistence mechanism is optional, though if omitted then only xref:userguide:fun:overview.adoc#view-models[view models] may be used, with hand-rolled persistence.
