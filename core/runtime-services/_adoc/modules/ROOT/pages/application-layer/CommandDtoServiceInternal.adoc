...
xref:rg:schema:cmd.adoc["cmd" schema], so can be reified so that its execution can be deferred until later,
...
----
<1> Returns a JAXB DTO being an instance of the xref:rg:schema:cmd.adoc["cmd" schema] (hence convertible to
...
`MemberExecutionDto` (from the xref:rg:schema:ixn.adoc["ixn" schema]).
