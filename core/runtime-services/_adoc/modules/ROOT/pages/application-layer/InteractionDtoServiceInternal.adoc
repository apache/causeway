...
<2> called by the framework when editing a property, to create a DTO (for the xref:rg:schema:ixn.adoc["ixn" schema]) capturing the details of the property edit (target, new value etc).
...
(from the xref:rg:schema:cmd.adoc["cmd" schema]).
