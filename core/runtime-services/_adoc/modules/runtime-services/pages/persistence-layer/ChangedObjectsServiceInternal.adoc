...
This is then used by various other  (internal) domain services, notably xref:core:runtime-services:persistence-layer/AuditingServiceInternal.adoc[`AuditingServiceInternal`] and xref:core:runtime-services:persistence-layer/PublishingServiceInternal.adoc[`PublishingServiceInternal`].
...
<2> Used by the framework to determine whether to set the "persist hint" on the `Command` object (as per xref:refguide:applib-svc:application-layer-api/CommandContext.adoc[`CommandContext`]).
<3> Used by xref:core:runtime-services:persistence-layer/PublishingServiceInternal.adoc[`PublishingServiceInternal`] to obtain details of and
counters of all objects changed within the transaction.
<4> Used by xref:core:runtime-services:persistence-layer/AuditingServiceInternal.adoc[`AuditingServiceInternal`] to obtain all pairs of
...
Both the xref:core:runtime-services:persistence-layer/AuditingServiceInternal.adoc[`AuditingServiceInternal`] and xref:core:runtime-services:persistence-layer/PublishingServiceInternal.adoc[`PublishingServiceInternal`] (internal) domain services query this object.
