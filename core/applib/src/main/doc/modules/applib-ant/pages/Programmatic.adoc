...
The `@Programmatic` annotation causes the method to be excluded completely from the Apache Isis metamodel. This means it won't appear in any UI, and it won't appear in any xref:refguide:applib-svc:integration-api/MementoService.adoc[memento]s or xref:refguide:applib-svc:integration-api/XmlSnapshotService.adoc[snapshot]s.
