...
`port` +
| port number (`_587_`)
|The port number for the SMTP service on the the external SMTP host (used by xref:refguide:applib-svc:integration-api/EmailService.adoc[`EmailService`]).  +
...
`sender.address` +
| email address
|The email address to use for sending out email (used by xref:refguide:applib-svc:integration-api/EmailService.adoc[`EmailService`]).
*Mandatory*.  +
...
|The hostname of the external SMTP provider (used by xref:refguide:applib-svc:integration-api/EmailService.adoc[`EmailService`]).  +
...
`sender.password` +
| email password
|The corresponding password for the email address to use for sending out email (used by xref:refguide:applib-svc:integration-api/EmailService.adoc[`EmailService`]).
*Mandatory*.  +
...
|`true`,`false` +
(`true`)
|Whether to enable TLS for the email SMTP connection (used by xref:refguide:applib-svc:integration-api/EmailService.adoc[`EmailService`]).  +
...
The email service is used by the xref:refguide:applib-svc:presentation-layer-spi/EmailNotificationService.adoc[`EmailNotificationService`] which is, in turn, used by xref:refguide:applib-svc:persistence-layer-spi/UserRegistrationService.adoc[`UserRegistrationService`].