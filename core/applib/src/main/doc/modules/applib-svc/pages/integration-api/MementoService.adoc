...
// TODO: v2: remove this service.

The `MementoService` was originally introduced to simplify the implementation of xref:userguide:fun:building-blocks.adoc#view-models[ViewModel]s which are required by the framework to return string representation of all of their backing state, moreover which is safe for use within a URL.
This usage is deprecated; use xref:userguide:fun:programming-model.adoc#jaxb[JAXB view models] instead.
...
As noted in the introduction, a common use case for this service is in the implementation of the xref:refguide:applib-cm:classes/super.adoc#AbstractViewModel[`ViewModel`] interface.

[TIP]
====
Rather than implementing `ViewModel`, it's usually easier to annotate your view models with xref:refguide:applib-ant:ViewModel.adoc[`@ViewModel`] (or equivalently xref:refguide:applib-ant:DomainObject.adoc#nature[`@DomainObject#nature=EXTERNAL_ENTITY`] or xref:refguide:applib-ant:DomainObject.adoc#nature[`@DomainObject#nature=INMEMORY_ENTITY`].
====
