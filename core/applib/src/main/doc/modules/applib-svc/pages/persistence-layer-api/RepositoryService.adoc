...
As an alternative, you could also use link:http://www.datanucleus.org/products/accessplatform_4_0/jdo/jdoql_typesafe.html[JDO typesafe queries] through the xref:refguide:applib-svc:persistence-layer-api/IsisJdoSupport.adoc[`IsisJdoSupport`] service.
...
xref:refguide:applib-svc:core-domain-api/FactoryService.adoc[`FactoryService`]'s `instantiate(...)` method, but is provided in the
...
For more details, see xref:refguide:applib-svc:persistence-layer-api/IsisJdoSupport.adoc[`IsisJdoSupport`].
====
...
The xref:refguide:applib-svc:core-domain-api/FactoryService.adoc[`FactoryService`] is often used in conjunction with the `RepositoryService`, to instantiate domain objects before persisting.
