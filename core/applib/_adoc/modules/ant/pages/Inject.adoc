...
In fact, it also injects domain services into xref:testing:integ-test-support:about.adoc[integration tests] and xref:ext-fixtures:ROOT:about.adoc[fixture scripts].
...
----
<1> an alternative implementation would be to implement `findRecentOrders()` as a xref:ug:fun:programming-model.adoc#contributed-action[contributed action].
