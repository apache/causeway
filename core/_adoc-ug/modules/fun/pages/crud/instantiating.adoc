...
When the framework instantiates the object, all services are injected into the framework, and an `ObjectCreatedEvent` xref:ug:fun:building-blocks.adoc#lifecycle-events[lifecycle event] will also be emitted.
...
To instantiate a xref:ug:fun:building-blocks.adoc#mixins[mixin], a different API of `FactoryService` is used:
