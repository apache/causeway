...
+
This is discussed in more detail below, in the section on xref:ug:fun:building-blocks.adoc#events[events].
...
It's worth extending the xref:ug:fun:core-concepts.adoc#hexagonal-architecture[Hexagonal Architecture] to show where domain services fit in:
