...
[NOTE]
====
The framework also allows side-effect code to be placed in separate xref:rg:cms:methods/prefixes.adoc#modify[`modify...()`], xref:rg:cms:methods/prefixes.adoc#clear[`clear...()`] supporting methods; if present then these will be called by the framework rather than the setter.
