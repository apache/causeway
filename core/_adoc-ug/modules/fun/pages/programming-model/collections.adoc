...
It's ok for a xref:ug:fun:programming-model.adoc#view-models[view model] to reference both view model and domain entities.
...
Unlike xref:ug:fun:programming-model.adoc#properties[properties], the framework does not allow collections to be "edited".
Instead, xref:ug:fun:programming-model.adoc#actions[action]s can be written that will modify the contents of the collection as a side-effect.
