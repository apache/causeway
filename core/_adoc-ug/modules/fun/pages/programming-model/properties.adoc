...
A property is an instance variable of a domain object, of a scalar type, that holds some state about either a xref:ug:fun:programming-model.adoc#view-models[view model].
...
The table in the xref:ug:fun:programming-model.adoc#properties-vs-parameters[Properties vs Parameters] section provides a handy reference of each.
