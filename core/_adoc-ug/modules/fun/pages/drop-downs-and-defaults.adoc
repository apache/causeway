...
If the list of available options is fixed then the developer can provided a list a xref:rg:cms:methods/prefixes/choices.adoc[`choices...()`] supporting method (for either and action parameter or when editing a property).
These are rendered in a drop-down.

If the list of available options is much larger, then the developer can use an xref:rg:cms:methods/prefixes/autoComplete.adoc[`autoComplete...()`] supporting method.
...
The developer indicates this using a xref:rg:cms:methods/prefixes/default.adoc[`default...()`] supporting method.
