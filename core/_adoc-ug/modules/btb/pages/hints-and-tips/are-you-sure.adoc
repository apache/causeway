...
Note that the action itself does not use the boolean parameter, it is only used by the supporting xref:rg:cms:methods/prefixes/validate.adoc[`validate...()`] method.