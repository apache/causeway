...
It also supports translations of messages raised imperatively, by which we mean as the result of a call to `title()` to obtain an object's title, or messages resulting from any business rule violations (eg xref:rg:cms:methods/prefixes.adoc#disable[`disable...()`] or xref:rg:cms:methods/prefixes.adoc#validate[`validate...()`], and so on.
