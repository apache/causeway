...
The `BackgroundCommandExecution` (in isis-core) is an abstract template class for  xref:ug:btb:about.adoc#AbstractIsisSessionTemplate[headless access], that defines an abstract hook method to obtain background `Command`s to be executed:
