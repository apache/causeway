...
The `MementoService` was originally introduced to simplify the implementation of xref:ug:fun:building-blocks.adoc#view-models[ViewModel]s which are required by the framework to return string representation of all of their backing state, moreover which is safe for use within a URL.
