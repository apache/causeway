...
The `AppManifest2` interface extends xref:rg:cms:classes/AppManifest-bootstrapping.adoc#api[`AppManifest`], additionally declaring the top-level `Module` that was used to create the app manifest, and fixtures for reference data setup and for teardown:
...
Additional `withXxx(...)` builder methods are inherited from xref:rg:cms:classes/AppManifest-bootstrapping.adoc#bootstrapping[`AppManifestAbstract.Builder`].
...
<4> Optionally each `Module` can define a xref:rg:cms:classes/super.adoc#FixtureScript[`FixtureScript`] which holds immutable "reference data".
...
<5> Similarly, optionally each `Module` can define a tear-down xref:rg:cms:classes/super.adoc#FixtureScript[`FixtureScript`], used to remove the contents of _all_ entities (both reference data and operational/transactional data).
...
xref:rg:cms:classes/AppManifest-bootstrapping.adoc#bootstrapping[Previously] this was done by using a single `AppManifest` for both the tests and the webapp.
