...
|xref:rg:cms:classes/uievent.adoc#TitleUiEvent[`o.a.i.applib.` +
`TitleUiEvent`]
|``o.a.i.core`` +
``events.ui`` +
``isis-core-applib``
...
|Broadcast whenever there is a requirement to obtain a title for a domain object.  Note that if the domain object defines its own xref:rg:cms:methods/reserved.adoc#title[`title()`] supporting method, or has xref:rg:ant:Title.adoc[`@Title`] annotation(s) on its properties, then these will take precedence.

|xref:rg:cms:classes/uievent.adoc#IconUiEvent[`o.a.i.applib.` +
`IconUiEvent`]
|``o.a.i.core`` +
``events.ui`` +
``isis-core-applib``
...
|Broadcast whenever there is a requirement to obtain an icon (or rather, the name of an icon) for a domain object.  Note that if the domain object defines its own xref:rg:cms:methods/reserved.adoc#iconName[`iconName()`] supporting method, or if it has the xref:rg:ant:DomainObjectLayout#cssClassFa.adoc[`@DomainObjectLayout#cssClassFa()`] attribute, then these will take precedence.

|xref:rg:cms:classes/uievent.adoc#CssClassUiEvent[`o.a.i.applib.` +
`CssClassUiEvent`]
|``o.a.i.core`` +
``events.ui`` +
``isis-core-applib``
...
|Broadcast whenever there is a requirement to obtain a CSS class hint for a domain object.  Note that if the domain object defines its own xref:rg:cms:methods/reserved.adoc#cssClass[`cssClass()`] supporting method then this
