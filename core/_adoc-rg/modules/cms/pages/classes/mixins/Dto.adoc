...
This will return the XML text wrapped up in a xref:rg:cms:classes/value-types.adoc#Clob[`Clob`].
...
If the domain object's JAXB annotations reference only a single XSD schema then this will return that XML text as a xref:rg:cms:classes/value-types.adoc#Clob[`Clob`] of that XSD.
If there are multiple XSD schemas referenced then the action will return a zip of those schemas, wrapped up in a xref:rg:cms:classes/value-types.adoc#Blob[`Blob`].
