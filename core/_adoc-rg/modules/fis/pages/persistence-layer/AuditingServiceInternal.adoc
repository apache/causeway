...
<2> uses the xref:rg:fis:persistence-layer/ChangedObjectsServiceInternal.adoc[`ChangedObjectsServiceInternal`] to obtain details of the changed properties, then call the configured xref:rg:svc:persistence-layer-spi/AuditerService.adoc[`AuditerService`](s).
...
The service delegates between the (internal) xref:rg:fis:persistence-layer/ChangedObjectsServiceInternal.adoc[`ChangedObjectsServiceInternal`] domain service  to the configured xref:rg:svc:persistence-layer-spi/AuditerService.adoc[`AuditerService`](s).
If no such service(s) are configured, then _this_ service is in effect a no-op.

The (internal) xref:rg:fis:persistence-layer/PublishingServiceInternal.adoc[`PublishingServiceInternal`] performs a similar function for the xref:rg:svc:persistence-layer-spi/PublisherService.adoc[`PublisherService`], also collating details of the changed objects from `ChangedObjectsServiceInternal`.
