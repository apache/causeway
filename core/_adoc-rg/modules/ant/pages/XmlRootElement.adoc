...
The `@XmlRootElement` annotation provides an alternative way to define a xref:ug:fun:building-blocks.adoc#view-models[view model], in particular one intended to act as a DTO for use within xref:vro:ROOT::about.adoc[RestfulObjects viewer], or which contains arbitrarily complex state.
...
In comparison to using either the xref:rg:cms:classes/super/ViewModel.adoc[`ViewModel`] interface or the xref:rg:ant:ViewModel.adoc[`@ViewModel`] annotation, using `@XmlRootElement` has a couple of significant advantages:
...
This adapter converts any references to such domain entities using the `oidDto` complex type (as defined by the Apache Isis xref:rg:cms:schema/common.adoc[common schema]): the object's type and its identifier.
...
If the view model XML graph is too large to be serialized to a string, then an alternative implementation (eg which maps XML strings to a GUID, say) can be configured using the technique described in xref:ug:btb:hints-and-tips/replacing-default-service-implementations.adoc[here] in the user guide.
====
