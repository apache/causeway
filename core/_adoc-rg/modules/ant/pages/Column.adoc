...
Isis provides custom value types for xref:rg:cms:classes/value-types/Blob.adoc[`Blob`]s and xref:rg:cms:classes/value-types/Clob.adoc[`Clob`]s.  These value types have multiple internal fields, meaning that they corresponding to multiple columns in the database.  Mapping this correctly requires using  `@Column` within JDO's `@Persistent` annotation.
