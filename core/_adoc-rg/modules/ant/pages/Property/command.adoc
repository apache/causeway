...
[NOTE]
====
If background ``Command``s are used, then an external scheduler, using xref:ug:btb:about.adoc#BackgroundCommandExecution[headless access], must also be configured.
====
