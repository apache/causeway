...
See xref:ug:btb:about.adoc#custom-validator[Custom Validator] to learn more.
...
|`true`,`false` +
(`true`)
|Ensures that all xref:ug:fun:programming-model.adoc#jaxb[JAXB view model]s are not `abstract` (so can be instantiated).
...
|`true`,`false` +
(`true`)
| Ensures that all xref:ug:fun:programming-model.adoc#jaxb[JAXB view model]s are not inner classes (so can be instantiated).
...
|`true`,`false` +
(`false`)
| Ensures that all xref:ug:fun:programming-model.adoc#jaxb[JAXB view model]s have a `public` no-arg constructor. +
...
|`true`,`false` +
(`true`)
| Ensures that for all xref:ug:fun:programming-model.adoc#jaxb[JAXB view model]s with properties that reference persistent entities, that those entities are annotated with `@XmlJavaTypeAdapter`.
...
|`true`,`false` +
(`true`)
| Ensures that for all xref:ug:fun:programming-model.adoc#jaxb[JAXB view model]s with properties that are dates or times, that those properties are annotated with `@XmlJavaTypeAdapter`.
...
See xref:ug:btb:about.adoc#finetuning[finetuning the programming model] for more details.
...
See xref:ug:btb:about.adoc#layout-metadata-reader[Layout Metadata Reader] for more information.
