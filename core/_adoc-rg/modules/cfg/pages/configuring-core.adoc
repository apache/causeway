...
|`true`,`false` +
(`false`)
| When searching for  xref:rg:cms:methods/prefixes.adoc#disable[`disableXxx()`] or xref:rg:cms:methods/prefixes.adoc#hide[`hideXxx()`] methods, whether to search only for the no-param version (or also for supporting methods that match the parameter types of the action). +
