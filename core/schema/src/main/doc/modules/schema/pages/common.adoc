...
This is basically a formal XML equivalent to the `Bookmark` object obtained from the xref:refguide:applib-svc:integration-api/BookmarkService.adoc[`BookmarkService`].
...
Further discussion and examples can be found xref:userguide:fun:building-blocks.adoc#oid[here].
...
<3> the object type, corresponding to either the xref:refguide:applib-ant:DomainObject.adoc#objectType[`@DomainObject#objectType()`] attribute, or to the (JDO)
...
The common schema also defines a number of ancillary types, used either by the common schema itself (see above) or by the xref:refguide:schema:cmd["cmd"] and xref:refguide:schema:about.adoc#ixn.adoc["ixn"] schemas.
...
Used by both the xref:refguide:schema:cmd["cmd"] and xref:refguide:schema:about.adoc#ixn.adoc["ixn"] schemas.
