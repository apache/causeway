...
<2> uses complex types defined in the xref:rg:schema:common.adoc["common" schema].
...
The transaction Id is used to correlate to the xref:rg:schema:interaction[interaction] that executes the command, and to any xref:rg:schema:about.adoc#changes.adoc[changes] to domain objects occurring as a side-effect of that interaction.
...
The `paramsDto` type is defined xref:rg:schema:cmd/ancillary.adoc[below].
...
Note also that there is a corresponding xref:rg:schema:ixn/memberExecutionDto.adoc[`memberExecutionDto`]
...
xref:rg:schema:common.adoc["common"] schema.
