...
This is used by the xref:rg:schema:cmd.adoc["cmd" schema] to represent the
...
* second, as references to the target of a command representing the _intention_ to invoke an action or edit a property, as described by the xref:rg:schema:cmd.adoc["cmd" (command)] schema.
+
...
* third, as references to the target of an interaction capturing the actual execution of an action invocation or property edit, as described by the xref:rg:schema:ixn.adoc["ixn" (interaction)] schema.
...
The common schema also defines a number of ancillary types, used either by the common schema itself (see above) or by the xref:rg:schema:cmd["cmd"] and xref:rg:schema:about.adoc#ixn.adoc["ixn"] schemas.
...
Used by both the xref:rg:schema:cmd["cmd"] and xref:rg:schema:about.adoc#ixn.adoc["ixn"] schemas.
