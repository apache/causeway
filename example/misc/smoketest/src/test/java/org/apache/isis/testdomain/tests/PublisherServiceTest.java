...
class PublisherServiceTest {
	...
	@Test
	@Disabled("service lookup fails")
	void publisherServiceShouldBeAwareOfInventoryChanges() {

		val serviceRegistry = IsisContext.getServiceRegistry();

		Map<Class<? extends PublisherService>, PublisherService> map = serviceRegistry.select(PublisherService.class)
				.stream().collect(Collectors.toMap(ps -> ps.getClass(), ps -> ps));
...
//        val publisherService = (PublisherServiceStub) publisherServiceAny; 
		val repository = IsisContext.getServiceRegistry().lookupServiceElseFail(RepositoryService.class);
