...
public class AutoMapper extends AbstractAutoMapper implements ObjectMapping,
	...
	public AutoMapper(final String className, final String parameterBase,
	...
	@Override
	public void createObject(final DatabaseConnector connector,
	...
	public void createTables(final DatabaseConnector connection) {
		...
		for (int i = 0; collectionMappers != null
				&& i < collectionMappers.length; i++) {
	...
	public void destroyObject(final DatabaseConnector connector,
		...
		if (updateCount == 0) {
			LOG.info("concurrency conflict object " + this
	...
	public ObjectAdapter getObject(final DatabaseConnector connector,
		...
		if (rs.next()) {
			...
			throw new ObjectNotFoundException("No object with with " + oid
					+ " in table " + table);
		}
	}

	@Override
	public boolean hasInstances(final DatabaseConnector connector,
	...
	private ObjectAdapter[] loadInstances(final DatabaseConnector connector,
			final ObjectSpecification cls, final String selectStatment) {
	...
	private ObjectAdapter loadObject(final DatabaseConnector connector,
			final ObjectSpecification cls, final Results rs) {
		Oid oid = idMapping.recreateOid(rs, specification);
		ObjectAdapter instance = getAdapter(cls, oid);

		if (instance.getResolveState()
				.isValidToChangeTo(ResolveState.RESOLVING)) {
	...
	@Override
	public void resolve(final DatabaseConnector connector,
			final ObjectAdapter object) {
		...
		if (rs.next()) {
			...
			rs.close();
			throw new SqlObjectStoreException("Unable to load data from "
					+ table + " with id "
	...
	@Override
	public void resolveCollection(final DatabaseConnector connector,
			final ObjectAdapter object, final ObjectAssociation field) {
		if (collectionMappers.length > 0) {
			DatabaseConnector secondConnector = connector.getConnectionPool()
					.acquire();
			for (int i = 0; i < collectionMappers.length; i++) {
				collectionMappers[i].loadInternalCollection(secondConnector,
						object);
			}
	...
	@Override
	public void save(final DatabaseConnector connector,
			final ObjectAdapter object) {
