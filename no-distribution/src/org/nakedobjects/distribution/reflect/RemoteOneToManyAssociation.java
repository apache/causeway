...
public class RemoteOneToManyAssociation implements OneToManyAssociation {
    ...
    public About getAbout(SecurityContext context, NakedObject inObject, NakedObject associate, boolean add) {
...
//    			}
    ...
    public void removeAssociation(NakedObject inObject, NakedObject associate) {
        ...
        if (inObject.isPersistent()  && fullProxy) {
            ...
            try {
                new DissociateRequest(inObject, this, (NakedObject) associate).execute();
            } catch (NakedObjectRuntimeException e) {
                LOG.error("Problem with distribution ", e.getCause());
                throw (NakedObjectRuntimeException) e.getCause();
            }
