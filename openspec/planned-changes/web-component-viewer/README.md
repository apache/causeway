...
The archived `extend-vaadin-to-domain-member-presentation` and `use-vaadin-grid-for-collection-presentation` changes and promoted active `use-vaadin-menu-bar-for-application-menus` change extend that internal toolkit to read-only fields and buttons, collections, and application navigation in independently reviewable tranches.
Their order, shared boundaries, and promotion gates are recorded in `vaadin-presentation-roadmap.md`.
The Grid evidence has been reviewed and the Menu Bar follow-on is now the single active tranche.
