...
| 5 | `add-paged-graphql-reference-autocomplete` *(active)* | Add honest bounded server response windows for reference autocomplete without claiming persistence query pushdown. | Public GraphQL operation, identity, preparation, ordering, continuation, and consistency contracts are accepted. |
