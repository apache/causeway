...
The active `fix-webcomponent-action-dispatch-correctness` change is the remaining Priority 0 correctness tranche identified by the pinned regression corpus.
...
| 3 | `fix-webcomponent-action-dispatch-correctness` *(active)* | Correct nested safe-query and flat mutation dispatch for object and service actions. | Reference Application action failures become successful typed interactions without changing public operations. |
