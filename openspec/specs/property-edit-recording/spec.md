# property-edit-recording Specification

## Purpose
Ensure command-log recording support captures ordinary property edits as replayable command log entries.

## Requirements
### Requirement: Recording support enables property edit command publishing
When command-log recording support is configured as `ENABLED` for the application metamodel, the system SHALL enable command publishing for ordinary property edits through the normal property command publishing facet model.
...
When command-log recording support is `DISABLED` for the application metamodel, property edits MUST retain existing command publishing behavior and MUST NOT be command logged solely because they are property edits.
...
- **GIVEN** an application context starts with command-log recording support configured as `DISABLED`
...
- **GIVEN** command-log recording support is `ENABLED` during metamodel construction
