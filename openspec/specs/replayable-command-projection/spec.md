...
State-changing command entries SHALL be eligible subject to the collection's existing replay-state and baseline filters.
...
The existing export and completed-or-excluded manager collections SHALL apply this policy without changing manager logical types, mementos, baselines, or actions.
...
The existing pending-or-failed replay collection SHALL wrap every command entry returned by its repository query.
...
- **THEN** the pending-or-failed collection wraps it as a `ReplayableCommand`

#### Scenario: Pending exception does not affect general collections
- **WHEN** the same resultless safe action is considered by a general command-sequence collection
