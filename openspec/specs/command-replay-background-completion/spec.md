...
When replaying multiple commands from the unified manager, the system SHALL stop replaying additional commands after a replayed command creates at least one pending background command. A background command SHALL be considered pending when it is persisted with `ExecuteIn.BACKGROUND` and has not yet started. The system MUST leave commands replayed before the pause in their resulting replay states. The system MUST allow the replay user to continue replaying remaining eligible commands after the pending background commands have executed and committed.
...
- **AND** the replay user is instructed to wait for pending background commands to complete before continuing replay
...
- **THEN** replay continues in manager order until its requested bound is reached, the collection is exhausted, or a command replay fails
...
#### Scenario: Replay failure still stops bounded replay
- **GIVEN** no background command is pending
- **WHEN** a command in bounded replay fails
- **THEN** later commands are not replayed
- **AND** replay states already committed before the failure remain unchanged
