...
- **AND** the stable shell presents the current username and host logout control

#### Scenario: Ordinary Vue bootstraps

- **WHEN** the ordinary bypass Vue application loads
- **THEN** it makes no authentication-context request and presents no secured-session chrome
...
The authenticated Vue shell SHALL present a native form that posts to the configured logout endpoint with current CSRF evidence.
The Vue action policy SHALL claim exact `causeway.security.LogoutMenu#logout` requests and submit that host form rather than invoking GraphQL.
...
- **WHEN** an authenticated user submits the shell logout control
- **THEN** Spring validates CSRF, invalidates the session, and redirects to signed-out login
