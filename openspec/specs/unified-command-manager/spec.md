...
Define the unified commandlog manager's identity, state, controls, foreground review collections, compatibility path, and P2 presentation boundary.
...
Manager collection tables SHALL identify commands using interaction id, timestamp, member, replay state, and result presence.
The layout MUST NOT expose known-participant status, reachability, redesigned export/import, workflow mutation, or background-gate controls.

#### Scenario: Unified manager layout remains within P2 scope
- **WHEN** the manager is rendered from fallback layout metadata
- **THEN** baseline, limit, all four collections, and the P1 identification columns are visible
- **AND** controls and columns owned by later reconciliation slices are absent
