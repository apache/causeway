...
- **THEN** the router renders the object page for that logical route identity
...
- **THEN** default route policy constructs the same canonical route
- **AND** HTMX replaces only the route region while pushing that route into browser history

#### Scenario: Route encoding is invalid
- **WHEN** a route contains malformed escapes, empty values, encoded separators, control characters, dot segments, overlong values, or a non-canonical encoding
- **THEN** the viewer presents a bounded invalid-route state
- **AND** does not disclose object state, authorization rules, or raw decoder exceptions
