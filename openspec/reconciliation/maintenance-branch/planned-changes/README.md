...
(`reconcile-replay-failure-continuation`, MA-3/MA-13) and slice 4 (`reconcile-replay-import-idempotency`, MA-2).
Slices 5–10 remain drafted here (all recommended at `medium` effort); there is currently **no active change**. The
next is slice 6 (`reconcile-replayable-command-presentation`, MA-6/7/8/9).
