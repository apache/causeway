...
- Add an SPI interface in the command log module's applib SPI package.
...
- Inject the available SPI implementations as a list and allow the list to be empty.
  This keeps the extension opt-in for applications and avoids requiring a no-op implementation.
...
- [Risk] Multiple SPI implementations may exist → Invoke all discovered implementations in the order supplied by the framework.
