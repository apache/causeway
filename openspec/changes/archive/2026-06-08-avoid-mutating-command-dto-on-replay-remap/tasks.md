...
- [x] 1.5 Identify command subscriber lifecycle sync that delegates to `CommandLogEntry#sync(Command)` and can copy remapped in-memory command DTOs back to replay entries.
...
- [x] 2.6 Ensure `CommandLogEntry#sync(Command)` preserves recorded DTO and outcome data for replay entries while still allowing command subscriber `onStarted(...)` and `onCompleted(...)` to delegate to it.
...
- [x] 4.5 Add tests proving `CommandLogEntry#sync(Command)` does not fully sync replay entries and subscriber lifecycle callbacks delegate to it.
