...
- [ ] 1.2 Define the targeted application-entry field, menu resource metadata, object home kind, localization, private no-store delivery, and diagnostics.
...
- [ ] 3.1 Detect whether `HomePageResolverService` returns a supported domain object or no usable home entry.
- [ ] 3.2 Resolve an object home page through the framework home-page service and corrected polymorphic rich output.
- [ ] 3.3 Keep unsupported action-based home configuration absent until a public semantic descriptor exists.
- [ ] 3.4 Define absence for missing, hidden, invalid, non-domain, or unresolvable home entries.

## 4. Security, Verification, and Documentation

- [ ] 4.1 Generate resources inside the current authorization and locale interaction and return private no-store cache policy.
...
- [ ] 4.4 Document the application-entry contract, canonical resource, service-action relationship, supported object home kind, no-store delivery, authorization, and viewer-policy boundary.
