...
When command-log recording support is `ENABLED`, the export manager SHALL provide a `moveCommandsDown` action that moves one or more selected commands from the active commands collection downward by retimestamping them immediately before a target command.
...
The target command choices for both movement actions SHALL include commands at or after the export manager baseline from the active commands collection.
...
#### Scenario: Up action target choices exclude selected commands
...
#### Scenario: Down action target choices exclude selected commands
- **GIVEN** command-log recording support is `ENABLED`
- **AND** an export manager baseline is set
- **AND** commands `A`, `B`, and `C` are in the active commands collection after the baseline
...
For downward movement, the last moved command SHALL receive a timestamp 10ms before the target command timestamp.
...
#### Scenario: Single command moves down before target with ten millisecond offset
...
- **WHEN** the user moves the selected command down before target command `B`
- **THEN** command `A` has timestamp `10:00:04.990`
...
- **AND** commands `A` and `B` are selected for downward movement
- **WHEN** the user moves the selected commands down before target command `C`
- **THEN** command `A` has timestamp `10:00:04.740`
- **AND** command `B` has timestamp `10:00:04.990`
...
For downward movement with timing squash selected, the last moved command SHALL receive a timestamp 1 second before the target command timestamp.
...
- **AND** commands `A` and `B` are selected for downward movement
- **WHEN** the user moves the selected commands down before target command `C` with timing squash selected
- **THEN** command `A` has timestamp `10:00:18.000`
- **AND** command `B` has timestamp `10:00:19.000`
...
- **AND** command `C` has timestamp `10:00:05.000`
- **AND** commands `A` and `B` are selected for downward movement
- **WHEN** the user moves the selected commands down before target command `C` with timing squash not selected
- **THEN** command `A` has timestamp `10:00:04.740`
- **AND** command `B` has timestamp `10:00:04.990`
- **AND** command `C` keeps timestamp `10:00:05.000`
