...
When command-log recording support is `ENABLED`, the first selected action command in an export sequence SHALL be valid only when its target is an export root.
A first selected action command targeting an ordinary domain object MUST be rejected unless an earlier selected command in export order establishes that object as a known result.
...
#### Scenario: First selected command on ordinary domain object is rejected
- **GIVEN** command-log recording support is `ENABLED`
- **AND** an export manager baseline is set
