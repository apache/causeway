...
The resolved component policy SHALL govern eligible references, editors, read-only field presentation, ordinary action buttons, and their CSP and asset delivery.
...
- **THEN** the resolved component policy enables every qualified Vaadin reference, field, read-only presentation, and ordinary action adapter
- **AND** unsupported or excluded shapes and controls retain native or explicit unsupported presentation
...
- **AND** routes request no reference, field-family, or action-button closure
...
When `component-toolkit` is absent and `editor-toolkit` is explicitly configured, its value SHALL resolve the complete component policy so existing explicit native rollback remains complete.

#### Scenario: Deprecated editor policy selects Vaadin
- **WHEN** `component-toolkit` is absent and `editor-toolkit=vaadin` is explicit
- **THEN** the resolved compatibility policy enables every qualified reference, field, read-only presentation, and ordinary action adapter
...
- **THEN** every qualified Vaadin adapter and style hash is disabled
...
- **AND** read-only presentation and ordinary action buttons remain native during the compatibility period
...
The generic HTMX viewer SHALL enable every qualified packaged adapter under the resolved component policy without eagerly importing any reference, field-family, or action-button closure.
...
- **AND** route readiness, other families, references, and actions do not wait for it
...
- **AND** no field-family closure is downloaded as a transitive convenience

#### Scenario: Route contains no eligible presentation or editor
- **WHEN** a landing, menu-only, custom, or other unaffected route contains no eligible reference, field presentation, editor, or ordinary action
- **THEN** it requests zero reference, basic, numeric, local-temporal, and action-button Vaadin assets
- **AND** default CSP hash permission does not cause a network request

#### Scenario: One family fails
- **WHEN** a default field or action closure fails to load or define its controls
...
- **THEN** `style-src` and `style-src-elem` contain the deterministic deduplicated reference, field-family, and action-button hash union
...
- **THEN** CSP contains the same reviewed union as the default component policy
...
- **AND** read-only-presentation-only and action-button hashes are absent
...
- **THEN** eligible references, read-only fields, editors, and ordinary actions use internal Vaadin adapters and preserve authoritative outcomes
- **AND** unexpected CSP, accessibility, console, page, external-request, stale-state, duplicate-control, focus, overlay, or overflow failures fail the gate

#### Scenario: Native release matrix runs
- **WHEN** the same journeys run with `component-toolkit=native`
- **THEN** native controls preserve the same GraphQL values, routes, interactions, semantic events, and classifications
...
- **THEN** authoritative outcomes match default component mode
