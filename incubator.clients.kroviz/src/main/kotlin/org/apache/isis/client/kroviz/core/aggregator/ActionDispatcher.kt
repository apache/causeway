        ...
        val action = logEntry.getTransferObject() as Action
                action.links.forEach { link ->
                    ...
                    if (link.isInvokeAction()) {
                        when (link.method) {
                            org.apache.isis.client.kroviz.to.Method.GET.name -> processGet(action, link)
                            org.apache.isis.client.kroviz.to.Method.POST.name -> processPost(action, link)
                            org.apache.isis.client.kroviz.to.Method.PUT.name -> invoke(link)
                        }
                    }
                }
    }
