...
import org.apache.isis.extensions.dnd.interaction.SimpleInternalDrag;
...
public class ScrollBorder extends AbstractViewDecorator {
    ...
    public static void setRender(ScrollBarRender render) {
    ...
    private void drawVerticalScrollBar(final Canvas canvas, final int contentWidth, final int contentHeight, final Color color) {
        ...
        if (right > 0 && (verticalScrollPosition > top || verticalVisibleAmount < contentHeight)) {
            ...
            /*
            canvas.drawSolidRectangle(x + 1, top, SCROLLBAR_WIDTH - 1, contentHeight, Toolkit.getColor(ColorsAndFonts.COLOR_SECONDARY3));
            canvas.drawSolidRectangle(x + 1, top + verticalScrollPosition, SCROLLBAR_WIDTH - 2, verticalVisibleAmount, color);
            canvas.drawRectangle(x, top, SCROLLBAR_WIDTH, contentHeight, Toolkit.getColor(ColorsAndFonts.COLOR_SECONDARY2));
            ...
            DrawingUtil.drawHatching(canvas, x + 3, top + verticalScrollPosition + 4, SCROLLBAR_WIDTH - 6,
                    verticalVisibleAmount - 8, Toolkit.getColor(ColorsAndFonts.COLOR_PRIMARY1), Toolkit.getColor(ColorsAndFonts.COLOR_PRIMARY3));
                    */
        }
    }
    ...
    private void drawHorizontalScrollBar(final Canvas canvas, final int contentWidth, final int contentHeight, final Color color) {
        ...
        if (bottom > 0 && (horizontalScrollPosition > left || horizontalVisibleAmount < contentWidth)) {
            ...
            /*
            canvas.drawSolidRectangle(left, y + 1, contentWidth, SCROLLBAR_WIDTH - 1, Toolkit.getColor(ColorsAndFonts.COLOR_SECONDARY3));
            canvas.drawSolidRectangle(x, y + 1, horizontalVisibleAmount, SCROLLBAR_WIDTH - 2, color);
            canvas.drawRectangle(left, y, contentWidth, SCROLLBAR_WIDTH, Toolkit.getColor(ColorsAndFonts.COLOR_SECONDARY2 ));
            canvas.drawRectangle(x, y + 1, horizontalVisibleAmount, SCROLLBAR_WIDTH - 2, Toolkit.getColor(ColorsAndFonts.COLOR_SECONDARY1));
    ...
    private Offset offset() {
        ...
        final int x = width == 0 ? 0 : horizontalScrollBar.getPosition() * wrappedView.getRequiredSize(Size.createMax()).getWidth()
                / width;
        final int height = contents.getHeight();
        final int y = height == 0 ? 0 : verticalScrollBar.getPosition() * wrappedView.getRequiredSize(Size.createMax()).getHeight()
                / height;
