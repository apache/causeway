...
This is a view model annotated with xref:system:generated:index/applib/annotation/HomePage.adoc[@HomePage]:
...
----
<.> Publishing enabled means that the action invocation is converted into XML and dispatched to all configured implementations of the xref:system:generated:index/applib/services/publishing/spi/ExecutionSubscriber.adoc[ExecutionSubscriber] SPI.
+
...
This will pick up the `SimpleObjects` repository because it is annotated with the framework's xref:system:generated:index/applib/annotation/DomainService.adoc[@DomainService] annotation; itself meta-annotated with `@Component`.

* all domain services
+
These are classes that are annotated with the framework's xref:system:generated:index/applib/annotation/DomainService.adoc[@DomainService] annotation.
...
* all entities.
+
These are entities that are annotated with both xref:system:generated:index/applib/annotation/DomainObject.adoc[`@DomainObject`] annotation and with `@javax.jdo.annotations.PersistenceCapable`.
...
These use the xref:system:generated:index/applib/services/wrapper/WrapperFactory.adoc[WrapperFactory] to simulate interactions through the UI.
