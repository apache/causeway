...
<.> Uses Spring Data JPA to automatically provide the query implementations.
...
<.> automatically discovers JPA entities
...
----
<.> Provides various optional framework-provided mixins in the UI, such as the `objectIdentifier` and `logicalTypeName` properties
...
<.> Enables various optional mixins provided by the xref:vw:ROOT:about.adoc[Wicket viewer], for example the `clearHints()` action.
