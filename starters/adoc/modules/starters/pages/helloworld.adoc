...
This will pick up the `HelloWorldObjects` repository because it is annotated with the framework's xref:system:generated:index/applib/annotation/DomainService.adoc[@DomainService] annotation (in turn meta-annotated with `@Component`).
...
It is also annotated with xref:system:generated:index/applib/annotation/DomainObject.adoc[`@DomainObject`]
+
