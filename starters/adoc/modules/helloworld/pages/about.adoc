...
Basically, though, it amounts to running the `main()` method in the `HelloWorldApp`, but also (and this bit is important) ensuring that the xref:toc:devguide:hints-and-tips/datanucleus-enhancer.adoc[DataNucleus enhancer] has properly processed all domain entities.
...
Instead, we suggest that you start with the xref:starters:simpleapp:about.adoc[simpleapp starter app].
