...
* the xref:_ug_reference-services-spi_manpage-UserRegistrationService[user registration service], which provides an API to create the user account
* the xref:_ug_reference-services-spi_manpage-EmailNotificationService[email notification service], which provides an API for to send the verification emails
* the xref:_ug_reference-services-api_manpage-EmailService[email service], that is used by the email notification service to actually send the email.
...
* configure the xref:_ug_reference-services-api_manpage-EmailService[email service]
...
It is also possible to configure the Wicket viewer to suppress xref:_ug_wicket-viewer_configuration-properties_suppressing-sign-up[the sign-up page link] and/or the xref:_ug_wicket-viewer_configuration-properties_suppressing-password-reset[password reset page].
