[[_ug_reference-object-layout_static]]
...
+
The name of these "member groups" are then referenced by xref:_ug_reference-annotations_manpage-MemberGroupLayout[`@MemberGroupLayout`].
...
+
If the `name()` attribute matches a property name, then the action's button is rendered close to the property, according to xref:_ug_reference-annotations_manpage-ActionLayout_position[`@ActionLayout#position()`] attribute. +
+
...
As of 1.8.0, all the layout annotations have been consolidated into the various `XxxLayout` annotations: xref:_ug_reference-annotations_manpage-ActionLayout, [`@ActionLayout`] xref:_ug_reference-annotations_manpage-CollectionLayout[`@CollectionLayout`],  xref:_ug_reference-annotations_manpage-DomainObjectLayout[`@DomainObjectLayout`],  xref:_ug_reference-annotations_manpage-DomainServiceLayout[`@DomainServiceLayout`],  xref:_ug_reference-annotations_manpage-ParameterLayout[`@ParameterLayout`], xref:_ug_reference-annotations_manpage-PropertyLayout[`@PropertyLayout`], and xref:_ug_reference-annotations_manpage-ViewModelLayout[`@ViewModelLayout`]
