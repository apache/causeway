...
public interface IsisJdoSupport {
    @Programmatic
    <T> T refresh(T domainObject);                              // <1>
    @Programmatic
    void ensureLoaded(Collection<?> collectionOfDomainObjects); // <2>
    ...
}
----
<1> force a reload of a domain object (using the JDO `PersistenceManager#refresh(...)` API).
<2> force the loading of a collection of domain objects (using the JDO `PersistenceManager#retrieveAll(...)` API)
