...
``isis-core-runtime``
|depends on: +
xref:_ug_reference-services-spi_manpage-BackgroundCommandService[`BackgroundCommand-Service`]
...
|API is also a concrete class. +
depends on: +
xref:_ug_reference-services-spi_manpage-CommandService[`CommandService`] for persistent `Command`, else in-memory impl. used
