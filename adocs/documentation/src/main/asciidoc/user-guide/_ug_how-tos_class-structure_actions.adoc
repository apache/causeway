= anchor:how-tos_class-structure[]Actions
...
The workaround is to mark the collection as `@Programmatic`, as described in xref:_ignoring_methods[Ignoring Methods].  This ensures that the collection is ignored by Isis.
====
