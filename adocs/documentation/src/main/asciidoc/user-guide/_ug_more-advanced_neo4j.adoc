...
The xref:_simpleapp_archetype[SimpleApp archetype] has been updated so that they can be optionally run under Neo4J.
...
In the xref:_simpleapp_archetype[SimpleApp archetype] this is defined under the "neo4j" profile so can be activated using `-P neo4j`.

== Try it out!

If you want to quickly try out neo4j for yourself:

* run the xref:_simpleapp_archetype[SimpleApp archetype] (v1.8.0)
