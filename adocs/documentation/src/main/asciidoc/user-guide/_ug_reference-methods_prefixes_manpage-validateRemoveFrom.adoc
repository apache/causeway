[[_ug_reference-methods_prefixes_manpage-validateRemoveFrom]]
= `validateRemoveFrom...()`
...
where the returned string is the reason the collection modification invocation is vetoed (or `null` if not vetoed).  Isis' xref:_ug_more-advanced_i18n[i18n support] extends this so that reasons can be internationalized if required.
...
See also xref:_ug_reference-methods_prefixes_manpage-removeFrom[`removeFrom...()`] and  xref:_ug_reference-methods_prefixes_manpage-validateAddTo[`validateAddTo...()`]`
