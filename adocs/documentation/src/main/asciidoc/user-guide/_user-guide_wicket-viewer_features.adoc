...
[TIP]
====
It is also possible to configure the Wicket viewer to <<_abbreviating_suppressing_titles_in_tables, abbreviate titles or suppress them>> completely.
====
...
* the <<_user_registration_service, user registration service>>, which provides an API to create the user account
* the <<_email_notification_service, email notification service>>, which provides an API for to send the verification emails
* the <<_email_service, email service>>, that is used by the email notification service to actually send the email.
...
* register an implementation of the <<_user_registration_service, user registration service>> (eg by using the https://github.com/isisaddons/isis-module-security[Isis addons security module])

* configure the <<_email_service, email service>>
...
It is also possible to configure the Wicket viewer to suppress <<_suppressing_sign_up, the sign-up page link>> and/or the <<_suppressing_password_reset, password reset page>>.
