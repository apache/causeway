...
IMPORTANT: WIP -  easiest to simply store using DataNucleus' support for collections, marked as xref:_ug_reference-annotations_manpage-Programmatic[`@Programmatic`] so that it is ignored by Isis.  Alternatively can store as json/xml in a varchar(4000) or clob and manually unpack.
