...
|the event type to be posted to the <<__code_eventbusservice_code, `EventBusService`>> to broadcast the action's business rule checking (hide, disable, validate) and its invocation (pre-execute and post-execution).
