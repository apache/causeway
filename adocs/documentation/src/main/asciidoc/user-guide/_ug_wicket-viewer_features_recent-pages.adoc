[[_ug_wicket-viewer_features_recent-pages]]
...
[NOTE]
====
Note that this screenshot show an earlier version of the xref:_ug_wicket-viewer[Wicket viewer] UI (specifically, pre 1.8.0).
====
...
The xref:_ug_wicket-viewer_features_bookmarked-pages[bookmarked pages] (sliding panel) also provides links to recently visited objects, but only those explicitly marked as `@DomainObject(bookmarking=...)`.  The bookmarks panel also nests related objects together hierarchically (the recent pages drop-down does not).
