...
The Apache Isis framework also provides numerous in-built domain services.  These are catalogued in the reference guide, see xref:_ug_reference-services-api[here] and xref:_ug_reference-services-spi[here].
...
The CDI annotation xref:_ug_reference-annotations_manpage-RequestScoped[`@javax.enterprise.context.RequestScoped`] is used to indicate this fact:
...
IMPORTANT: WIP - just xref xref:_ug_how-tos_contributed-members[contributed members] section.
...
The framework will call any `public` method annotated with xref:_ug_reference-annotations_manpage-PostConstruct[`@PostConstruct`] with either no arguments of an argument of type `Map<String,String>`
...
Shutdown is similar; the framework will call any method annotated with xref:_ug_reference-annotations_manpage-PreDestroy[`@PreDestroy`].
...
Optionally, a service may provide a xref:_ug_reference-methods_reserved_manpage-getId[`getId()`] method.  This method returns a logical identifier for a service, independent of its implementation.
