[[_ug_reference-methods_prefixes_manpage-get]]
= `get...()`
...
Any methods "left over" that _do_ use one of the Isis prefixes, are interpreted to be orphaned.  Isis "fails-fast" and will not boot, instead printing an error message to the log so that the issue can be easily diagnosed.
