[[_ug_reference-annotations_manpage-Inject]]
= `@Inject` (`javax`)
...
Apache Isis automatically injects domain services into other domain services and also into domain objects and view models.  In fact, it also injects domain services into xref:_ug_testing_integ-test-support[integration tests] and xref:_ug_testing_fixture-scripts[fixture scripts].

[NOTE]
====
One omission: Isis (currently) does not inject services into `o.a.i.applib.spec.Specification` instances (as used by xref:_ug_reference-annotations_manpage-Property_mustSatisfy[`@Property#mustSatisfy()`] and xref:_ug_reference-annotations_manpage-Parameter_mustSatisfy[`@Parameter#mustSatisfy()`] annotations.
====
...
----
<1> an alternative implementation would be to implement `findRecentOrders()` as a xref:_ug_how-tos_contributed-members[contributed action].
...
----
<1> where `container` is an instance of xref:_ug_reference-services-api_manpage-DomainObjectContainer[`DomainObjectContainer`].
