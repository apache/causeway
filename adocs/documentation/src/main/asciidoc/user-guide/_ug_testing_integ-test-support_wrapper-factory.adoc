= anchor:testing_integ-test-support[]Wrapper Factory
...
[NOTE]
====
In this respect the wrapper is more functional than the xref:_wicket_viewer[Wicket viewer] (which does not expose the ability to mutate collections directly).
====
...
----
<1> inject xref:__code_EventBusService_code[`EventBusService`] into this test
