[[_ug_reference-services-api_manpage-GuiceBeanProvider]]
= `GuiceBeanProvider`
...
The `GuiceBeanProvider` domain service acts as a bridge between Isis' xref:_ug_wicket-viewer[Wicket viewer] internal bootstrapping using link:https://github.com/google/guice[Google Guice].

This service operates at a very low-level, and you are unlikely to have a need for it.  It is used internally by the framework, in the default implementation of the xref:_ug_reference-services-api_manpage-DeepLinkService[`DeepLinkService`].
...
The xref:_ug_wicket-viewer[Wicket viewer] this provides an implementation of this service.



== Usage

Using the xref:_ug_wicket-viewer[Wicket viewer] requires subclassing of `IsisWicketApplication`.  In the subclass it is commonplace to override `newIsisWicketModule()`, for example:
...
and that the xref:_ug_wicket-viewer[Wicket viewer] is being used then an implementation of `GuiceBeanProvider` is automatically registered and injected (it is annotated with `@DomainService`) so no further configuration is required.
