= anchor:security[]Configuring Isis to use Shiro
...
[TIP]
====
The xref:__a_id_getting_started_a_simpleapp_archetype[SimpleApp archetype] is pre-configured to use Apache Shiro, so much of what follows is set up already.
====
