...
If using the text-based <<_shiro_ini_realm, `IniRealm`>> or <<_isis_ldap_realm, Isis' LDAP realm>>, then note that Shiro also allows the string representation of the permissions to be mapped (resolved) to alternative `Permission` instances.  Isis provides its own `IsisPermission` which introduces the concept of a "veto".
