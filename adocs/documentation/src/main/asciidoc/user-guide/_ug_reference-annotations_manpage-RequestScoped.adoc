[[_ug_reference-annotations_manpage-RequestScoped]]
= `@RequestScoped` (`javax`)
...
The `@javax.enterprise.context.RequestScoped` link:https://jcp.org/en/jsr/detail?id=299[JSR-299] CDI annotation is used to specify that a xref:_ug_reference-annotations_manpage-DomainService[domain service] should be request-scoped rather than a singleton.
...
One of the built-in domain services that uses this annotation is xref:_ug_reference-services-api_manpage-Scratchpad[`Scratchpad`], intended to allow the arbitrary sharing of data between objects.  Here is the full source code of this service is:
