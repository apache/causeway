...
In the xref:_ug_wicket-viewer[Wicket viewer], a link to a bookmarked object is shown in the bookmarks panel:
...
[TIP]
====
The xref:_ug_wicket-viewer[Wicket viewer] supports `alt-[` as a shortcut for opening the bookmark panel.  `Esc` will close.
====
