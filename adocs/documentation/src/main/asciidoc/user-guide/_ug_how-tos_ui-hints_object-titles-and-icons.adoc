= anchor:how-tos_ui-hints[]Object Titles and Icons
...
In Apache Isis every object is identified to the user by a xref:_object_title[title] (label) and an xref:_object_icon[icon].  This is shown in several places: as the main heading for an object; as a link text for an object referencing another object, and also in tables representing collections of objects.
...
It is also possible for an object to return a xref:_object_css_styling[CSS class].  In conjunction with xref:_tweaking_css_classes[customized CSS] this can be used to apply arbitrary styling; for example each object could be rendered in a page with a different background colour.
...
[TIP]
====
Titles can sometimes get be long and therefore rather cumbersome in "parented" tables.  If `@Title` has been used then the Wicket viewer will automatically exclude portions of the title belonging to the owning object.  See xref:_titles_in_tables[here] for further discussion.
====
