[[_ug_how-tos_ui-hints_object-titles-and-icons]]
...
It is also possible for an object to provide a CSS class hint.  In conjunction with xref:_ug_wicket-viewer_customisation_tweaking-css-classes[customized CSS] this can be used to apply arbitrary styling; for example each object could be rendered in a page with a different background colour.
...
The xref:_ug_reference-annotations_manpage-Title[`@Title`] annotation can be used build up the title of an object from its constituent parts.
...
IMPORTANT: TODO - see xref:_ug_reference-methods_reserved_manpage-title[`title()`]
...
IMPORTANT: TODO - xref:_ug_reference-methods_reserved_manpage-iconName[`iconName()`], xref:_ug_reference-annotations_manpage-DomainObjectLayout_cssClassFa[`@DomainObjectLayout#cssClassFa()`]
...
It is also possible for an object to return a xref:_ug_reference-methods_reserved_manpage-cssClass[CSS class].  In conjunction with xref:_ug_wicket-viewer_customisation_tweaking-css-classes[customized CSS] this can be used to apply arbitrary styling; for example each object could be rendered in a page with a different background colour.

IMPORTANT: TODO - xref:_ug_reference-methods_reserved_manpage-cssClass[`cssClass()`], xref:_ug_reference-annotations_manpage-DomainObjectLayout_cssClass[`@DomainObjectLayout#cssClass()`] xref:_ug_reference-annotations_manpage-ActionLayout_cssClass[`@ActionLayout#cssClass()`], xref:_ug_reference-annotations_manpage-PropertyLayout_cssClass[`@PropertyLayout#cssClass()`], xref:_ug_reference-annotations_manpage-CollectionLayout_cssClass[`@CollectionLayout#cssClass()`]
