...
* by xref:_ug_wicket-viewer_customisation_tweaking-css-classes[tweaking the UI using CSS]
* by xref:_ug_wicket-viewer_customisation_custom-javascript[tweaking the UI using Javascript]
* by writing a xref:_ug_extending_wicket-viewer_custom-bootstrap-theme[custom bootstrap theme]
* by xref:_ug_extending_wicket-viewer_replacing-page-elements[replacing elements of the page] using the `ComponentFactory` interface
* by implementing xref:_ug_extending_wicket-viewer_custom-pages[replacement page implementations] for the standard page types
