= anchor:extending[]Wicket viewer
...
* by xref:_tweak_the_ui_with_css[tweaking the UI using CSS]
* by xref:_tweak_the_ui_with_javascript[tweaking the UI using Javascript]
* by writing a xref:_writing_a_custom_theme[custom bootstrap theme]
* by xref:_replacing_page_elements[replacing elements of the page] using the `ComponentFactory` interface
* by implementing xref:_custom_pages[replacement page implementations] for the standard page types
