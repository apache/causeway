[[_ug_core-concepts_principles_apache-isis-vs_metawidget]]
= vs MetaWidget
...
In contrast, Apache Isis is full-stack and does generate a complete UI; we then allow you to customize or extend this UI (as per the various link:http://www.isisaddons.org[Isis Addons] (non-ASF), and we also provide a full REST API through the xref:_ug_restfulobjects-viewer[Restful Objects viewer]
