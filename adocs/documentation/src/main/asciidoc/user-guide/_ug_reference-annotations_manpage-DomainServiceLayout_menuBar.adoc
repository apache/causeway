...
In the xref:_ug_wicket-viewer[Wicket viewer], domain services placed on the `PRIMARY` menu bar appears to the left:
...
The grouping of multiple domain services actions within a single drop-down is managed by the xref:_ug_reference-annotations_manpage-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] attribute.


[NOTE]
====
The xref:_ug_restfulobjects-viewer[RestfulObjects viewer] does not support this attribute.
====
