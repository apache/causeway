...
[NOTE]
====
The support for xref:_ug_reference-annotations_aaa_deprecated[disallowing deprecated annotations] is also implemented using the metamodel validator.
====
...
[TIP]
====
Isis' xref:_ug_isis-maven-plugin[Maven plugin] will also validate the domain object model during build time.
====
