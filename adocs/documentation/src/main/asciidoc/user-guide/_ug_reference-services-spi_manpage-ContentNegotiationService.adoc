[[_ug_reference-services-spi_manpage-ContentNegotiationService]]
...
The `ContentNegotiationService` is a plug-in point for the xref:_ug_restfulobjects-viewer[RestfulObjects viewer] so that it can generate representations according to HTTP `Accept` header of the request.  This idea is discussed in section 34.1 of the link:http://restfulobjects.org[Restful Objects spec] v1.0.
...
* first is to solve the problem at the domain layer by defining a regular Isis xref:_ug_more-advanced_view-models[view model].  This is then surfaced over the RO viewer.  +
+
...
* a second option is to solve the problem at the persistence layer, but defining a (SQL) view in the database and then xref:_ug_more-advanced_mapping-rdbms-views[mapping this] to a (read-only) entity.  Again this is surfaced by the RO viewer.  +
+
...
+
The Isis framework provides a default implementation of `ContentNegotiationService` which inspects the "x-ro-domaintype" component of the HTTP `Accept` header.  If present, this implementation will delegate to the companion xref:_ug_reference-services-spi_manpage-ContentMappingService[`ContentMappingService`] service, if configured.  +
+
...
<2> representation of a single collection of an object, as per section 17.5 of the RO spec v1.0
<2> representation of a single action (prompt) of an object, as per section 18.2 of the RO spec v1.0
<2> representation of the results of a single action invocation, as per section 19.5 of the RO spec v1.0
...
The value of the `x-ro-domain-type` parameter corresponds to the DTO to be mapped into by the xref:_ug_reference-services-spi_manpage-ContentMappingService[`ContentMappingService`].
...
The default implementation of `ContentNegotiationService` delegates to xref:_ug_reference-services-spi_manpage-ContentMappingService[`ContentMappingService`] (if present) to convert domain entities into a stable form (eg DTO).