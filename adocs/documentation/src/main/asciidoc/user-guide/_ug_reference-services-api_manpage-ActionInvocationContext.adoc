[[_ug_reference-services-api_manpage-ActionInvocationContext]]
...
The `ActionInvocationContext` domain service is a xref:_ug_reference-annotations_manpage-RequestScoped[`@RequestScoped`] service intended to support the implementation of "bulk" actions annotated with xref:_ug_reference-annotations_manpage-Action_invokeOn[`@Action#invokeOn()`].  This allows the user to select multiple objects in a table and then invoke the same action against all of them.
...
----
<1> is xref:_ug_reference-annotations_manpage-RequestScoped[`@RequestScoped`], so this domain service instance is scoped to a particular request and is then destroyed
...
One idiom is for the domain objects to also use the xref:_ug_reference-services-api_manpage-Scratchpad[`Scratchpad`] service to share information, for example to aggregate values. The `ActionInvocationContext#isLast()` method can then be used to determine if all the information has been gathered, and then do something with it (eg derive variance across a range of values, render a graph etc).
...
<2> otherwise (if invoked on collection of objects), return null, so that the xref:_ug_wicket-viewer[Wicket viewer] will re-render the list of objects
