...
The typical way to indicate that an action should be treated as a command is to annotate it with the xref:_ug_reference-annotations_manpage_Action_command[`@Action#command()`] annotation, for example:
...
For some applications it may be required to record (as commands) only those actions that update objects; in other words queries and searches can be ignored. Put another way, only actions annotated (with xref:_ug_reference-annotations_manpage_Action_semantics[`@Action#semantics()`]) as IDEMPOTENT or NON_IDEMPOTENT should be treated as commands. In this case, use:
