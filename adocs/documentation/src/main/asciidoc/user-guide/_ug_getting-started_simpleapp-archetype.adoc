...
The `simpleapp` archetype generates a single WAR file, configured to run both the <<_wicket_viewer, Wicket viewer>> and the <<_restful_objects_viewer, Restful Objects viewer>>.  The archetype also configures the [JDO Objectstore](../../components/objectstores/jdo/about.html) to use an in-memory HSQLDB connection.
...
Most of the time, though, you'll probably want to run the app from within your IDE.  The mechanics of doing this will vary by IDE; see the appendices for details on <<_setting_up_eclipse, setting up Eclipse>> or <<_setting_up_intellij_idea, setting up IntelliJ IDEA>>.  Basically, though, it amounts to running `org.apache.isis.WebServer`, and ensuring that the <<_datanucleus_enhancer, DataNucleus enhancer>> has properly processed all domain entities.
...
It is also possible to start the application with a pre-defined set of data; useful for demos or manual exploratory testing.  This is done by specifying a <<_fixture_scripts, fixture script>> on the command line.
...
When you start the app, you'll be presented with a welcome page from which you can access the webapp using either the <<_wicket_viewer, Wicket viewer>> or the <<_restful_objects_viewer, Restful Objects viewer>>:
...
The app itself is configured to run using <<_security, shiro security>>, as configured in the `WEB-INF/shiro.ini` config file.  You can login with:
