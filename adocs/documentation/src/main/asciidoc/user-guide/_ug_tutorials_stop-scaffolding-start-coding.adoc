...
** see also the docs on xref:_ug_reference-object-layout_static[static layouts]
...
** use a xref:_ug_reference-methods_prefixes_manpage-autoComplete[`autoComplete...()`] supporting method
*** on a property
*** on an action parameter
...
* to create objects, xref:_ug_how-tos_class-structure_inject-services[inject] associated domain service
...
* Delete the various hint annotations and instead specify layout hints using a xref:_ug_reference-object-layout_dynamic[.layout.json] file.
