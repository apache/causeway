= anchor:tutorials[]Stop scaffolding, start coding
...
There's a bit of overlap with the xref:_pet_clinic[Pet Clinic] tutorial initially, but it then sets off on its own.
...
Set up xref:_setting_up_a_development_environment[an IDE] and import the project to be able to run and debug the app.

Then set up a launch configuration and check that you can:

* Run the app from within the IDE
* Run the app in debug mode
* Run with different deploymentTypes; note whether prototype actions (those annotated xref:__code_action_code[`@Action(restrictTo=PROTOTYPING`]) are available or not:
...
* specify a xref:_how_to_specify_a_title_for_a-domain-entity[title]
* specify an xref:_how_to_specify_an_icon_for_a-domain-entity[icon]
...
** as per the docs describing xref:_how_to_create_or_delete_objects_within_your_code[how to create or delete objects]
* rename if you wish
** eg `newPet(...)` or `addPet(...)`
* review `listAll` action (acting as a repository)
* as per the docs describing xref:_how_to_write_a_custom_repository[how to write a custom repository]
* note the annotations on the corresponding domain class (originally called `SimpleObject`, though renamed by now, eg to `Pet`)
* rename if you wish
** eg `listPets()`
* note the xref:__code_domainservice_code[`@DomainService`] annotation
...
* use the injected xref:__code_isisjdosupport_code[`IsisJdoSupport`] domain service.
...
* use the xref:__code_parameterlayout_code[`@ParameterLayout(named=...)`] annotation to specify the name of action parameters
* use the xref:__code_action_code[`@Action(semanticsOf=...)`]  annotation to indicate the semantics of the action (safe/query-only, idempotent or non-idempotent)
* annotate safe action as bookmarkable using xref:__code_actionlayout_code[`@ActionLayout(bookmarking=...)`]
* confirm is available from bookmark panel (top-left of Wicket UI)
...
* experiment changing xref:__code_action_code[`@Action(semantics=...)`] on actions
...
* add some xref:_how_to_add_a_property_to_a_domain_entity[value properties]; also:
* for string properties
** use the xref:__code_Property_code[`@Property(multiLine=...)`] annotation to render a text area instead of a text box
** use the xref:__code_Property_code[`@Property(maxLength=...)`] annotation to specify the maximum number of characters allowable
** use joda date/time properties, bigdecimals and blob/clob properties
* use the xref:__code_Property_code[`@Column(allowsNull=...)`] annotation specify whether a property is optional or mandatory
...
* revisit the title, consider whether to use the xref:__code_Title_code[`@Title`] annotation
** rather than the xref:__code_title_code[`title()`] `title()` method
* order the properties using the xref:__code_MemberOrder_code[`@MemberOrder`], also `@MemberGroupLayout`
** see also the docs on xref:_static_layouts[static layouts]
* use the xref:__code_PropertyLayout_code[`@PropertyLayout`] annotation to position property/action parameter labels either to the LEFT, TOP or NONE
** do the same for parameters using xref:__code_ParameterLayout_code[`@ParameterLayout`]
...
* add some xref:_how_to_add_a_property_to_a_domain_entity[reference properties]
...
** use the xref:__code_domainobjectlayout_code[`@DomainObjectLayout(bounded=...)`] annotation on the referenced type if there are only a small number (bounded) of instances
** use a xref:__code_choices_code[`choices...()`] supporting method
*** on a xref:_how_to_specify_a_set_of_choices_for_a_property[property]
*** on a xref:_how_to_specify_a_set_of_choices_for_a_parameter[action parameter]
** use a xref:__code_autoComplete_code[`autoComplete...()`] supporting method
*** on a xref:_how_to_specify_a_set_of_autoComplete_for_a_property[property]
*** on a xref:_how_to_specify_a_set_of_autoComplete_for_a_parameter[action parameter]
...
* Add xref:_how_to_specify_default_values_for_a_parameter[defaults] for action parameters
...
** use the xref:__code_objectcontracts_code[`ObjectContracts`] utility class to help implement `Comparable`
*** you can also `equals()`, `hashCode()`, `toString()`
* Add a xref:_how_to_create_a_1_to_m_collection[collection] to one of the entities
** Use `SortedSet` as the class
** Use the xref:__code_collectionlayout_code[`@CollectionLayout(render=...)`] annotation to indicate if the collection should be visible or hidden by default
* optional: use the xref:__code_collectionlayout_code[`@CollectionLayout(sortedBy=...)`] annotation to specify a different comparator than the natural ordering
...
** generally we recommend using the xref:__code_inject_code[`@Inject`] annotation with either private or default visibility
* the service itself should use xref:__code_domainobjectcontainer_code[`DomainObjectContainer`]
* use the xref:__code_memberorder_code[`@MemberOrder(name=...)`] annotation to associate an action with a property or with a collection
...
* the bootstrap "btn" CSS classes can be used using the xref:__code_actionlayout_code[`@ActionLayout(cssClass=...)`] annotation

*  the http://fortawesome.github.io/Font-Awesome/icons/[Font Awesome] icons can be used using the xref:__code_actionlayout_code[`@ActionLayout(cssClassFa=...)`]

It's also possible to use Font Awesome icons for the xref:_how_to_specify_the_icon_for_a_domain_entity[domain object icon].
...
* Delete the various hint annotations and instead specify layout hints using a xref:_dynamic_layouts[.layout.json] file.
...
* Use the xref:__code_property_code[`Property(hidden=...)`] annotation to make properties invisible
** likewise xref:__code_collection_code[`@Collection(hidden=...)`] for collections
** the xref:__code_programmatic_code[`@Programmatic`]  annotation can also be used and in many cases is to be preferred; the difference is that the latter means the member is not part of the Isis metamodel.
* Use the xref:__code_hide_code[`hide...()`] supporting method on xref:_how_to_hide_a_property[properties], xref:_how_to_hide_a_collection[collections] and xref:_how_to_hide_an_action[actions] to make a property/collection/action invisible according to some imperative rule


=== Use it!

* Use the xref:__code_property_code[`Property(editing=...)`] annotation to make property read-only
** likewise xref:__code_collection_code[`@Collection(editing=...)`] for collections
** alternatively, use xref:__code_domainobject_code[`@DomainObject(editing=...)`] to disable editing for all properties/collections
* Use the xref:__code_disable_code[`disable...()`] supporting method on xref:_how_to_prevent_a_property_from_being_modified[properties] and xref:_how_to_prevent_an_action_from_being_invoked[actions] to make a property/action disabled according to some imperative rule


=== Do it!

* Validate string properties or action parameters:
* use the xref:__code_property_code[`@Property(regex=...)`] annotation to specify a pattern
** and xref:<<__code_parameter_code[`@Parameter(regex=...)`] for parameters
* use the xref:__code_property_code[`@Property(minLength=...)`] annotation to indicate a minimum number of characters
** and xref:<<__code_parameter_code[`@Parameter(minLength=...)`] for parameters
* Use the `validateXxx()` supporting method on xref:_how_to_validate_user_input_for_a_property[properties]] or xref:_how_to_validate_an_action_parameter_argument[action parameter]
...
* Add the xref:__code_homepage_code[`@HomePage`] annotation to one (no more) of the domain services' no-arg actions
...
* inject xref:__code_clockservice_code[`ClockService`]
...
** annotate with xref:__code_domainservice_code[`@DomainService(natureOfService=VIEW_CONTRIBUTIONS_ONLY)`]
...
* For this action:
** add the xref:__code_actionlayout_code[`@ActionLayout(contributedAs=ASSOCIATION)`] annotation
...
By default the events generated are `ActionDomainEvent.Default` (for actions) and `PropertyDomainEvent.Default` (for properties). Subclasses of these can be specified using the xref:__code_action_code[`@Action(domainEvent=...)`] or `Property(domainEvent=...)` for properties.


Using the guidance in the docs for the xref:__code_eventbusservice_code[`EventBusService`]:
...
Bulk actions are actions that can be invoked on a collection of actions, that is on collections returned by invoking an action. Actions are specified as being bulk actions using the xref:__code_action_code[`@action(invokeOn=OBJECT_AND_COLLECTION)`] annotation.
...
* Inject the xref:__code_actioninteractioncontext_code[`ActionInteractionContext`] (request-scoped) service
...
The similar xref:__code_scratchpad_code[`ScratchPad`] (request-scoped) domain service is a good way to share information between bulk action invocations:
...
The xref:__code_queryresultscache_code[`QueryResultsCache`] (request-scoped) domain service allows arbitrary objects to be cached for the duration of a request.
...
Each element in the Wicket viewer (entity form, properties, collections, action button etc) is a component, each created by a internal API (`ComponentFactory`, described xref:_extending_the_wicket_viewer[here]). For collections there can be multiple views, and the Wicket viewer provides a view selector drop down (top right of each collection panel).
...
To support these use cases, Isis therefore allows you to write a view model, either by annotating the class with xref:__code_viewmodel_code[`@ViewModel`] or (for more control) by implementing the `ViewModel` interface.
...
Isis has great support for writing xref:_integ_test_support[integration tests]; well-written integration tests should leverage fixture scripts and use the xref:__code_wrapperfactory_code[`@WrapperFactory`] domain service.
...
* as per xref:_restful_objects_viewer[the RO viewer's docs], configure the Restful Objects viewer to generate a simplified object representation: +
+
[source,ini]
----
