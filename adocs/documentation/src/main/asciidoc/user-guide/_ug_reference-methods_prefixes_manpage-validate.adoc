[[_ug_reference-methods_prefixes_manpage-validate]]
= `validate...()`
...
The reason for vetoing a modification/invocation is normally returned as a string.  However, Isis' xref:_ug_more-advanced_i18n[i18n support] extends this so that reasons can be internationalized if required.
