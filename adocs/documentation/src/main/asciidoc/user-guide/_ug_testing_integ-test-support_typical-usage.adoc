[[_ug_testing_integ-test-support_typical-usage]]
= Typical Usage
...
----
<1> the xref:_ug_reference-classes_super_manpage-FixtureScripts[`FixtureScripts`] domain service is injected, providing us with the ability to run fixture scripts
...
<2> the http://github.com/isisaddons/isis-app-todoapp[todoapp] uses link:http://assertj.org[AssertJ].
