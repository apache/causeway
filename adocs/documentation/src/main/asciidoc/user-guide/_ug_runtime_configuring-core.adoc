...
|`configuration-and-annotation`, `configuration`, `FQCN` (`configuration`)
...
|The hostname of the external SMTP provider.  +
...
`email.port` +
| port number (`_587_`)
|The port number for the SMTP service on the the external SMTP host.  +
...
| email address
|The email address to use for sending out email. This property must be configured in order to use xref:_ug_reference-services-api_manpage-EmailService[`EmailService`].  +
...
| email password
|The corresponding password for the email address to use for sending out email.  This property must be configured in order to use xref:_ug_reference-services-api_manpage-EmailService[`EmailService`]. +
...
|which implementation to use as the underlying event bus.

| `isis.services.` +
`eventbus.allowLateRegistration` +
| `true`, `false`, (`_false_`)
|whether a domain service can register after any events have posted. +
