[[_ug_reference-annotations_manpage-DomainObject_nature]]
= `nature()`
...
Those natures that indicate the domain object is an entity (of some sort or another) mean then that the domain object is considered to be part of the domain model layer.  As such the domain object's class cannot be annotated with xref:_ug_reference-annotations_manpage-ViewModel[`@ViewModel`] or implement the xref:_ug_reference-classes_super_manpage-ViewModel[`ViewModel`] interface.
