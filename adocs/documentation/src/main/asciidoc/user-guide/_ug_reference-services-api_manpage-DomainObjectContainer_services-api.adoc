...
The primary use case is to instantiate domain objects using a regular constructor ("new is the new new") rather than using the xref:_ug_reference-services-api_manpage-DomainObjectContainer_object-creation-api[`#newTransientInstance()`] API, and then using the `#injectServicesInto(...)` API to set up any dependencies.
