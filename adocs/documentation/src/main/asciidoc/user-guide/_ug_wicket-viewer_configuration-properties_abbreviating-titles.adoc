...
Objects whose title is overly long can be cumbersome in titles.  The Wicket viewer has a xref:_ug_wicket-viewer_features_titles-in-tables[mechanism to automatically shorten] the titles of objects specified using `@Title`.  As an alternative/in addition, the viewer can also be configured to simply truncate titles longer than a certain length.
