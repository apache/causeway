...
`JUnitRuleMockery2` is an extension to the link:http://www.jmock.org/[JMock]'s `JunitRuleMockery`, providing a simpler API and also providing support for autowiring.

[TIP]
====
We're fans of JMock, and we're even bigger fans of JMock's authors, Steve Freeman and Nat Pryce.  If testing is even a little bit of interest to you, check out their great link:http://www.growing-object-oriented-software.com/[GOOS] book, and check out their blogs, in particular link:http://natpryce.com/articles.html[Nat's].
====
...
[NOTE]
====
Isis also includes (and automatically uses) a link:http://www.csg.ci.i.u-tokyo.ac.jp/~chiba/javassist/[Javassist]-based implementation of JMock's link:http://www.jmock.org/mocking-classes.html[`ClassImposteriser`] interface, so that you can mock out concrete classes as well as interfaces.  We've provided this rather than JMock's own cglib-based implementation (which is problematic for us given its own dependencies on link:http://asm.ow2.org/[asm]).
====
...
Apache Isis' unit test support is automatically configured if you use the <<_simpleapp_archetype, SimpleApp archetype>>.  To set it up manually, update the `pom.xml` of your domain object model module:
