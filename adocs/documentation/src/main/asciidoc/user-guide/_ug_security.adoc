...
Shiro ships with a simple text-based realm -- the `IniRealm` -- which reads users (and password), user roles and role permissions from the `WEB-INF/shiro.ini` file.  The xref:_simpleapp_archetype[SimpleApp archetype] is configured to use this realm.
