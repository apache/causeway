[[_ug_reference-methods_prefixes_manpage-default]]
= `default...()`
...
Defaults are also supported (of course) for xref:_ug_how-tos_contributed-members[contributed actions].  For example, here is a contributed action for updating category/subcategory of the (non-ASF) http://github.com/isisaddons/isis-app-todoapp[Isis addons' todoapp]:
...
The first is to use the xref:_ug_reference-methods_lifecycle_manpage-created[`created()`] callback, called by the framework when xref:_ug_reference-services-api_manpage-DomainObjectContainer[`DomainObjectContainer#newTransientInstance(...)`] is called.  This method is called after any dependencies have been injected into the service.
