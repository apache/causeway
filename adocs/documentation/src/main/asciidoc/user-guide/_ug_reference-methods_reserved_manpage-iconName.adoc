[[_ug_reference-methods_reserved_manpage-iconName]]
= `iconName()`
...
Every object is represented by an icon; this is based on the domain object's simple name.  The xref:_ug_wicket-viewer[Wicket viewer] searches for the image in the same package as the `.class` file for the domain object.
