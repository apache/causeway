= anchor:security[]Usage by Isis' Viewers
...
By and large the security mechanisms within Isis are transparent to the rest of the framework (the xref:_wicket_viewer[Wicket Viewer] and xref:_restful_objects_viewer[Restful viewer], and the overall xref:_runtime[runtime]).
...
The xref:_wicket_viewer[Wicket viewer] defines a relatively small number of pages (by which we mean subclasses of link:http://ci.apache.org/projects/wicket/apidocs/6.0.x/org/apache/wicket/markup/html/WebPage.html[`org.apache.wicket.markup.html.WebPage`]):
...
[TIP]
====
The sign-in page to render is pluggable; see xref:_custom_pages[extensions chapter] for details.
====
...
[TIP]
====
The sign-in verification page to render is pluggable; see xref:_custom_pages[extensions chapter] for details.
====
