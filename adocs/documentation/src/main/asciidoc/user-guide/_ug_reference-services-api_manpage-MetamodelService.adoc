[[_ug_reference-services-api_manpage-MetamodelService]]
= `MetamodelService`
...
Currently this is limited to looking up the object type (as specified in xref:_ug_reference-annotations_manpage-DomainObject_objectType[`@DomainObject#objectType()`] and equivalent mechanisms, and as used in  xref:_ug_reference-services-api_manpage-BookmarkService[`Bookmark`]s and elsewhere) from an object's class, and vice versa.  In the future we expect other aspects of the metamodel to also be formally surfaced through this API.
