...
Going the other way -- getting hold of the underlying (wrapped) domain object -- is just as easy; call `WrapperFactory#unwrap(...)`.
...
----
<1> inject <<__code_EventBusService_code, `EventBusService`>> into this test
