...
In such cases, though, it would probably make more sense to annotate the action as either xref:_ug_reference-annotations_manpage-Action_hidden[hidden] or indeed xref:_ug_reference-annotations_manpage-Programmatic[`@Programmatic`].



[NOTE]
====
Unlike other `@ActionLayout` attributes, this attribute _cannot_ be specified dynamically in the `.layout.json` xref:_ug_wicket-viewer_layout_dynamic-object-layout[dynamic layout file] because it relates to the contributor domain service, not the contributee domain object.
====
