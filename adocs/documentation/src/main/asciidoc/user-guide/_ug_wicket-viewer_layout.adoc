...
Using the (third-party) link:http://github.com/isisaddons/isis-jrebel-plugin[Isis addons' jrebel] plugin (non-ASF) comes close to getting the best of both words: metadata is specified in a type-safe way using annotations, but can be reloaded automatically.
