...
The `publishing()` attribute determines whether and how a modified object instance is published via the registered implementation of a xref:_ug_reference-services-spi_manpage-PublishingService[`PublishingService`]).  A common use case is to notify external "downstream" systems of changes in the state of the Isis application.
