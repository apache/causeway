...
Furthermore, DataNucleus will search for various other XML mapping files, eg `mappings.jdo`.  A full list can be found http://www.datanucleus.org/products/datanucleus/jdo/metadata.html[here].  The metadata in these XML can be used to override the annotations of annotated entities; see xref:_ug_more-advanced_overriding-jdo-annotations[Overriding JDO Annotatons] for further discussion.
