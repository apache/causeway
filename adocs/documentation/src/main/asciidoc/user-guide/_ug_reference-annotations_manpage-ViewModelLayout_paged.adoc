...
The `paged()` attribute specifies the number of rows to display in a standalone collection, as returned from an action invocation.  This attribute can also be applied to xref:_ug_reference-annotations_manpage-CollectionLayout_paged[collections] and xref:_ug_reference-annotations_manpage-DomainObjectLayout_paged[domain objects].
