...
| +ve integer, (`_12_`)
| See discussion <<_abbreviating_suppressing_titles_in_tables, below>>.
...
Objects whose title is overly long can be cumbersome in titles.  The Wicket viewer has a <<_titles_in_tables, mechanism to automatically shorten>> the titles of objects specified using `@Title`.  As an alternative/in addition, the viewer can also be configured to simply truncate titles longer than a certain length.
