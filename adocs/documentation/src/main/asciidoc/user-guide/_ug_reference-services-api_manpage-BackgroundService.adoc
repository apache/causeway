...
The `BackgroundService`, and the companion xref:_ug_reference-services-spi_manpage-BackgroundCommandService[`BackgroundCommandService`] SPI service (used by Isis itself), enable action invocations to be persisted such that they may be invoked in the background.
...
The default `BackgroundServiceDefault` implementation (provided by isis-core) works by using a proxy wrapper around the target so that it can capture the action to invoke and its arguments using (a private copy of) xref:_ug_reference-services-api_manpage-MementoService[`MementoService`].  The persistence delegates the persistence of the memento to an appropriate implementation of the companion `BackgroundCommandService`.  One such implementation of `BackgroundCommandService` is provided by (non-ASF) http://github.com/isisaddons/isis-module-command[Isis addons' command] module.
...
The action method invoked must be part of the Isis metamodel, which is to say it must be public, accept only scalar arguments, and must not be annotated with xref:_ug_reference-annotations_manpage-Programmatic[`@Programmatic`] or `@Ignore`. However, it may be annotated with xref:_ug_reference-annotations_manpage-Action_hidden[`@Action#hidden()`] or xref:_ug_reference-annotations_manpage-ActionLayout_hidden[`@ActionLayout#hidden()`] and it will still be invoked.
...
[TIP]
====
If you want to check business rules, you can use xref:_ug_reference-annotations_manpage-WrapperFactory[`@WrapperFactory#wrapNoExecute(...)`].
====
...
However, an alternative is to rely on the fact that the `BackgroundService` will automatically hint that the `Command` representing the original interaction (to `submitCustomerInvoices()` in the example above) should be persisted. This will be available if the related xref:_ug_reference-services-api_manpage-CommandContext[`CommandContext`] and xref:_ug_reference-services-spi_manpage-CommandService[`CommandService`] domain services are configured, and the `CommandService` supports persistent commands.  Note that (non-ASF) http://github.com/isisaddons/isis-module-command[Isis addons' command] module does indeed provide such an implementation of `CommandService` (as well as of the required `BackgroundCommandService`).
...
----
<1> the injected xref:_ug_reference-services-api_manpage-CommandContext[`CommandContext`] domain service.
...
This service is closely related to the xref:_ug_reference-services-api_manpage-CommandContext[`CommandContext`] and also that service's supporting xref:_ug_reference-services-spi_manpage-CommandService[`CommandService`] service.
...
The implementations of `CommandService` and `BackgroundCommandService` go together; typically both parent `Command`s and child background `Command`s will be persisted in the same way. The (non-ASF) http://github.com/isisaddons/isis-module-command[Isis addons' command] module provides implementations of both (see xref:_ug_reference-services-spi_manpage-CommandService[`CommandService`] and xref:_ug_reference-services-spi_manpage-BackgroundCommandService[`BackgroundCommandService`]).


include::_ug_reference-services-api_manpage-BackgroundService_BackgroundCommandExecution.adoc[leveloffset=+1]
include::_ug_reference-services-api_manpage-BackgroundService_Quartz.adoc[leveloffset=+1]
