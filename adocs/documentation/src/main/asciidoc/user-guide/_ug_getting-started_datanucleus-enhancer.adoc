...
[TIP]
====
The configuration described below is automatically set up by the xref:_simpleapp_archetype[SimpleApp archetype].
====
...
The xref:_simpleapp_archetype[SimpleApp archetype]  sets up the plugin correctly in the `dom` (domain object model) module.  (It's actually a little bit more complex to cater for users of the Eclipse IDE using Eclipse's m2e plugin).
...
Again, the xref:_simpleapp_archetype[SimpleApp archetype] does this.
