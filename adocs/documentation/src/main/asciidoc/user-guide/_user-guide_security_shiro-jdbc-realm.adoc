...
jdbcRealm.authenticationQuery =         \              # <5>
        select password                 \
          from users                    \
         where username = ?

jdbcRealm.userRolesQuery =              \              # <6>
        select r.label                  \
          from users_roles ur           \
    inner join roles r                  \
            on ur.role_id = r.id        \
...
jdbcRealm.permissionsLookupEnabled=true                 # <8>
----
<5> query to find password for user
<6> query to find roles for user
<7> query to find permissions for role
<8> enable permissions lookup
...
[source,ini]
----
dps = org.apache.shiro.authc.credential.DefaultPasswordService   # <9>
pm = org.apache.shiro.authc.credential.PasswordMatcher           # <10>
pm.passwordService = $dps
jdbcRealm.credentialsMatcher = $pm                               # <11>
----
<9> mechanism to encrypts password
<10> service to match passwords
<11> instruct JDBC realm to use password matching service when authenticating
...
[source,ini]
----
securityManager.realms = $jdbcRealm                     # <12>
----
