...
[NOTE]
====
This technique is much less powerful than using xref:_ug_more-advanced_decoupling_event-bus[the event bus] .  We present it mostly for completeness.
====
...
<3> the xref:_ug_reference-methods_prefixes_manpage-addTo[`addTo...()`] method increments the derived properties
<4> the xref:_ug_reference-methods_prefixes_manpage-removeTo[`removeTo...()`] method similarly decrements the derived properties
