...
* a system initializer class.  You can reuse the  system initializer from any integration tests (as described in <<_integration_test_support, Integration Test Support>>, bootstrapping section).


Here's an example of a feature from the <<_simpleapp_archetype, SimpleApp archetype>>:
...
The JSON formatter allows integration with enhanced reports, for example as provided by http://www.masterthought.net/section/cucumber-reporting[Masterthought.net] (screenshots at end of page). (Commented out) configuration for this is provided in the <<_simpleapp_archetype, SimpleApp archetype>>.
