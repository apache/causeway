[[_ug_reference-services-spi_manpage-EventSerializer]]
= `EventSerializer`
...
The `EmailSerializer` service is a supporting service intended for use by (any implementation of) xref:_ug_reference-services-spi_manpage-PublishingService[`PublishingService`].  Its responsibility is to combine the `EventMetadata` and the `EventPayload` into some serialized form (such as JSON, XML or a string) that can then be published.

See xref:_ug_reference-services-spi_manpage-PublishingService[`PublishingService`] for further discussion.
...
<2> standard metadata about the event, such as the user, the xref:_ug_reference-classes_mixins_manpage-HasTransactionId[`transactionId`], date/time etc
...
This service is intended (though not mandated) to be used by implementations of xref:_ug_reference-services-spi_manpage-PublishingService[`PublishingService`].