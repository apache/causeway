[[_ug_reference-services-spi_manpage-ContentMappingService]]
...
The `ContentMappingService` supports the (default implementation of the) xref:_ug_reference-services-spi_manpage-ContentNegotiationService[`ContentNegotiationService`] allowing the xref:_ug_restfulobjects-viewer[RestfulObjects viewer] to allow domain objects to be transformed into some other format as specified by the HTTP `Accept` header.

See xref:_ug_reference-services-spi_manpage-ContentNegotiationService[`ContentNegotiationService`] for further discussion.
...
<9> and map its oid (by way of the xref:_ug_reference-services-api_manpage-BookmarkService[`BookmarkService`]
...
This service is a companion to the default implementation of the xref:_ug_reference-services-spi_manpage-ContentNegotiationService[`ContentNegotiationService`].