...
[TIP]
====
If all that is required is a very simple representations (of objects), you can instead reconfigure RO viewer to provide a simplified output, as described xref:_ug_restfulobjects-viewer_configuration-properties_simplified-object-representation[here].  Or, it is also possible to simply suppress certain elements, as described  xref:_ug_restfulobjects-viewer_configuration-properties_suppressing-elements[here].
====
