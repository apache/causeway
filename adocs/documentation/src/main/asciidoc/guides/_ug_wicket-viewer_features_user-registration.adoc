[[_ug_wicket-viewer_features_user-registration]]
= User Registration
...
* the xref:rg.adoc#_rg_services-spi_manpage-UserRegistrationService[user registration service], which provides an API to create the user account
* the xref:rg.adoc#_rg_services-spi_manpage-EmailNotificationService[email notification service], which provides an API for to send the verification emails
* the xref:rg.adoc#_rg_services-api_manpage-EmailService[email service], that is used by the email notification service to actually send the email.
...
* register an implementation of the xref:rg.adoc#_rg_services-spi_manpage-UserRegistrationService[user registration service] (eg by using the https://github.com/isisaddons/isis-module-security[Isis addons security module])

* configure the xref:rg.adoc#_rg_services-api_manpage-EmailService[email service]
...
It is also possible to configure the Wicket viewer to suppress xref:ug.adoc#_ug_wicket-viewer_configuration-properties_suppressing-sign-up[the sign-up page link] and/or the xref:ug.adoc#_ug_wicket-viewer_configuration-properties_suppressing-password-reset[password reset page].
