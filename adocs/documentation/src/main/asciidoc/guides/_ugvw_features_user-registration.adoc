...
* the xref:rgsvc.adoc#_rgsvc-spi_UserRegistrationService[user registration service], which provides an API to create the user account
* the xref:rgsvc.adoc#_rgsvc-spi_EmailNotificationService[email notification service], which provides an API for to send the verification emails
* the xref:rgsvc.adoc#_rgsvc-api_EmailService[email service], that is used by the email notification service to actually send the email.
...
* register an implementation of the xref:rgsvc.adoc#_rgsvc-spi_UserRegistrationService[user registration service] (eg by using the https://github.com/isisaddons/isis-module-security[Isis addons security module])

* configure the xref:rgsvc.adoc#_rgsvc-api_EmailService[email service]
