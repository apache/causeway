[[_ugbtb_more-advanced_view-models_jaxb]]
...
from the xref:rgsvc.adoc#_rgsvc-api_manpage-BookmarkService[`BookmarkService`].
...
form, by way of the xref:rgsvc.adoc#_rgsvc-spi_manpage-UrlEncodingService[`UrlEncodingService`].
...
These use the xref:rgsvc.adoc#_rgsvc-api_manpage-JaxbService[`JaxbService`] to generate the schemas; using the
