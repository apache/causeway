...
[NOTE]
====
Note that there is (currently) no support for specifying UI hints for domain objects through the dynamic xref:ug.adoc#_ug_object-layout_dynamic[`.layout.json`] file (only for properties, collections and actions are supported).
====
