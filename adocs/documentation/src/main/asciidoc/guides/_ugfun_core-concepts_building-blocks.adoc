...
xref:ug.adoc#_ugfun_object-layout[layout hints], such as `Customer.layout.json`.  These provide metadata such as
...
xref:rgsvc.adoc#_rgsvc-api_Scratchpad[`Scratchpad`] service, for sharing arbitrary data between multiple objects.
...
For further discussion on view models, see xref:ugbtb.adoc#_ugbtb_more-advanced_view-models[this topic].
...
For more information, see xref:ugbtb.adoc#_ugbtb_more-advanced_decoupling_contributions[this topic on contribution]s, and
xref:ugbtb.adoc#_ugbtb_more-advanced_decoupling_mixins[this topic on mixin]s.
...
As well as defining a xref:ug.adoc#_ugfun_core-concepts_building-blocks_metamodel[metamodel] of the structure (domain
...
xref:rgsvc.adoc#_rgsvc-spi_AuditingService[`AuditingService`], in fact).
...
xref:rgsvc.adoc#_rgsvc-api_BookmarkService[`BookmarkService`].  Deep links meanwhile can be obtained from
