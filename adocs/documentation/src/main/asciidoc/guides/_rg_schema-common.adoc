...
xref:rgant.adoc#_rgant_manpage-DomainObject_objectType[`@DomainObject#objectType()`] attribute, or to the (JDO)
xref:rgant.adoc#_rgant_manpage-PersistenceCapable[`@PersistenceCapable`] annotation (`schema` and/or `table`
attributes), or to the (JDO) xref:rgant.adoc#_rgant_manpage-Discriminator[`@Discriminator`] annotation.  If none
...
xref:rgant.adoc#_rgant_manpage-XmlRootElement[`@XmlRootElement`]), that reference domain entities. +
+
