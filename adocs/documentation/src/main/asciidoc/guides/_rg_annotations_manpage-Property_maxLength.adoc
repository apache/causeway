...
That said, properties are most commonly defined on persistent domain objects (entities), in which case the JDO xref:rg.adoc#_rg_annotations_manpage-Column[`@Column`] will in any case need to be specified.  Apache Isis can infer the `maxLength` semantic directly from the equivalent `@Column#length()` annotation/attribute.
