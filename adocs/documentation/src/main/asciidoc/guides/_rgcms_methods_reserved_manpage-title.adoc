[[_rgcms_methods_reserved_manpage-title]]
= `title()`
...
----
<1> simple xref:rgcms.adoc#_rgcms_classes_utility_manpage-TitleBuffer[tility class] to help construct the title string
<2> imperative conditional logic
<3> using the xref:rgsvc.adoc#_rgsvc-api_WrapperFactory[`WrapperFactory`] to determine if the `dueBy` field is visible for this user ...
<4> ... but ignore if not
