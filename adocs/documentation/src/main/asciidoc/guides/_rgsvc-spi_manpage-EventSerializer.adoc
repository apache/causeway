[[_rgsvc-spi_manpage-EventSerializer]]
= `EventSerializer`
...
The `EmailSerializer` service is a supporting service intended for use by (any implementation of) xref:rgsvc.adoc#_rgsvc-spi_manpage-PublishingService[`PublishingService`].  Its responsibility is to combine the `EventMetadata` and the `EventPayload` into some serialized form (such as JSON, XML or a string) that can then be published.

See xref:rgsvc.adoc#_rgsvc-spi_manpage-PublishingService[`PublishingService`] for further discussion.
...
<2> standard metadata about the event, such as the user, the xref:rg.adoc#_rg_classes_mixins_manpage-HasTransactionId[`transactionId`], date/time etc
...
implementation of the xref:rgsvc.adoc#_rgsvc-spi_manpage-PublishingService[`PublishingService`].
...
This service is intended (though not mandated) to be used by implementations of xref:rgsvc.adoc#_rgsvc-spi_manpage-PublishingService[`PublishingService`].  The (non-ASF) http://github.com/isisaddons/isis-module-publishing[Isis addons' publishing] module does use it (though the (non-ASF)
