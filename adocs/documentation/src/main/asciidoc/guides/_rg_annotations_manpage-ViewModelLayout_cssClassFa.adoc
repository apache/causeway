[[_rg_annotations_manpage-ViewModelLayout_cssClassFa]]
= `cssClassFa()`
...
These attribute can also be applied to xref:rg.adoc#_rg_annotations_manpage-DomainObjectLayout_cssClassFa[domain objects] to specify the object's icon, and to xref:rg.adoc#_rg_annotations_manpage-ActionLayout_cssClassFa[actions] to specify an icon for the action's representation as a button or menu item.
...
[NOTE]
====
The similar xref:rg.adoc#_rg_annotations_manpage-ViewModelLayout_cssClass[`@ViewModelLayout#cssClass()`] annotation attribute is also used as a hint to apply CSS, but for wrapping the representation of an object or object member so that it can be styled in an application-specific way.
====
