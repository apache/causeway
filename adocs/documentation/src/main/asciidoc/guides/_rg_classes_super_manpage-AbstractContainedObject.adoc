[[_rg_classes_super_manpage-AbstractContainedObject]]
...
Each of these methods simply delegates to an equivalent method in xref:rg.adoc#_rg_services-api_manpage-DomainObjectContainer[`DomainObjectContainer`].

[TIP]
====
In practice we find that there's little to gain from subclassing; it's easier/less obscure to simply inject xref:rg.adoc#_rg_services-api_manpage-DomainObjectContainer[`DomainObjectContainer`] into a simple pojo class.
====
