[[_rgant_manpage-CollectionLayout_render]]
= `render()`
...
Also, if a xref:rgant.adoc#_rgant_manpage-CollectionLayout_defaultView[`@CollectionLayout#defaultView()`] attribute has been specified then that will take precedence over the value of the `render()` attribute.
====
