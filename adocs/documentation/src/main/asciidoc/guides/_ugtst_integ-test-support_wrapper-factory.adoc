...
The xref:rgsvc.adoc#_rgsvc-api_WrapperFactory[`WrapperFactory`] service is responsible for wrapping a domain object in a dynamic proxy, of the same type as the object being proxied.  And the role of this wrapper is to simulate the UI.
...
----
<1> inject xref:rgsvc.adoc#_rgsvc-api_EventBusService[`EventBusService`] into this test
