...
The CDI annotation xref:rgant.adoc#_rgant_manpage-RequestScoped[`@javax.enterprise.context.RequestScoped`] is used to indicate this fact:
...
which contain xref:rgant.adoc#_rgant_manpage-DomainService[`@DomainService`]-annotated classes.
...
The framework will call any `public` method annotated with xref:rgant.adoc#_rgant_manpage-PostConstruct[`@PostConstruct`] with either no arguments of an argument of type `Map<String,String>`
...
Shutdown is similar; the framework will call any method annotated with xref:rgant.adoc#_rgant_manpage-PreDestroy[`@PreDestroy`].
