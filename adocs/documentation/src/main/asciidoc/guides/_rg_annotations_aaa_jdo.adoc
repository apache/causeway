...
If the `schema()` attribute is specified (and if xref:rg.adoc#_rg_annotations_manpage-Discriminator[`@Discriminator`] _hasn't_ been specified), is also used to derive the object type, as used in `Bookmark`s, URLs for xref:ugvro.adoc[RestfulObjects viewer] and elsewhere.
