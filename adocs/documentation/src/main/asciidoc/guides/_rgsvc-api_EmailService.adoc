...
The email service is used by the xref:rgsvc.adoc#_rgsvc-spi_EmailNotificationService[`EmailNotificationService`] which is, in turn, used by xref:rgsvc.adoc#_rgsvc-spi_UserRegistrationService[`UserRegistrationService`].