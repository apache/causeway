...
<6> skip all business rules and then execute (including creating xref:rgant.adoc#_rgant_manpage-Action_command[command]s and firing pre- and post-execute xref:rgant.adoc#_rgant_manpage-Action_domainEvent[domain event]s).
<7> validate all business rules (including those from xref:rgant.adoc#_rgant_manpage-Action_domainEvent[domain event]s) but do not execute.
