[[_rg_services-api_manpage-WrapperFactory]]
= `WrapperFactory`
...
[NOTE]
====
For a discussion of the use of the `WrapperFactory` within integration tests (the primary or at least original use case for this service) can be found xref:ug.adoc#_ug_testing_integ-test-support_wrapper-factory[here]
====
...
This capability is frequently used within xref:ug.adoc#_ug_testing_integ-test-support[integration tests], but can also be used in production code. (There are analogies that can be drawn here with the way that JEE beans can interact through an EJB local interface).
...
<6> skip all business rules and then execute (including creating xref:rg.adoc#_rg_annotations_manpage-Action_command[command]s and firing pre- and post-execute xref:rg.adoc#_rg_annotations_manpage-Action_domainEvent[domain event]s).
<7> validate all business rules (including those from xref:rg.adoc#_rg_annotations_manpage-Action_domainEvent[domain event]s) but do not execute.
...
[NOTE]
====
It ought to be possible to implement an xref:rg.adoc#_rg_services-spi_manpage-ExceptionRecognizer[`ExceptionRecognizer`]s that would allow the above boilerplate to be removed.  This recognizer service would recognize the `InteractionException` and convert to a suitable message.
