[[_ug_security_usage-by-isis-viewers]]
...
The xref:ug.adoc#_ug_wicket-viewer[Wicket viewer] defines a relatively small number of pages (by which we mean subclasses of link:http://ci.apache.org/projects/wicket/apidocs/6.0.x/org/apache/wicket/markup/html/WebPage.html[`org.apache.wicket.markup.html.WebPage`]):
...
[TIP]
====
The sign-in page to render is pluggable; see xref:ug.adoc#_ug_extending_wicket-viewer_custom-pages[extensions chapter] for details.
====
...
As well as providing a sign-in screen, the Wicket viewer also provides the ability for xref:_user_registration[users to self-register].  By and large this operates outside of Apache Isis' security mechanisms; indeed the various pages (sign-up, sign-up verification, password reset) are all rendered _without_ there being any current user session.  These pages all "reach inside" Apache Isis framework using a mechanism similar to xref:ug.adoc#_ug_headless-access[Headless access] in order to actually do their stuff.

[TIP]
====
The sign-in verification page to render is pluggable; see xref:ug.adoc#_ug_extending_wicket-viewer_custom-pages[extensions chapter] for details.
====

User registration is only available if the xref:rg.adoc#_rg_services-spi_manpage-UserRegistrationService[`UserRegistrationService`] is configured; this is used by the framework to actually create new instances of the user as accessed by the corresponding (Shiro) realm.
