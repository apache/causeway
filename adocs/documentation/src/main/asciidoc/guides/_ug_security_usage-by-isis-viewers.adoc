...
By and large the security mechanisms within Isis are transparent to the rest of the framework (the xref:ugvw.adoc#_ugvw[Wicket Viewer] and xref:ug.adoc#_ug_restfulobjects-viewer[Restful Objects viewer], and the overall xref:rg.adoc#_rg_runtime[runtime]).
...
The xref:ugvw.adoc#_ugvw[Wicket viewer] defines a relatively small number of pages (by which we mean subclasses of link:http://ci.apache.org/projects/wicket/apidocs/6.0.x/org/apache/wicket/markup/html/WebPage.html[`org.apache.wicket.markup.html.WebPage`]):
