...
The `xsd` goal of the `isis-maven-plugin` uses the xref:rgsvc.adoc#_rgsvc-api_manpage-JaxbService[`JaxbService`] to
generate XSD schemas from any JAXB-annotated xref:ugbtb.adoc#_ugbtb_more-advanced_view-models[view model/DTOs].
...
* `jaxbClasses` - a list of xref:rg.adoc#_ugbtb_more-advanced_view-models_jaxb[JAXB-annotated view model] classes;
