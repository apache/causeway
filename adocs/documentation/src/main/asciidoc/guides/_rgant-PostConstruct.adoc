...
[TIP]
====
Alternatively, you could inject xref:rgsvc.adoc#_rgsvc-api_manpage-DomainObjectContainer[`DomainObjectContainer`] into the service and read configuration properties using `DomainObjectContainer#getProperty(...)` and related methods.  Note that when using this latter API only those configuration properties prefixes `application.` key are provided.
====

A common use case is for domain services that interact with the xref:rgsvc.adoc#_rgsvc-api_manpage-EventBusService[`EventBusService`].  For example:
