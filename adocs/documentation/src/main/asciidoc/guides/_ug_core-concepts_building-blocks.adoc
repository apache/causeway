...
xref:rgsvc.adoc#_rgsvc-api_manpage-Scratchpad[`Scratchpad`] service, for sharing arbitrary data between multiple objects.
...
xref:rgsvc.adoc#_rgsvc-spi_manpage-AuditingService[`AuditingService`], in fact).
...
xref:rgsvc.adoc#_rgsvc-api_manpage-BookmarkService[`BookmarkService`].  Deep links meanwhile can be obtained from
