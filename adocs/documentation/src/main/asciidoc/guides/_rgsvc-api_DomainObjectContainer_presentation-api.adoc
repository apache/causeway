[[_rgsvc-api_DomainObjectContainer_presentation-api]]
= Presentation API
...
A responsibility of every domain object is to return a title.  This can be done declaratively using the xref:rgant.adoc#_rgant-Title[`@Title`] annotation on property/ies, or it can be done imperatively by writing a xref:rgcms.adoc#_rgcms_methods_reserved_manpage-title[`title()`] method.
...
Similarly, it often makes sense if xref:rgsvc.adoc#_rgsvc-api_DomainObjectContainer_messages-api[raising messages] to use the title of an object in a message rather (than a some other property of the object), because this is how end-users will be used to identifying the object.
