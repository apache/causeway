...
[NOTE]
====
An alternative and almost equivalent design would be to publish an event using the xref:rgsvc.adoc#_rgsvc-api_manpage-EventBusService[`EventBusService`] and implement the domain services as subscribers to
...
----
<1> where `container` is an instance of xref:rgsvc.adoc#_rgsvc-api_manpage-DomainObjectContainer[`DomainObjectContainer`].
