...
----
<1> an alternative implementation would be to implement `findRecentOrders()` as a xref:ug.adoc#_ugfun_how-tos_contributed-members[contributed action].
...
[NOTE]
====
An alternative and almost equivalent design would be to publish an event using the xref:rgsvc.adoc#_rgsvc-api_EventBusService[`EventBusService`] and implement the domain services as subscribers to
...
----
<1> where `container` is an instance of xref:rgsvc.adoc#_rgsvc-api_DomainObjectContainer[`DomainObjectContainer`].
