[[_ug_extending_wicket-viewer_custom-pages]]
= Custom pages
...
In the vast majority of cases customization should be sufficient by xref:ug.adoc#_ug_extending_wicket-viewer_replacing-page-elements[replacing elements of a page]. However, it is also possible to define an entirely new page for a given page type.
...
| SIGN_UP
| The sign-up page (if xref:rg.adoc#_rg_services-spi_manpage-UserRegistrationService[user registration] is enabled).

| SIGN_UP_VERIFY
| The sign-up verification page (if xref:rg.adoc#_rg_services-spi_manpage-UserRegistrationService[user registration] is enabled; as accessed by link from verification email)
