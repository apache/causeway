...
|Not supported by either the xref:ugvw.adoc#_ugvw[Wicket viewer] or the xref:ug.adoc#_ug_restfulobjects-viewer[RestfulObjects viewer]; use prototype mode instead (xref:rg.adoc#_rg_annotations_manpage-Action_restrictTo[`@Action#restrictTo()`])
|UI
|
