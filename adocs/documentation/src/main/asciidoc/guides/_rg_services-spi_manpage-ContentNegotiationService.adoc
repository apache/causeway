...
* first is to solve the problem at the domain layer by defining a regular Isis xref:ug.adoc#_ug_more-advanced_view-models[view model].  This is then surfaced over the RO viewer.  +
+
...
+
The Isis framework provides a default implementation of `ContentNegotiationService` which inspects the "x-ro-domaintype" component of the HTTP `Accept` header.  If present, this implementation will delegate to the companion xref:rg.adoc#_rg_services-spi_manpage-ContentMappingService[`ContentMappingService`] service, if configured.  +
+
...
If the property is not set, then the default depends on the xref:_ug_runtime_deployment-types[deployment type]; production mode will disable pretty printing, while prototyping mode will enable it.
