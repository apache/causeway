[[_rgant_manpage-ActionLayout_cssClassFa]]
= `cssClassFa()`
...
These attributes can also be applied to xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_cssClassFa[domain objects] and to xref:rgant.adoc#_rgant_manpage-ViewModelLayout_cssClassFa[view models] to specify the object's icon.
...
[NOTE]
====
The similar xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClass[`@ActionLayout#cssClass()`] annotation attribute is also used as a hint to apply CSS, but for wrapping the representation of an object or object member so that it can be styled in an application-specific way.
====
