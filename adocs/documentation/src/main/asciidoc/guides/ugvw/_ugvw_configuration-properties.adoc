...
As well as the `welcome.html` file, applications can also define an about file, residing in `about/index.html` (relative to `/src/main/webapp`).

This page is shown as the root; the archetypes provide a version of this file that hyperlink to the wicket and swagger UIs.
