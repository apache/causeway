[[_ug_more-advanced_tips-n-tricks_simulating-collections-of-values]]
...
IMPORTANT: WIP -  easiest to simply store using DataNucleus' support for collections, marked as xref:rg.adoc#_rg_annotations_manpage-Programmatic[`@Programmatic`] so that it is ignored by Apache Isis.  Alternatively can store as json/xml in a varchar(4000) or clob and manually unpack.
