[[_rgant_manpage-Property_mustSatisfy]]
= `mustSatisfy()`
...
The `mustSatisfy()` attribute allows arbitrary validation to be applied to properties using an (implementation of a) `org.apache.isis.applib.spec.Specification` object.
 The attribute is also supported on xref:rgant.adoc#_rgant_manpage-Parameter_mustSatisfy[parameters].

[TIP]
====
The specification implementations can (of course) be reused between properties and xref:rgant.adoc#_rgant_manpage-Parameter_mustSatisfy[parameters].
====
