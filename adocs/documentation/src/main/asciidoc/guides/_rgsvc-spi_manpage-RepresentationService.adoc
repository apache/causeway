[[_rgsvc-spi_manpage-RepresentationService]]
...
The default implementation generates representations according to the link:http://restfulobjects.org[Restful Objects spec] v1.0.  However, it also delegates to the xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentNegotiationService[`ContentNegotiationService`] which provides a mechanism for altering representations according to the HTTP `Accept` header.
...
In all there are three domain services that can influence the representations generated: this service, xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentNegotiationService[`ContentNegotiationService`]and the xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentMappingService[`ContentMappingService`].  The diagram below shows how these collaborate:
...
As discussed in the introduction, the framework provides a default implementation, `o.a.i.v.ro.rendering.service.RepresentationServiceForRestfulObjects`.   This delegates to xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentNegotiationService[`ContentNegotiationService`] to generate an alternative representation; but if none is provided then it falls back on generating the representations as defined in the link:http://restfulobjects.org[Restful Objects spec] v1.0.
...
The default implementation delegates to xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentNegotiationService[`ContentNegotiationService`], whose default implementation may delegate in turn to xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentMappingService[`ContentMappingService`] (if present).