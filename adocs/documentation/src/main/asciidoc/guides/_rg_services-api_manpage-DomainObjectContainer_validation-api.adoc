...
If an object is changed by editing its properties, then (using the xref:ugvw.adoc#_ugvw[Wicket viewer] at least) Apache Isis will check that the state of the object is valid.  There is currently however no equivalent guarantee for actions.
