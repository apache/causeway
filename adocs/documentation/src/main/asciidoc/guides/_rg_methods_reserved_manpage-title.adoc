[[_rg_methods_reserved_manpage-title]]
= `title()`
...
Note that Isis' xref:ug.adoc#_ug_more-advanced_i18n[i18n support] extends this so that titles can also be internationalized.
...
<3> using the xref:rg.adoc#_rg_services-api_manpage-WrapperFactory[`WrapperFactory`] to determine if the `dueBy` field is visible for this user ...
<4> ... but ignore if not

As the example above shows, the implementation can be as complex as you like.

In many cases, though, you may be able to use the xref:rg.adoc#_rg_annotations_manpage-Title[`@Title`] annotation.
