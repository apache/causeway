...
The `CommandContext` service is a xref:rgant.adoc#_rgant_manpage-RequestScoped[request-scoped] service that reifies the invocation of an action on a domain object into an object itself. This reified information is encapsulated within the `Command` object.
...
The typical way to indicate that an action should be treated as a command is to annotate it with the xref:rgant.adoc#_rgant_manpage-Action_command[`@Action#command()`] annotation.
...
See xref:rgant.adoc#_rgant_manpage-Action_command[`@Action#command()`] and xref:rg.adoc#_rg_runtime_configuring-core[runtime configuration] for further details.
====


The xref:rgant.adoc#_rgant_manpage-Action_command[`@Action#command()`] annotation can also be used to specify whether the command should be performed in the background, for example:
