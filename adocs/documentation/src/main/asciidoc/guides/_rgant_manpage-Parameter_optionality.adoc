[[_rgant_manpage-Parameter_optionality]]
= `optionality()`
...
By default, Apache Isis assumes that all parameters of an action are required (mandatory).  The `optionality()` attribute allows this to be relaxed.  The attribute is also supported for xref:rgant.adoc#_rgant_manpage-Property_optionality[properties].
