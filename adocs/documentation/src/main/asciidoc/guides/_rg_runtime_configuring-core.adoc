...
|`true`,`false` +
(`true`)
|Whether to enable TLS for the email SMTP connection (used by xref:rgsvc.adoc#_rgsvc-api_manpage-EmailService[`EmailService`]).  +
...
|The hostname of the external SMTP provider (used by xref:rgsvc.adoc#_rgsvc-api_manpage-EmailService[`EmailService`]).  +
...
`email.port` +
| port number (`_587_`)
|The port number for the SMTP service on the the external SMTP host (used by xref:rgsvc.adoc#_rgsvc-api_manpage-EmailService[`EmailService`]).  +
...
| email address
|The email address to use for sending out email (used by xref:rgsvc.adoc#_rgsvc-api_manpage-EmailService[`EmailService`]).  *Mandatory*.  +
...
| email password
|The corresponding password for the email address to use for sending out email (used by xref:rgsvc.adoc#_rgsvc-api_manpage-EmailService[`EmailService`]).  *Mandatory*.  +
...
| `guava`, `axon`, +
FQCN (`_guava_`)
|which implementation to use by the xref:rgsvc.adoc#_rgsvc-api_manpage-EventBusService[`EventBusService`] as the underlying event bus.

| `isis.services.` +
`eventbus.allowLateRegistration` +
|`true`,`false` +
(`false`)
|whether a domain service can register with the xref:rgsvc.adoc#_rgsvc-api_manpage-EventBusService[`EventBusService`] after any events have posted. +
...
Filtering is supported by the Wicket viewer, and by the xref:rgsvc.adoc#_rgsvc-api_manpage-WrapperFactory[`WrapperFactory`] domain service (provided the wrapper's execution mode is _not_ "skip rules").  However the Restful Objects viewer does _not_ currently support filtering.
