...
The xref:rgant.adoc#_rgant_manpage-Title[`@Title`] annotation can be used build up the title of an object from its constituent parts.
...
NOTE: TODO - xref:rg.adoc#_rg_methods_reserved_manpage-iconName[`iconName()`], xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_cssClassFa[`@DomainObjectLayout#cssClassFa()`]
...
It is also possible for an object to return a xref:rg.adoc#_rg_methods_reserved_manpage-cssClass[CSS class].  In conjunction with xref:ugvw.adoc#_ugvw_customisation_tweaking-css-classes[customized CSS] this can be used to apply arbitrary styling; for example each object could be rendered in a page with a different background colour.

NOTE: TODO - xref:rg.adoc#_rg_methods_reserved_manpage-cssClass[`cssClass()`], xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_cssClass[`@DomainObjectLayout#cssClass()`] xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClass[`@ActionLayout#cssClass()`], xref:rgant.adoc#_rgant_manpage-PropertyLayout_cssClass[`@PropertyLayout#cssClass()`], xref:rgant.adoc#_rgant_manpage-CollectionLayout_cssClass[`@CollectionLayout#cssClass()`]
