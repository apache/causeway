 ...
 * component classes, that capture the layout metadata for an object's properties, collections and actions.  These are broadly equivalent to the "layout" annotations (xref:rgant.adoc#_rgant_PropertyLayout[`@PropertyLayout`], xref:rgant.adoc#_rgant_CollectionLayout[`@CollectionLayout`], xref:rgant.adoc#_rgant_ActionLayout[`@ActionLayout`] and xref:rgant.adoc#_rgant_DomainObjectLayout[`@DomainObjectLayout`]

 * grid classes, that define a grid structure of rows, columns, tab groups and tabs, into which the components can be associated.
...
also possible to extend the layout architecture for other grid systems.  The components are intended to be reusable across all grid systems.
