[[_rg_services-api_manpage-EventBusService]]
= `EventBusService`
...
The primary user of the service is the framework itself, which automatically emit events for xref:rgant.adoc#_rgant_manpage-Action_domainEvent[actions], xref:rgant.adoc#_rgant_manpage-Property_domainEvent[properties] and xref:rgant.adoc#_rgant_manpage-Collection_domainEvent[collections].  Multiple events are generated:
...
<2> allows domain services to register themselves.  This should be done in their xref:rgant.adoc#_rgant_manpage-PostConstruct[`@PostConstruct`] initialization method (for both singleton and xref:rgant.adoc#_rgant_manpage-RequestScoped[`@RequestScoped`] domain services.
...
The `register()` method should be called in the xref:rgant.adoc#_rgant_manpage-PostConstruct[`@PostConstruct`] lifecycle method.  It is valid and probably the least confusing to readers to also "unregister" in the xref:rgant.adoc#_rgant_manpage-PreDestroy[`@PreDestroy`] lifecycle method (though as noted xref:rg.adoc#_rg_services-api_manpage-EventBusService_api-and-implementation[above], unregistering is actually a no-op).
...
services.  The easiest way to do this is to use the xref:rgant.adoc#_rgant_manpage-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] attribute.
<3> register with the event bus service during xref:rgant.adoc#_rgant_manpage-PostConstruct[`@PostConstruct`]
    initialization
<4> corresponding deregister when shutting down

This works for both singleton (application-scoped) and also xref:rgant.adoc#_rgant_manpage-RequestScoped[`@RequestScoped`] domain services.
...
In practice we suspect there will be few cases where the programmatic approach is required rather than the declarative approach afforded by xref:rgant.adoc#_rgant_manpage-Action_domainEvent[`@Action#domainEvent()`] et al.
