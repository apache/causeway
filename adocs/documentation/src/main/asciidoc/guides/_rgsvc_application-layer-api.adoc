...
|(`1.12.0-SNAPSHOT`) Methods for batching long-running work (eg data migration) into multiple sessions.
|`SessionManagementService-` +
``Default`` +
``o.a.i.core`` +
``isis-core-runtime``
...
`services.title` +
`TitleService`]
|(`1.12.0-SNAPSHOT`) Methods to programmatically obtain the title or icon of a domain object.
|`TitleService-` +
``Default`` +
``o.a.i.core`` +
``isis-core-metamodel``
