...
The `auditing()` attribute indicates that if the object is modified, then each of its changed properties should be submitted to the xref:rgsvc.adoc#_rgsvc-spi_manpage-AuditingService[`AuditingService`], if one has been configured.
