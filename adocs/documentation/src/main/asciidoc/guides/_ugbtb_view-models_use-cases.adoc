...
using the xref:rgsvc.adoc#_rgsvc_api_JaxbService[`JaxbService`].  Using the formalism of XML also allows
...
In case it's not clear, these DTOs are still usable as "regular" view models; they will render in the xref:ugvw.adoc#[Wicket viewer] just like any other.  Indeed (as the xref:ugbtb.adoc#_ugbtb_view-models_programming-model[programming model] section below makes clear), these
...
A xref:rgsvc.adoc#_rgsvc_spi_PublishingService[`PublishingService`] implementation (eg that provided by
