...
|
|(Added in `1.12.0-SNAPSHOT`) Replaces (and simplifies) the earlier `ContentMappingService` that defined an SPI using classes internal to the framework. +
+
...
`services.grid` +
`GridSystemService`]
|(`1.12.0-SNAPSHOT`) Validates and normalizes the grid layout for a domain class (with respect to a particular grid
...
`services.grid` +
`GridLoaderService`]
|(`1.12.0-SNAPSHOT`) Responsible for loading a grid layout for a domain class, eg from a `layout.xml` file.
...
`services.grid` +
`GridService`]
|(`1.12.0-SNAPSHOT`) A facade on top of both xref:rgsvc.adoc#_rgsvc_spi_GridLoaderService[`GridLoaderService`] and
...
`services.hint` +
`HintStore`]
|(`1.12.0-SNAPSHOT`) Stores UI hints on a per-object basis. For example, the viewer remembers which tabs are selected, and for collections which view is selected (eg table or hidden), which page of a table to render, or whether "show all" (rows) is toggled.
