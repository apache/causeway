[[_rgsvc_api_DomainObjectContainer_object-persistence-api]]
...
[NOTE]
====
(As of `1.12.0-SNAPSHOT`), the majority of these methods have been moved to the xref:rgsvc.adoc#_rgsvc_api_RepositoryService[`RepositoryService`] or xref:rgsvc.adoc#_rgsvc_api_TransactionService[`TransactionService`]; the corresponding methods in `DomainObjectContainer` are therefore deprecated.
====
