[[_rg_methods_prefixes_manpage-default]]
= `default...()`
...
Defaults are also supported (of course) for xref:ug.adoc#_ug_how-tos_contributed-members[contributed actions].  For example, here is a contributed action for updating category/subcategory of the (non-ASF) http://github.com/isisaddons/isis-app-todoapp[Isis addons' todoapp]:
...
The first is to use the xref:rg.adoc#_rg_methods_lifecycle_manpage-created[`created()`] callback, called by the framework when xref:rg.adoc#_rg_services-api_manpage-DomainObjectContainer[`DomainObjectContainer#newTransientInstance(...)`] is called.  This method is called after any dependencies have been injected into the service.
