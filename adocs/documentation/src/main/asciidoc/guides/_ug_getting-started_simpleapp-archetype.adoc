...
The `simpleapp` archetype generates a single WAR file, configured to run both the xref:ugvw.adoc#_ugvw[Wicket viewer] and the xref:ug.adoc#_ug_restfulobjects-viewer[Restful Objects viewer].  The archetype also configures the DataNucleus/JDO Objectstore to use an in-memory HSQLDB connection.
...
When you start the app, you'll be presented with a welcome page from which you can access the webapp using either the xref:ugvw.adoc#_ugvw[Wicket viewer] or the xref:ug.adoc#_ug_restfulobjects-viewer[Restful Objects viewer]:
...
|`myapp-webapp`
|Run as a webapp (from `web.xml`) hosting the xref:ugvw.adoc#_ugvw[Wicket viewer] and/or the xref:ug.adoc#_ug_restfulobjects-viewer[RestfulObjects viewer]
