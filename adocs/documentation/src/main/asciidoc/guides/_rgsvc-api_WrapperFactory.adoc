...
* the xref:rgcms.adoc#_rgcms_methods_reserved_manpage-title[`title()`] and `toString()` methods
* any xref:rgcms.adoc#_rgcms_methods_prefixes_manpage-default[`default...()`], xref:rgcms.adoc#_rgcms_methods_prefixes_manpage-choices[`choices...()`] or xref:rgcms.adoc#_rgcms_methods_prefixes_manpage-autoComplete[`autoComplete...()`] methods
...
[NOTE]
====
It ought to be possible to implement an xref:rgsvc.adoc#_rgsvc-spi_ExceptionRecognizer[`ExceptionRecognizer`]s that would allow the above boilerplate to be removed.  This recognizer service would recognize the `InteractionException` and convert to a suitable message.
