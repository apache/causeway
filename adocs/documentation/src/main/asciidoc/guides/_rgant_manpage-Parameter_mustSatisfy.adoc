[[_rgant_manpage-Parameter_mustSatisfy]]
= `mustSatisfy()`
...
The `mustSatisfy()` attribute allows arbitrary validation to be applied to parameters using an (implementation of a) `org.apache.isis.applib.spec.Specification` object.  The attribute is also supported on xref:rgant.adoc#_rgant_manpage-Property_mustSatisfy[properties].

[TIP]
====
The specification implementations can (of course) be reused between parameters and xref:rgant.adoc#_rgant_manpage-Property_mustSatisfy[properties].
====
