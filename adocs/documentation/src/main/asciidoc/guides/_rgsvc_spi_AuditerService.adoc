...
The auditing service works very well with the xref:rgsvc.adoc#_rgsvc_spi_CommandService[`CommandService`].  The `CommandService` captures the _cause_ of an interaction (an action was invoked, a property was edited), while the `AuditingService3` captures the _effect_ of that interaction in terms of changed state.
