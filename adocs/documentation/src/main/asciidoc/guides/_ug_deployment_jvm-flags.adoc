...
If using Tomcat, update the `CATALINA_OPTS` variable.  (This variable is also updated if xref:_ug_deployment_externalized-configuration_Log4j[configuring logging to run externally]).
