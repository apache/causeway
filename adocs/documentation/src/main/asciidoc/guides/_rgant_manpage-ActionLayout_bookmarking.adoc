[[_rgant_manpage-ActionLayout_bookmarking]]
= `bookmarking()`
...
The `bookmarking()` attribute indicates if an action (with safe xref:rgant.adoc#_rgant_manpage-Action_semantics[action semantics]) is automatically bookmarked. This attribute is also supported for  xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_bookmarking[domain objects].
...
[NOTE]
====
The enum value `AS_CHILD` has no meaning for actions; it relates only to bookmarked xref:rgant.adoc#_rgant_manpage-DomainObject_bookmarking[domain object]s.
====
