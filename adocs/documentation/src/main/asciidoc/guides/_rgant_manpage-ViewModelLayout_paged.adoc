[[_rgant_manpage-ViewModelLayout_paged]]
= `paged()`
...
The `paged()` attribute specifies the number of rows to display in a standalone collection, as returned from an action invocation.  This attribute can also be applied to xref:rgant.adoc#_rgant_manpage-CollectionLayout_paged[collections] and xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_paged[domain objects].
