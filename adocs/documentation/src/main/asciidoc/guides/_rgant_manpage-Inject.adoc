...
[NOTE]
====
One omission: Apache Isis (currently) does not inject services into `o.a.i.applib.spec.Specification` instances (as used by xref:rgant.adoc#_rgant_manpage-Property_mustSatisfy[`@Property#mustSatisfy()`] and xref:rgant.adoc#_rgant_manpage-Parameter_mustSatisfy[`@Parameter#mustSatisfy()`] annotations.
====
