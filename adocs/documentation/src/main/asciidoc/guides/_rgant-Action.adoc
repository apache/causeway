...
`services.command.Command` object through the xref:rgsvc.adoc#_rgsvc-api_manpage-CommandContext[`CommandContext`] service.


|`commandExecuteIn()`
|`FOREGROUND`,`BACKGROUND` +
(`FOREGROUND`)
|whether to execute the command immediately, or to persist it (assuming that an appropriate implementation of xref:rgsvc.adoc#_rgsvc-spi_manpage-CommandService[`CommandService`] has been configured) such that a background scheduler can execute the command asynchronously


|`commandPersistence()`
|`PERSISTED`, `NOT_PERSISTED`, `IF_HINTED` +
(`PERSISTED`)
|whether the reified `Command` (as provided by the `CommandContext` domain service) should actually be persisted (assuming an appropriate implementation of xref:rgsvc.adoc#_rgsvc-spi_manpage-CommandService[`CommandService`] has been configured).


|xref:rgant.adoc#_rgant-Action_domainEvent[`domainEvent()`]
|subtype of `ActionDomainEvent` +
(`ActionDomainEvent.Default`)
|the event type to be posted to the xref:rgsvc.adoc#_rgsvc-api_manpage-EventBusService[`EventBusService`] to
...
|`AS_CONFIGURED`, `ENABLED`, `DISABLED` +
(`AS_CONFIGURED`)
|whether the action invocation should be published to the registered xref:rgsvc.adoc#_rgsvc-spi_manpage-PublishingService[`PublishingService`].
