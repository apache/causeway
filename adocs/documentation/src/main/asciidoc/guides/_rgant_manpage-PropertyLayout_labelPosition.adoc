[[_rgant_manpage-PropertyLayout_labelPosition]]
= `labelPosition()`
...
The `labelPosition()` attribute determines the positioning of labels for properties.  This attribute can also be specified for xref:rgant.adoc#_rgant_manpage-ParameterLayout_labelPosition[parameters].

The positioning of labels is typically `LEFT`, but can be positioned to the `TOP`.  The one exception is xref:rgant.adoc#_rgant_manpage-ParameterLayout_multiLine[`multiLine()`] string properties, where the label defaults to `TOP` automatically (to provide as much real-estate for the multiline text field as possible).
