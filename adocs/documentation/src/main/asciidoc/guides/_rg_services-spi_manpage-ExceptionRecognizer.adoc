...
If the JDO exception recognizers are not required (rather unlikely), then they can be disabled en-masse using the xref:_ug_runtime_configuring-core[configuration property] `isis.services.ExceptionRecognizerCompositeForJdoObjectStore.disable`.
