[[_rg_annotations_manpage-DomainObjectLayout_cssClassFa]]
= `cssClassFa()`
...
These attributes can also be applied to xref:rg.adoc#_rg_annotations_manpage-ViewModelLayout_cssClassFa[view models] to specify the object's icon, and to xref:rg.adoc#_rg_annotations_manpage-ActionLayout_cssClassFa[actions] to specify an icon for the action's representation as a button or menu item.
...
[TIP]
====
The similar xref:rg.adoc#_rg_annotations_manpage-DomainObjectLayout_cssClass[`@DomainObjectLayout#cssClass()`] annotation attribute is also used as a hint
