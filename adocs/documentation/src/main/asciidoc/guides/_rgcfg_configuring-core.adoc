...
|`true`,`false` +
(`true`)
| (`1.14.0-SNAPSHOT`) When searching for  xref:rgcms.adoc#_rgcms_methods_prefixes_disable[`disableXxx()`] or xref:rgcms.adoc#_rgcms_methods_prefixes_hide[`hideXxx()`] methods, whether to search only for the no-param version. +

The previous behaviour also searched for a methods that had the exact set of arguments as the method it was supporting.
However, the parameter arguments when called are always set to null, so serve no real purpose.
