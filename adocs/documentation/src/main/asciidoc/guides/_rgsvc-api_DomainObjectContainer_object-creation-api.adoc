[[_rgsvc-api_DomainObjectContainer_object-creation-api]]
= Object Creation API
...
<2> create a new view model, with the specified memento (as per xref:rgcms.adoc#_rgcms_classes_super_manpage-AbstractViewModel[ViewModel#viewModelMemento()].  In general it is easier to just annotate with xref:rgant.adoc#_rgant-ViewModel[`@ViewModel`] and let Apache Isis manage the memento automatically.
...
Doing this will not inject any domain services, but they can be injected manually using xref:rg .adoc#_rgsvc-api_DomainObjectContainer_services-api[#injectServicesInto(...)`].

[NOTE]
====
Calling `new(...)` also this circumvents Apache Isis' xref:rgcms.adoc#_rgcms_methods_reserved_manpage-created[`created()`]
callback, and in addition any default values for properties (either explicitly set by
xref:rgcms.adoc#_rgcms_methods_prefixes_manpage-default[`default...()`] or defaulted implicitly according to Apache Isis'
