...
A `AppManifest.Util` helper class provides a number of static methods that can be used to set up configuration properties appropriate for integration testing (eg run using an in-memory database).
This allows the responsibility of returning the configuration properties to belong exlusively to the `AppManifest`.
...
Previously the `IsisConfigurationJdoIntegTests` (subclass of `IsisConfiguration`) was provided to set up these configuration properties.
