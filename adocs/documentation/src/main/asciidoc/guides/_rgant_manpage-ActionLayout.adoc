...
|xref:rgant.adoc#_rgant_manpage-ActionLayout_bookmarking[`bookmarking()`]
|`AS_ROOT`, `NEVER` +
(`NEVER`)
|indicates if an action (with safe xref:rgant.adoc#_rgant_manpage-Action_semantics[action semantics]) is automatically bookmarked.


|xref:rgant.adoc#_rgant_manpage-ActionLayout_contributedAs[`contributedAs()`]
...
For a domain service action to be contributed, the domain services must have a xref:rgant.adoc#_rgant_manpage-DomainService_nature[nature] nature of either `VIEW` or `VIEW_CONTRIBUTIONS_ONLY`, and the action must have xref:rgant.adoc#_rgant_manpage-Action_semantics[safe action semantics], and takes a single argument, namely the contributee domain object.


|xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClass[`cssClass()`]
...
|xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClassFa[`cssClassFa()`]
...
|xref:rgant.adoc#_rgant_manpage-ActionLayout_describedAs[`describedAs()`]
|String.
|provides a short description of the action, eg for rendering as a 'tool tip'.


|xref:rgant.adoc#_rgant_manpage-ActionLayout_hidden[`hidden()`]
|`EVERYWHERE`, `NOWHERE` +
(`NOWHERE`)
|indicates where (in the UI) the action should be hidden from the user.


|xref:rgant.adoc#_rgant_manpage-ActionLayout_named[`named()`]
|String.
|to override the name inferred from the action's name in code. +

A typical use case is if the desired name is a reserved Java keyword, such as `default` or `package`.

|xref:rgant.adoc#_rgant_manpage-ActionLayout_position[`position()`]
|`BELOW`, `RIGHT`, `PANEL`, `PANEL_DROPDOWN` (`BELOW`)
|for actions associated (using xref:rgant.adoc#_rgant_manpage-MemberOrder[`@MemberOrder#named()`]) with properties, the positioning of the action's button with respect to the property
...
include::_rgant_manpage-ActionLayout_bookmarking.adoc[leveloffset=+1]
include::_rgant_manpage-ActionLayout_contributedAs.adoc[leveloffset=+1]
include::_rgant_manpage-ActionLayout_cssClass.adoc[leveloffset=+1]
include::_rgant_manpage-ActionLayout_cssClassFa.adoc[leveloffset=+1]
include::_rgant_manpage-ActionLayout_describedAs.adoc[leveloffset=+1]
include::_rgant_manpage-ActionLayout_hidden.adoc[leveloffset=+1]
include::_rgant_manpage-ActionLayout_named.adoc[leveloffset=+1]
include::_rgant_manpage-ActionLayout_position.adoc[leveloffset=+1]
