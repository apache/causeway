...
In this chapter we also discuss the xref:ug.adoc#_ug_getting-started_datanucleus-enhancer[DataNucleus enhancer].  link:http://www.datanucleus.org/[DataNucleus] is the reference implementation of the JDO (Java data objects) spec, and Isis integrates with DataNucleus as its persistence layer.  The enhancer performs post-processing on the bytecode of your persistent domain entities, such that they can be persisted by Isis' JDO/DataNucleus objectstore.
...
You'll probably also want to use an IDE; the Isis committers use either IntelliJ or Eclipse; in the xref:ug.adoc#_ug_appendices[appendices] we have detailed setup instructions for using these two IDEs.  If you're a NetBeans user you should have no problems as it too has strong support for Maven.
