...
If an even simpler representations (of objects) are required, see xref:ug.adoc#_ug_restfulobjects-viewer_configuration-properties_simplified-object-representation[simplified object representation], immediately below.  And if the above isn't flexible and you need complete control over all representations see the section on xref:ug.adoc#_ug_extending_restfulobjects-viewer[Extending the Restful Objects viewer] in the xref:ug.adoc#_ug_extending[Extending] chapter.
