[[_ug_testing_integ-test-support_wrapper-factory]]
= Wrapper Factory
...
The xref:rg.adoc#_rg_services-api_manpage-WrapperFactory[`WrapperFactory`] service is responsible for wrapping a domain object in a dynamic proxy, of the same type as the object being proxied.  And the role of this wrapper is to simulate the UI.
...
[NOTE]
====
In this respect the wrapper is more functional than the xref:ug.adoc#_ug_wicket-viewer[Wicket viewer] (which does not expose the ability to mutate collections directly).
====
...
----
<1> inject xref:rg.adoc#_rg_services-api_manpage-EventBusService[`EventBusService`] into this test
