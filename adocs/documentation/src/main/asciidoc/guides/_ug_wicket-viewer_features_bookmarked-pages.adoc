...
The Wicket viewer supports the bookmarking of both domain objects and query-only (xref:rg.adoc#_rg_annotations_manpage-Action_semantics[@Action(semantics=...)]) actions.
...
To indicate a class is bookmarkable, use the xref:rg.adoc#_rg_annotations_manpage-DomainObjectLayout[@DomainObjectLayout] annotation:
...
To indicate that a safe (query only) action is bookmarkable, use the xref:rg.adoc#_rg_annotations_manpage-ActionLayout[@ActionLayout] annotation:
...
The xref:ug.adoc#_ug_wicket-viewer_features_recent-pages[Recent Pages] also lists recently visited pages, selected from a drop-down.
