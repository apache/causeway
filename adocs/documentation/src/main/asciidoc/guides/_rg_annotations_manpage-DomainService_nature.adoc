...
<2> methods on `DOMAIN` services are often xref:rg.adoc#_rg_annotations_manpage-Programmatic[`@Programmatic`]; they will never be exposed in the UI, so there's little point in including them in Isis' metamodel
