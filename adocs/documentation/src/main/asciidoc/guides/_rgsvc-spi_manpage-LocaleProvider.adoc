[[_rgsvc-spi_manpage-LocaleProvider]]
= `LocaleProvider`
...
The `LocaleProvider` service is one of the services that work together to implement Apache Isis' support for i18n, being used by Isis' default implementation of xref:rgsvc.adoc#_rgsvc-spi_manpage-TranslationService[`TranslationService`].
...
This service works in conjunction with xref:rgsvc.adoc#_rgsvc-spi_manpage-TranslationService[`TranslationService`] and xref:rgsvc.adoc#_rgsvc-spi_manpage-TranslationsResolver[`TranslationsResolver`] in order to provide i18n support.
