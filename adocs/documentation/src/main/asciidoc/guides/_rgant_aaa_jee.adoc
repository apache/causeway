...
|Purpose
|Layer
|xref:ug.adoc#_ugfun_object-layout_dynamic[Dynamic layout]?
...
`.annotation` +
`XmlJavaTypeAdapter`]
|JAXB annotation defining how to serialize an entity.  Used in conjunction with the (framework provided) `PersistentEntityAdapter` class to serialize persistent entities into a canonical OID (equivalent to the `Bookmark` provided by the xref:rgsvc.adoc#_rgsvc-api_BookmarkService[`BookmarkService`]).
|Domain
|
