...
This will return the XML text wrapped up in a xref:rgcms.adoc#_rgcms_classes_value-types_manpage-Clob[`Clob`].
...
a xref:rgcms.adoc#_rgcms_classes_value-types_manpage-Clob[`Clob`] of that XSD.  If there are multiple XSD schemas referenced
then the action will return a zip of those schemas, wrapped up in a
xref:rgcms.adoc#_rgcms_classes_value-types_manpage-Blob[`Blob`].  The `IsisSchemas` parameter to the action can be used to
