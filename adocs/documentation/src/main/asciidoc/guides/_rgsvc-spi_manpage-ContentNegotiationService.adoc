[[_rgsvc-spi_manpage-ContentNegotiationService]]
...
+
The Apache Isis framework provides a default implementation of `ContentNegotiationService` which inspects the "x-ro-domaintype" component of the HTTP `Accept` header.  If present, this implementation will delegate to the companion xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentMappingService[`ContentMappingService`] service, if configured.  +
+
...
The value of the `x-ro-domain-type` parameter corresponds to the DTO to be mapped into by the xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentMappingService[`ContentMappingService`].
...
[_rgsvc-spi_manpage-ContentNegotiationService_Configuration]]
== Configuration
...
The default implementation of `ContentNegotiationService` delegates to xref:rgsvc.adoc#_rgsvc-spi_manpage-ContentMappingService[`ContentMappingService`] (if present) to convert domain entities into a stable form (eg DTO).

The `ContentNegotiationService` is itself called by the (default implementation of) xref:rgsvc.adoc#_rgsvc-spi_manpage-RepresentationService[`RepresentationService`].