[[_rg_methods_prefixes_manpage-autoComplete]]
= `autoComplete...()`
...
<2> the xref:rgant.adoc#_rgant_manpage-MinLength[`@MinLength`] annotation specifies the minimum number of characters that must be entered before a search is performed for matching objects
...
----
<1> the xref:rgant.adoc#_rgant_manpage-MinLength[`@MinLength`] annotation specifies the minimum number of characters that must be entered before a search is performed for matching objects
