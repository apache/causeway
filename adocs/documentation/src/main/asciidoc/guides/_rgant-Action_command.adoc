...
* xref:rgsvc.adoc#_rgsvc-api_manpage-CommandContext[`CommandContext`]
* xref:rgsvc.adoc#_rgsvc-spi_manpage-CommandService[`CommandService`]
* xref:rgsvc.adoc#_rgsvc-spi_manpage-BackgroundService[`BackgroundService`] and
* xref:rgsvc.adoc#_rgsvc-spi_manpage-BackgroundCommandService[`BackgroundCommandService`]


Each action invocation is reified by the xref:rgsvc.adoc#_rgsvc-api_manpage-CommandContext[`CommandContext`] service into a `Command` object, capturing details of the target object, the action, the parameter arguments, the user, a timestamp and so on.
...
The `command()` attribute determines whether the action invocation should be reified into a `Command` object (by the xref:rgsvc.adoc#_rgsvc-api_manpage-CommandContext[`CommandContext`] service).
...
[NOTE]
====
Note: `Command` reification does not necessarily imply that `Command` objects will be persisted; that depends on whether there is a xref:rgsvc.adoc#_rgsvc-spi_manpage-CommandService[`CommandService`] configured that will persist said ``Command``s.
====
...
will suppress the persistence of the `Command` object _unless_ a child background `Command` has been created in the body of the action by way of the xref:rgsvc.adoc#_rgsvc-api_manpage-BackgroundService[`BackgroundService`].
