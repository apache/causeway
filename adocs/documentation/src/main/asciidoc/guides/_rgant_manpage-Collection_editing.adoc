[[_rgant_manpage-Collection_editing]]
= `editing()`
...
The `editing()` annotation indicates whether a collection can be added to or removed from within the UI.  This attribute can also be specified for xref:rgant.adoc#_rgant_manpage-Property_editing[properties], and can also be specified for the xref:rgant.adoc#_rgant_manpage-DomainObject_editing[domain object]
...
The workaround is to create add/remove actions and use xref:rgant.adoc#_rgant_manpage-MemberOrder[UI hints] to render them close to the collection.
====

Whether a collection is enabled or disabled depends upon these factors:

* whether the domain object has been configured as immutable through the xref:rgant.adoc#_rgant_manpage-DomainObject_editing[`@DomainObject#editing()`] attribute
