...
The `ContentMappingService` supports the (default implementation of the) xref:rgsvc.adoc#_rgsvc_spi_ContentNegotiationService[`ContentNegotiationService`] allowing the xref:ugvro.adoc#[RestfulObjects viewer] to allow domain objects to be transformed into some other format as specified by the HTTP `Accept` header.
...
<9> and map its oid (by way of the xref:rgsvc.adoc#_rgsvc_api_BookmarkService[`BookmarkService`]
