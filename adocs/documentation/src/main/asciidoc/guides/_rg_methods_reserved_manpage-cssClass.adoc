[[_rg_methods_reserved_manpage-cssClass]]
= `cssClass()`
...
The xref:ugvw.adoc#_ugvw[Wicket viewer] wraps the object's representation in a containing `<div>` with the class added.  This is done both for rendering the object either in a table or when rendering the object on its own page.
