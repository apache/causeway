...
* `xsd` +
+
Uses the xref:rgsvc.adoc#_rgsvc-api_JaxbService[`JaxbService`] to generate XSD schemas from any JAXB-annotated view models/DTOs. +
+
...
Both goals require an xref:rgcms.adoc#_rgcms_classes_AppManifest-bootstrapping[`AppManifest`] to point the plugin at, so that
it knows how to bootstrap an Isis runtime.  This is discussed below, followed by sections on configuring the two goals.


== `AppManifest`

As noted in the introduction, both goals require an xref:rgcms.adoc#_rgcms_classes_AppManifest-bootstrapping[`AppManifest`]
to point the plugin at, so that it knows how to bootstrap an Isis runtime.

This can be extremely minimal.  For example, the xref:ug.adoc#_ugfun_getting-started_simpleapp-archetype[SimpleApp]'s
manifest is:
