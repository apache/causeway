    ...
    <build>
        <plugins>
            <plugin>
                ...
                <dependencies>
                    <dependency>
                        <groupId>org.apache.isis.example.application</groupId>          <!-- 4 -->
...
<4> the `dom` module containing itself; adjust as required
<5> binds the plugin's `validate` goal to the Maven `test` lifecycle phase (ie will be called using `mvn test`).
