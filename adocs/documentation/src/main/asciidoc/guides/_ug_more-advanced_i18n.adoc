...
Normally Isis configuration files are read from the `WEB-INF` file. However, Isis can be configured to read config files from an xref:ug.adoc#_ug_deployment_externalized-configuration[external directory]; this is also supported for translations.
