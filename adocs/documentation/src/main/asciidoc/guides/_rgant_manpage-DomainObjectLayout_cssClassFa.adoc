[[_rgant_manpage-DomainObjectLayout_cssClassFa]]
= `cssClassFa()`
...
These attributes can also be applied to xref:rgant.adoc#_rgant_manpage-ViewModelLayout_cssClassFa[view models] to specify the object's icon, and to xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClassFa[actions] to specify an icon for the action's representation as a button or menu item.
...
[TIP]
====
The similar xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_cssClass[`@DomainObjectLayout#cssClass()`] annotation attribute is also used as a hint
