...
For more details on the implementation, see xref:ugbtb.adoc#_ugbtb_more-advanced_i18n[i18n support].
...
If the menu items are not required then these can be suppressed either using security or by implementing a xref:ugbtb.adoc#_ugbtb_more-advanced_decoupling_vetoing-visibility[vetoing subscriber].
...
This service works in conjunction with xref:rgsvc.adoc#_rgsvc-spi_LocaleProvider[`LocaleProvider`] and xref:rgsvc.adoc#_rgsvc-spi_TranslationsResolver[`TranslationsResolver`] in order to provide i18n support.
