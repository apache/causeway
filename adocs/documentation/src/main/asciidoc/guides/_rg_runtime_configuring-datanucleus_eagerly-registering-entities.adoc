...
* as of 1.9.0 the recommended (and simpler) approach is to specify an xref:rg.adoc#_rg_classes_super_manpage-AppManifest[`AppManifest`], either as a xref:rg.adoc#_rg_runtime_configuring-components[`isis.appManifest`] configuration property or programmatically.
...
Further discussion on specifying the package(s) in integration testing (for either approach) can be found in the xref:ug.adoc#_ug_testing_integ-test-support_bootstrapping[user guide].
