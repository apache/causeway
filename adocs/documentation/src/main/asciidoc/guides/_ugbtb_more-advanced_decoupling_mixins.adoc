[[_ugbtb_more-advanced_decoupling_mixins]]
= Mixins
...
xref:rgsvc.adoc#_rgsvc-api_DomainObjectContainer_object-creation-api[`DomainObjectContainer#mixin(...)`
method.  For example:
...
xref:rgcms.adoc#_rgcms_classes_super_manpage-FixtureScript[`FixtureScript`] classes both provide a `mixin(...)` convenience
method.
