...
It is also recognized for xref:ug.adoc#http://localhost:4000/guides/ug.html#_ugbtb_more-advanced_view-models[view models] (eg annotated with xref:rgant.adoc#_rgant_manpage-ViewModel[`@ViewModel`]).

For the default application-scoped (singleton) domain services, this means that the method, if present, is called during the bootstrapping of the application.  For xref:rgant.adoc#_rgant_manpage-RequestScoped[`@RequestScoped`] domain services, the method is called at the beginning of the request.
...
See also xref:rgant.adoc#_rgant_manpage-PreDestroy[`@PreDestroy`]