...
|the event type to be posted to the xref:rgsvc.adoc#_rgsvc-api_manpage-EventBusService[`EventBusService`] to broadcast the property's business rule checking (hide, disable, validate) and its modification (before and after).
