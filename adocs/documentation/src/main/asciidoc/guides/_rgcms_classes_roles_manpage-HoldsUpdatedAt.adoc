[[_rgcms_classes_roles_manpage-HoldsUpdatedAt]]
= `HoldsUpdatedAt`
...
The current time is obtained from the xref:rgsvc.adoc#_rgsvc-api_ClockService[`ClockService`].

Entities that implement this interface often also implement xref:rgcms.adoc#_rgcms_classes_roles_manpage-HoldsUpdatedBy[`HoldsUpdatedBy`] role interface; as a convenience the xref:rgcms.adoc#_rgcms_classes_roles_manpage-Timestampable[`Timestampable`] interface combines the two roles.
