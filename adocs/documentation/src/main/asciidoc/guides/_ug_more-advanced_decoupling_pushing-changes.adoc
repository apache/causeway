[[_ug_more-advanced_decoupling_pushing-changes]]
= Pushing Changes
...
[NOTE]
====
This technique is much less powerful than using xref:ug.adoc#_ug_more-advanced_decoupling_event-bus[the event bus] .  We present it mostly for completeness.
====
...
<3> the xref:rg.adoc#_rg_methods_prefixes_manpage-addTo[`addTo...()`] method increments the derived properties
<4> the xref:rg.adoc#_rg_methods_prefixes_manpage-removeFrom[`removeFrom...()`] method similarly decrements the derived properties
