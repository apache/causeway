[[_rgant_manpage-DomainServiceLayout_menuBar]]
= `menuBar()`
...
The grouping of multiple domain services actions within a single drop-down is managed by the xref:rgant.adoc#_rgant_manpage-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] attribute.
