...
|xref:rgant.adoc#_rgant_manpage-Action_domainEvent[`@Action#domainEvent()`]
|Domain
|
...
|xref:rgant.adoc#_rgant_manpage-DomainObject_auditing[`@DomainObject#` +
`auditing()`]
|Domain
|
...
|xref:rgant.adoc#_rgant_manpage-DomainObject_autoCompleteRepository[`@DomainObject` +
...
|xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_bookmarking[`@DomainObjectLayout` +
`#bookmarking()`]
|UI
|
...
|xref:rgant.adoc#_rgant_manpage-DomainObject_bounded[`@DomainObject#bounded()`]
|Domain
|

|`@Bulk`
|Indicates an action is a bulk action, can be applied to multiple instances.
|xref:rgant.adoc#_rgant_manpage-Action_invokeOn[`@Action#invokeOn()`]
|UI, Domain
|
...
|xref:rgant.adoc#_rgant_manpage-Collection_domainEvent[`@Collection#domainEvent()`]
|Domain
|
...
|xref:rgant.adoc#_rgant_manpage-Action_command[`@Action#command()`]
|Domain
|

|`@CssClass`
|Allow visual representation of individual objects or object members layout to be customized by application-specific CSS.
|`#cssClass()` attribute for: xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_cssClass[`@DomainObjectLayout`], xref:rgant.adoc#_rgant_manpage-PropertyLayout_cssClass[`@PropertyLayout`],  xref:rgant.adoc#_rgant_manpage-CollectionLayout_cssClass[`@CollectionLayout`], xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClass[`@ActionLayout`] and xref:rgant.adoc#_rgant_manpage-ParameterLayout_cssClass[`@ParameterLayout`]
|UI
|Yes

|`@CssClassFa`
|So that font awesome icons can be applied to action buttons/menu items and optionally as an object icon.
|`cssClassFa()` attribute for: xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClassFa[`@ActionLayout`], xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_cssClassFa[`DomainObjectLayout`] and xref:rgant.adoc#_rgant_manpage-ViewModelLayout_cssClassFa[`ViewModelLayout`]
|UI
|Yes

|`@Debug`
|Action only invokable in debug mode.
|Not supported by either the xref:ugvw.adoc#[Wicket viewer] or the xref:ugvro.adoc#[RestfulObjects viewer]; use prototype mode instead (xref:rgant.adoc#_rgant_manpage-Action_restrictTo[`@Action#restrictTo()`])
|UI
|

|`@DescribedAs`
|Provide a longer description/tool-tip of an object or object member.
|`#describedAs()` attribute for xref:rgant.adoc#_rgant_manpage-DomainObject_describedAs[`@DomainObjectLayout`], xref:rgant.adoc#_rgant_manpage-PropertyLayout_describedAs[`@PropertyLayout`], xref:rgant.adoc#_rgant_manpage-CollectionLayout_describedAs[`@CollectionLayout`], xref:rgant.adoc#_rgant_manpage-ActionLayout_describedAs[`@ActionLayout`] and xref:rgant.adoc#_rgant_manpage-ParameterLayout_describedAs[`@ParameterLayout`]
|UI
|Yes
...
|`#editing()` attribute for xref:rgant.adoc#_rgant_manpage-Property_editing[`@Property`],  xref:rgant.adoc#_rgant_manpage-Collection_editing[`@Collection`] and xref:rgant.adoc#_rgant_manpage-DomainObject_editing[`@DomainObject`]
|UI, Domain
|Yes
...
|For domain object members, use `#hidden()` attribute of xref:rgant.adoc#_rgant_manpage-Action_hidden[`Action`], xref:rgant.adoc#_rgant_manpage-Property_hidden[`Property`] or xref:rgant.adoc#_rgant_manpage-Collection_hidden[`Collection`].  +
For domain service, use xref:rgant.adoc#_rgant_manpage-DomainService_nature[`@DomainService(` +
`nature=DOMAIN)`]
|UI, Domain
|Yes
...
|xref:rgant.adoc#_rgant_manpage-DomainObject_editing[`@DomainObject#editing()`]
|Domain
|
...
|`#maxLength()` attribute for xref:rgant.adoc#_rgant_manpage-Property_maxLength[`@Property`] or xref:rgant.adoc#_rgant_manpage-Parameter_maxLength[`@Parameter`]
|Domain
|
...
|`#multiLine()` attribute for xref:rgant.adoc#_rgant_manpage-Property_multiLine[`@Property`] or xref:rgant.adoc#_rgant_manpage-Parameter_multiLine[`@Parameter`]
|UI
|Yes

|`@MustSatisfy`
|Specify arbitrary specification constraints on a property or action parameter.
|`#mustSatisfy()` attribute for xref:rgant.adoc#_rgant_manpage-Property_mustSatisfy[`@Property`] or xref:rgant.adoc#_rgant_manpage-Parameter_mustSatisfy[`@Parameter`]
|Domain
|

|`@Named`
|Override name inferred from class. Required for parameter names (prior to Java8).
|`#named()` attribute for xref:rgant.adoc#_rgant_manpage-DomainServiceLayout_named[`@DomainServiceLayout`], xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_named[`@DomainObjectLayout`], xref:rgant.adoc#_rgant_manpage-PropertyLayout_named[`@PropertyLayout`], xref:rgant.adoc#_rgant_manpage-CollectionLayout_named[`@CollectionLayout`], xref:rgant.adoc#_rgant_manpage-ActionLayout_named[`@ActionLayout`] and xref:rgant.adoc#_rgant_manpage-ParameterLayout_named[`@ParameterLayout`]
|UI
|Yes
...
|Use xref:rgant.adoc#_rgant_manpage-DomainService_nature[`@DomainService#nature()`] to specify whether any of the actions in a domain service should appear in the menu bars (applies at type level, not action level).
For individual actions, use xref:rgant.adoc#_rgant_manpage-ActionLayout_contributedAs[`@ActionLayout#` +
...
|xref:rgant.adoc#_rgant_manpage-DomainService_nature[`@DomainService#nature()`] to signify that none of the actions in a domain service should appear in the menu bars
|UI
|
...
|`#notPersisted()` attribute of xref:rgant.adoc#_rgant_manpage-Property_notPersisted[`@Property`] and xref:rgant.adoc#_rgant_manpage-Collection_notPersisted[`@Collection`]
|Domain, Persistence
|
...
|xref:rgant.adoc#_rgant_manpage-DomainObject_objectType[`@DomainObject#objectType()`]
|Domain
|

|`@Optional`
|Specifies that a property or action parameter is not mandatory.
|`#optionality()` attribute for xref:rgant.adoc#_rgant_manpage-Property_optionality[`@Property`] or  xref:rgant.adoc#_rgant_manpage-Parameter_optionality[`@Parameter`]
|Domain
|

|`@Paged`
|Number of instances to display in tables representing (standalone or parented) collections.
|`#paged()` attribute for xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_paged[`@DomainObjectLayout`] or xref:rgant.adoc#_rgant_manpage-CollectionLayout_paged[`@CollectionLayout`]
|UI
|Yes

|`@Plural`
|For the irregular plural form of an entity type.
|xref:rgant.adoc#_rgant_manpage-DomainObject_plural[`@DomainObjectLayout` +
`#plural()`]
|UI
|
...
|xref:rgant.adoc#_rgant_manpage-Property_domainEvent[`@Property#domainEvent()`]
|Domain
|
...
|`#regexPattern()` for xref:rgant.adoc#_rgant_manpage-Property_regexPattern[`@Property`] or  xref:rgant.adoc#_rgant_manpage-Property_regexPattern[`@Parameter`].
|Domain
|

|`@Render`
|Eagerly (or lazily) render the contents of a collection.
|xref:rgant.adoc#_rgant_manpage-CollectionLayout_render[`@CollectionLayout` +
`#render()`]
|UI
|Yes
...
|`#renderedAsDayBefore()` attribute for xref:rgant.adoc#_rgant_manpage-PropertyLayout_renderedAsDayBefore[`@PropertyLayout`] and xref:rgant.adoc#_rgant_manpage-ParameterLayout_renderedAsDayBefore[`@ParameterLayout`].
|UI
|
...
|xref:rgant.adoc#_rgant_manpage-CollectionLayout_sortedBy[`@CollectionLayout` +
`#sortedBy()`]
|UI
|Yes
...
|`#typeOf()` attribute for xref:rgant.adoc#_rgant_manpage-Collection_typeOf[`@Collection`] and xref:rgant.adoc#_rgant_manpage-Action_typeOf[`@Action`]
|Domain
|

|`@TypicalLength`
|The typical length of a string property, eg to determine a sensible length for a textbox.
|`#typicalLength()` attribute for xref:rgant.adoc#_rgant_manpage-PropertyLayout_typicalLength[`@PropertyLayout`] and xref:rgant.adoc#_rgant_manpage-ParameterLayout_typicalLength[`@ParameterLayout`]
|UI
|Yes
