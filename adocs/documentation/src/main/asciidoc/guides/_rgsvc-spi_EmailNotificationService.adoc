[[_rgsvc-spi_EmailNotificationService]]
...
The default implementation of this service uses the xref:rgsvc.adoc#_rgsvc-api_EmailService[`EmailService`], which must be configured in order for user registration to be enabled.
...
As noted elsewhere, the default implementation of this service uses xref:rgsvc.adoc#_rgsvc-api_EmailService[`EmailService`].  This service has no specific configuration properties but does require that the `EmailService` has been configured.

Conversely, this service is used by (Isis' default implementation of) xref:rgsvc.adoc#_rgsvc-spi_UserRegistrationService[`UserRegistrationService`].
