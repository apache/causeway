[[_rgsvc-api_manpage-XmlSnapshotService]]
= `XmlSnapshotService`
...
The xref:rgsvc.adoc#_rgsvc-api_manpage-BookmarkService[`BookmarkService`] provides a mechanism for obtaining a string representations of a single domain object.

The xref:rgsvc.adoc#_rgsvc-api_manpage-MementoService[`MementoService`] also provides a mechanism for generating string representations of domain objects.

The xref:rgsvc.adoc#_rgsvc-api_manpage-JaxbService[`JaxbService`] (`1.11.0-SNAPSHOT`) is a simple wrapper around
