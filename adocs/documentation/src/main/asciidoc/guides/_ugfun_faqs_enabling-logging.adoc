...
* In the JDBC Driver +
+
Configure `log4jdbc` JDBC rather than the vanilla driver (see `WEB-INF/persistor_datanucleus.properties`) and configure log4j logging (see `WEB-INF/logging.properties`).  There are examples of both in the xref:ug.adoc#_ugfun_getting-started_simpleapp-archetype[SimpleApp archetype].
