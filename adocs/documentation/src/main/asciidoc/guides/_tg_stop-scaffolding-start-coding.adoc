...
* use the injected xref:rgsvc.adoc#_rgsvc-api_manpage-IsisJdoSupport[`IsisJdoSupport`] domain service.
...
* the service itself should use xref:rgsvc.adoc#_rgsvc-api_manpage-DomainObjectContainer[`DomainObjectContainer`]
...
* inject xref:rgsvc.adoc#_rgsvc-api_manpage-ClockService[`ClockService`]
...
Using the guidance in the docs for the xref:rgsvc.adoc#_rgsvc-api_manpage-EventBusService[`EventBusService`]:
...
* Inject the xref:rgsvc.adoc#_rgsvc-api_manpage-ActionInvocationContext[`ActionInteractionContext`] (request-scoped) service
...
The similar xref:rgsvc.adoc#_rgsvc-api_manpage-Scratchpad[`Scratchpad`] (request-scoped) domain service is a good way to share information between bulk action invocations:
...
The xref:rgsvc.adoc#_rgsvc-api_manpage-QueryResultsCache[`QueryResultsCache`] (request-scoped) domain service allows arbitrary objects to be cached for the duration of a request.
...
Isis has great support for writing xref:ugtst.adoc#_ugtst_integ-test-support[integration tests]; well-written integration tests should leverage fixture scripts and use the xref:rgsvc.adoc#_rgsvc-api_manpage-WrapperFactory[`@WrapperFactory`] domain service.
