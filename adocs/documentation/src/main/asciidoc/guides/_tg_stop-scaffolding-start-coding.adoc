...
* Run with different deploymentTypes; note whether prototype actions (those annotated xref:rgant.adoc#_rgant_manpage-Action_restrictTo[`@Action(restrictTo=PROTOTYPING`]) are available or not:
...
* make the entity bookmarkable by adding the xref:rgant.adoc#_rgant_manpage-DomainObjectLayout_bookmarking[`@DomainObjectLayout#bookmarking()`] attribute.
...
* rename if you wish
** eg `listPets()`
* note the xref:rgant.adoc#_rgant_manpage-DomainService[`@DomainService`] annotation
...
* use the xref:rgant.adoc#_rgant_manpage-ParameterLayout_named[`@ParameterLayout(named=...)`] annotation to specify the name of action parameters
* use the xref:rgant.adoc#_rgant_manpage-Action_semantics[`@Action(semanticsOf=...)`]  annotation to indicate the semantics of the action (safe/query-only, idempotent or non-idempotent)
* annotate safe action as bookmarkable using xref:rgant.adoc#_rgant_manpage-ActionLayout_bookmarking[`@ActionLayout(bookmarking=...)`]
* confirm is available from bookmark panel (top-left of Wicket UI)
...
* experiment changing xref:rgant.adoc#_rgant_manpage-Action_semantics[`@Action(semantics=...)`] on actions
...
** use the xref:rgant.adoc#_rgant_manpage-PropertyLayout_multiLine[`@Property(multiLine=...)`] annotation to render a text area instead of a text box
** use the xref:rgant.adoc#_rgant_manpage-Property_maxLength[`@Property(maxLength=...)`] annotation to specify the maximum number of characters allowable
** use joda date/time properties, bigdecimals and blob/clob properties
* use the xref:rgant.adoc#_rgant_manpage-Property_optionality[`@Column(allowsNull=...)`] annotation specify whether a property is optional or mandatory
...
* revisit the title, consider whether to use the xref:rgant.adoc#_rgant_manpage-Title[`@Title`] annotation
** rather than the xref:rg.adoc#_rg_methods_reserved_manpage-title[`title()`] `title()` method
* order the properties using the xref:rgant.adoc#_rgant_manpage-MemberOrder[`@MemberOrder`], also `@MemberGroupLayout`
** see also the docs on xref:rg.adoc#_rg_object-layout_static[static layouts]
* use the xref:rgant.adoc#_rgant_manpage-PropertyLayout[`@PropertyLayout`] annotation to position property/action parameter labels either to the LEFT, TOP or NONE
** do the same for parameters using xref:rgant.adoc#_rgant_manpage-ParameterLayout[`@ParameterLayout`]
...
** use the xref:rgant.adoc#_rgant_manpage-DomainObject_bounded[`@DomainObjectLayout(bounded=...)`] annotation on the referenced type if there are only a small number (bounded) of instances
...
** Use the xref:rgant.adoc#_rgant_manpage-CollectionLayout_render[`@CollectionLayout(render=...)`] annotation to indicate if the collection should be visible or hidden by default
* optional: use the xref:rgant.adoc#_rgant_manpage-CollectionLayout_sortedBy[`@CollectionLayout(sortedBy=...)`] annotation to specify a different comparator than the natural ordering
...
** generally we recommend using the xref:rgant.adoc#_rgant_manpage-Inject[`@Inject`] annotation with either private or default visibility
* the service itself should use xref:rg.adoc#_rg_services-api_manpage-DomainObjectContainer[`DomainObjectContainer`]
* use the xref:rgant.adoc#_rgant_manpage-MemberOrder[`@MemberOrder(name=...)`] annotation to associate an action with a property or with a collection
...
* the bootstrap "btn" CSS classes can be used using the xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClass[`@ActionLayout(cssClass=...)`] annotation

*  the http://fortawesome.github.io/Font-Awesome/icons/[Font Awesome] icons can be used using the xref:rgant.adoc#_rgant_manpage-ActionLayout_cssClassFa[`@ActionLayout(cssClassFa=...)`]
...
* Use the xref:rgant.adoc#_rgant_manpage-Property_hidden[`Property(hidden=...)`] annotation to make properties invisible
** likewise xref:rgant.adoc#_rgant_manpage-Collection_hidden[`@Collection(hidden=...)`] for collections
** the xref:rgant.adoc#_rgant_manpage-Programmatic[`@Programmatic`]  annotation can also be used and in many cases is to be preferred; the difference is that the latter means the member is not part of the Apache Isis metamodel.
...
* Use the xref:rgant.adoc#_rgant_manpage-Property_editing[`Property(editing=...)`] annotation to make property read-only
** likewise xref:rgant.adoc#_rgant_manpage-Collection_editing[`@Collection(editing=...)`] for collections
** alternatively, use xref:rgant.adoc#_rgant_manpage-DomainObject_editing[`@DomainObject(editing=...)`] to disable editing for all properties/collections
...
* use the xref:rgant.adoc#_rgant_manpage-Property_regexPattern[`@Property(regexPattern=...)`] annotation to specify a regex pattern for properties, and use xref:rgant.adoc#_rgant_manpage-Parameter_regexPattern[`@Parameter(regexPattern=...)`] for parameters
* use the xref:rgant.adoc#_rgant_manpage-Property_maxLength[`@Property(maxLength=...)`] annotation to indicate a maxmum number of characters, and xref:rgant.adoc#_rgant_manpage-Parameter_maxLength[`@Parameter(maxLength=...)`] for parameters
...
** use the xref:rgant.adoc#_rgant_manpage-Property_mustSatisfy[`Property(mustSatisfy=...)`] and xref:rgant.adoc#_rgant_manpage-Parameter_mustSatisfy[`Parameter(mustSatisfy=...)`] annotations to specify arbitrary constraints on properties and parameters
...
* Add the xref:rgant.adoc#_rgant_manpage-HomePage[`@HomePage`] annotation to one (no more) of the domain services' no-arg actions
...
** annotate with xref:rgant.adoc#_rgant_manpage-DomainService_nature[`@DomainService(nature=NatureOfService.VIEW_CONTRIBUTIONS_ONLY)`]
...
* For this action:
** add the xref:rgant.adoc#_rgant_manpage-ActionLayout_contributedAs[`@ActionLayout(contributedAs=ASSOCIATION)`] annotation
...
Bulk actions are actions that can be invoked on a collection of actions, that is on collections returned by invoking an action. Actions are specified as being bulk actions using the xref:rgant.adoc#_rgant_manpage-Action_invokeOn[`@action(invokeOn=OBJECT_AND_COLLECTION)`] annotation.
...
To support these use cases, Apache Isis therefore allows you to write a view model, either by annotating the class with xref:rgant.adoc#_rgant_manpage-ViewModel[`@ViewModel`] or (for more control) by implementing the xref:rg.adoc#_rg_classes_super_manpage-AbstractViewModel[`ViewModel`] interface.
...
Isis has great support for writing xref:ugtst.adoc#_ugtst_integ-test-support[integration tests]; well-written integration tests should leverage fixture scripts and use the xref:rg.adoc#_rg_services-api_manpage-WrapperFactory[`@WrapperFactory`] domain service.
...
* as per xref:ugvro.adoc#_ugvro_configuration-properties_simplified-object-representation[the documentation], configure the Restful Objects viewer to generate a simplified object representation: +
+
[source,ini]
----
