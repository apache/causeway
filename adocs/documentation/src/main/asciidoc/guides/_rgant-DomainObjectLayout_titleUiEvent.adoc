...
This is as an alternative to implementing xref:rgcms.adoc#_rgcms_methods_reserved_manpage-title[`title()`] reserved method, or
...
by calling the xref:rgsvc.adoc#_rgsvc-api_EventBusService[`EventBusService`] API directly, or as a result
of calling the xref:rgsvc.adoc#_rgsvc-api_DomainObjectContainer[`DomainObjectContainer`]'s
`titleOf(...)` method.
