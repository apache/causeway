...
It also supports translations of messages raised imperatively, by which we mean as the result of a call to `title()` to obtain an object's title, or messages resulting from any business rule violations (eg xref:rgcms.adoc#_rgcms_methods_prefixes_manpage-disable[`disable...()`] or xref:rgcms.adoc#_rgcms_methods_prefixes_manpage-validate[`validate...()`], and so on.
...
The same class is used in xref:rgsvc.adoc#_rgsvc-api_DomainObjectContainer[`DomainObjectContainer`] so that you can raise translatable info, warning and error messages; each of the relevant methods are overloaded.
...
Another mechanism by which messages can be rendered to the user are as the result of exception messages thrown and recognized by an xref:rgsvc.adoc#_rgsvc-spi_ExceptionRecognizer[`ExceptionRecognizer`].
...
Behind the scenes Apache Isis uses a JUnit rule (`ExceptionRecognizerTranslate`) to intercept any exceptions that are thrown.  These are simply passed through to the registered xref:rgsvc.adoc#_rgsvc-spi_ExceptionRecognizer[`ExceptionRecognizer`]s so that any messages are recorded as requiring translation.
====
