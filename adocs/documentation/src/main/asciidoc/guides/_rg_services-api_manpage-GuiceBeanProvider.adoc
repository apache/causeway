[[_rg_services-api_manpage-GuiceBeanProvider]]
= `GuiceBeanProvider`
...
The `GuiceBeanProvider` domain service acts as a bridge between Apache Isis' xref:ugvw.adoc#_ugvw[Wicket viewer] internal bootstrapping using link:https://github.com/google/guice[Google Guice].

This service operates at a very low-level, and you are unlikely to have a need for it.  It is used internally by the framework, in the default implementation of the xref:rg.adoc#_rg_services-api_manpage-DeepLinkService[`DeepLinkService`].
...
The xref:ugvw.adoc#_ugvw[Wicket viewer] this provides an implementation of this service.



== Usage

Using the xref:ugvw.adoc#_ugvw[Wicket viewer] requires subclassing of `IsisWicketApplication`.  In the subclass it is commonplace to override `newIsisWicketModule()`, for example:
...
xref:ugvw.adoc#_ugvw[Wicket viewer] is being used, then an implementation of `GuiceBeanProvider` is
