...
The `GuiceBeanProvider` domain service acts as a bridge between Apache Isis' xref:ug.adoc#_ug_wicket-viewer[Wicket viewer] internal bootstrapping using link:https://github.com/google/guice[Google Guice].
...
The xref:ug.adoc#_ug_wicket-viewer[Wicket viewer] this provides an implementation of this service.



== Usage

Using the xref:ug.adoc#_ug_wicket-viewer[Wicket viewer] requires subclassing of `IsisWicketApplication`.  In the subclass it is commonplace to override `newIsisWicketModule()`, for example:
