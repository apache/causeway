[[_ug_testing_bdd-spec-support_writing-a-bdd-spec]]
= Writing a BDD spec
...
* a system initializer class.  You can reuse the  system initializer from any integration tests (as described in xref:ug.adoc#_ug_testing_integ-test-support[Integration Test Support], bootstrapping section).


Here's an example of a feature from the xref:ug.adoc#_ug_getting-started_simpleapp-archetype[SimpleApp archetype]:
...
The JSON formatter allows integration with enhanced reports, for example as provided by http://www.masterthought.net/section/cucumber-reporting[Masterthought.net] (screenshots at end of page). (Commented out) configuration for this is provided in the xref:ug.adoc#_ug_getting-started_simpleapp-archetype[SimpleApp archetype].
