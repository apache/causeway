[[_ugbtb_more-advanced_view-models_programming-model]]
= Programming Model
...
|Annotated with xref:rgant.adoc#_rgant_manpage-ViewModel[`@ViewModel`] annotation (effectively just an alias)' memento is as preceding: from "persisted" properties, collections ignored
...
----
|Annotate using JAXB xref:rgant.adoc#_rgant_manpage-XmlRootElement[`@XmlRootElement`] annotation.  Memento
