...
URL can become quite long, but Apache Isis offers a mechanism (the xref:rgsvc.adoc#_rgsvc-spi_UrlEncodingService[`UrlEncodingService`]) if it exceeds the maximum length for a URL
...
|Implement xref:rgcms.adoc#_rgcms_classes_super_manpage-ViewModel[`ViewModel`] interface.  The memento is as defined by the
