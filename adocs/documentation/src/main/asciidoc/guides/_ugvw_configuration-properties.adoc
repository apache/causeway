...
| `true`,`false` +
(`_true_`)
| (`1.13.2-SNAPSHOT`) Whether to disable a form submit button after it has been clicked, to prevent users causing an error if they do a double click.

|`isis.viewer.wicket.` +
`preventDoubleClickForNoArgAction`
| `true`,`false` +
(`_true_`)
| (`1.13.2-SNAPSHOT`) Whether to disable a no-arg action button after it has been clicked, to prevent users causing an error if they do a double click.
...
| `true`,`false` +
(`_true_`)
| (`1.13.2-SNAPSHOT`) Whether to replace 'disabled' tag with 'readonly' (for link:https://www.w3.org/TR/2014/REC-html5-20141028/forms.html#the-readonly-attribute[w3 spec]-compliant browsers such as for Firefox and Chrome 54+) which prevent copy from 'disabled' fields.
...
In prototype mode, though, (as of `1.13.2-SNAPSHOT`), a fixed key will still be used; this saves the developer having to login each time.
...
| `true`,`false` +
(`_true_`)
| (`1.13.2-SNAPSHOT`) Whether to show an indicator for a form submit button that it has been clicked.

|`isis.viewer.wicket.` +
`useIndicatorForNoArgAction`
| `true`,`false` +
(`_true_`)
| (`1.13.2-SNAPSHOT`) Whether to show an indicator for a no-arg action button that it has been clicked.
