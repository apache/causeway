...
Isis also provides xref:rgant.adoc#_rgant_manpage-Property_optionality[`@Property#optionality()`] attribute.  If both are specified, Apache Isis will check when it initializes for any contradictions, and will fail-fast with an appropriate error message in the log if there are.
...
In the vast majority of cases you should be fine just to add the `@Column#allowsNull()` attribute to the getter.  But see the documentation for xref:rgant.adoc#_rgant_manpage-Property_optionality[`@Property#optionality()`] attribute for discussion on one or two minor edge cases.
...
Isis also provides xref:rgant.adoc#_rgant_manpage-Property_maxLength[`@Property#maxLength()`] attribute.  If both are specified, Apache Isis will check when it initializes for any contradictions, and will fail-fast with an appropriate error message in the log if there are.
...
For ``BigDecimal``s it is also possible to specify the xref:rgant.adoc#_rgant_manpage-Digits[`@Digits`] annotation, whose form is `@Digits(integer, fraction)`.  There is a subtle difference here: while `@Column#scale()` corresponds to `@Digits#fraction()`, the value of `@Column#length()` (ie the precision) is actually the _sum_ of the `@Digits`' `integer()` and `fraction()` parts.
