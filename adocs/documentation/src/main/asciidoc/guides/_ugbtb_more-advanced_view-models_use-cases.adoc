...
business.  Then build out the minimum domain entities to support that use case (refining the xref:ug.adoc#_ugfun_core-concepts_philosophy_domain-driven-design_ubiquitous-language[ubiquitous language] as you
...
the xref:rgsvc.adoc#_rgsvc-spi_ContentMappingService[`ContentMappingService`]); the XSD can be obtained
using the xref:rgsvc.adoc#_rgsvc-api_JaxbService[`JaxbService`].  Using the formalism of XML also allows
DTOs to be carefully and properly versioned using XML namespaces.

[TIP]
====
If the view model DTO implements the xref:rgcms.adoc#_rgcms_classes_mixins_Dto[`Dto`] marker interface, then the framework
...
A xref:rgsvc.adoc#_rgsvc-spi_PublishingService[`PublishingService`] implementation (eg that provided by
