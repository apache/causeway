...
The Apache Isis framework also provides numerous in-built domain services.  These are catalogued in the reference guide, see xref:rgsvc.adoc#_rgsvc_api[here] and xref:rgsvc.adoc#_rgsvc_spi[here].
...
The framework provides a number of request-scoped services, include a scratchpad service, query results caching, and support for co-ordinating bulk actions.  See xref:rgsvc.adoc#_rgsvc_api[here] and xref:rgsvc.adoc#_rgsvc_spi[here] for further details.
...
Examples of framework-provided services (as defined in the applib) include clock, auditing, publishing, exception handling, view model support, snapshots/mementos, and user/application settings management; see the xref:rgsvc.adoc#_rgsvc_api[here] and _rgsvc_spi[here] for further details.
