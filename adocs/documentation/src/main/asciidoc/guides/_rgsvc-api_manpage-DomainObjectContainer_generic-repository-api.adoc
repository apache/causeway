[[_rgsvc-api_manpage-DomainObjectContainer_generic-repository-api]]
...
As an alternative, you could also use link:http://www.datanucleus.org/products/accessplatform_4_0/jdo/jdoql_typesafe.html[JDO typesafe queries] through the xref:rgsvc.adoc#_rgsvc-api_manpage-IsisJdoSupport[`IsisJdoSupport`] service.
...
[TIP]
====
It is also possible to query using DataNucleus' type-safe query API.  For more details, see xref:rgsvc.adoc#_rgsvc-api_manpage-IsisJdoSupport[`IsisJdoSupport`].
====
