...
Metadata providing UI hints can be specified either statically, using annotations, or dynamically, using either a xref:ugfun.adoc#_ugfun_object-layout_dynamic_xml[`layout.xml`] file (`1.12.0-SNAPSHOT`) or a xref:ugfun.adoc#_ugfun_object-layout_dynamic[`.layout.json`] file.
