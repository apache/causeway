[[_rg_methods_prefixes_manpage-validate]]
= `validate...()`
...
The reason for vetoing a modification/invocation is normally returned as a string.  However, Apache Isis' xref:ug.adoc#_ug_more-advanced_i18n[i18n support] extends this so that reasons can be internationalized if required.
