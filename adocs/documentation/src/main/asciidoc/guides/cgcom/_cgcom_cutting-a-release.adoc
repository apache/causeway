...
Obviously, alter `$ISISDEV` and ``$ISISREL`` as required (``$ISISDEV`` should be the snapshot after ``$ISISREL``), and bump ``$ISISRC`` for re-releasing following an xref:../cgcom/cgcom.adoc#_cgcom_post-release-unsuccessful[unsuccessful] release.
...
After release, there is a xref:cgcom.adoc#__cgcom_cutting-a-release_bump-revision-to-ISISDEV[similar step at the end] to update back to `1.0.0-SNAPSHOT`.
...
With the release now deployed we now need to bump the revision back down to the base snapshot, ie `1.0.0-SNAPSHOT`.
...
[source,bash,subs="attributes+"]
----
sed -i -E "s|<revision>[^<]+<|<revision>1.0.0-SNAPSHOT<|g" core/pom.xml
sed -i -E "s|<revision>[^<]+<|<revision>1.0.0-SNAPSHOT<|g" example/application/simpleapp/pom.xml
sed -i -E "s|<revision>[^<]+<|<revision>1.0.0-SNAPSHOT<|g" example/application/helloworld/pom.xml
git commit -am "${ISISJIRA}: bumps revision property across all pom.xml's back to 1.0.0-SNAPSHOT"
----
