...
[NOTE]
====
One omission: Isis (currently) does not inject services into `o.a.i.applib.spec.Specification` instances (as used by xref:rg.adoc#_rg_annotations_manpage-Property_mustSatisfy[`@Property#mustSatisfy()`] and xref:rg.adoc#_rg_annotations_manpage-Parameter_mustSatisfy[`@Parameter#mustSatisfy()`] annotations.
====
