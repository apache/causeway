[[_rg_annotations_manpage-PropertyLayout_labelPosition]]
= `labelPosition()`
...
The `labelPosition()` attribute determines the positioning of labels for properties.  This attribute can also be specified for xref:rg.adoc#_rg_annotations_manpage-ParameterLayout_labelPosition[parameters].

The positioning of labels is typically `LEFT`, but can be positioned to the `TOP`.  The one exception is xref:rg.adoc#_rg_annotations_manpage-ParameterLayout_multiLine[`multiLine()`] string properties, where the label defaults to `TOP` automatically (to provide as much real-estate for the multiline text field as possible).
...
To get an idea of how these are rendered (in the xref:ug.adoc#_ug_wicket-viewer[Wicket viewer]), we can look at the (non-ASF) http://github.com/isisaddons/isis-app-todoapp[Isis addons' todoapp] that happens to have examples of most of these various label positions.
