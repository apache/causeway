...
extends TextFieldWithConverter<T> {
    ...
    private DateTimeConfig createDatePickerConfig(
        ...
        val isisDatePickerConfig = commonContext.getConfiguration().getViewer().getWicket().getDatePicker();

        //XXX future extensions might allow to set bounds on a per member basis (via ValueSemantics annotation)
        config.minDate(isisDatePickerConfig.getMinDate());
        config.maxDate(isisDatePickerConfig.getMaxDate());
        config.allowInputToggle(isisDatePickerConfig.isPopupOnFocus());
        return config;
    }
