...
public interface PopModelChangeDispatcher {

    @NonNull Iterable<PopModelChangeListener> getChangeListeners();
    @NonNull PopPanelAbstract getPopPanel();

    default void notifyUpdate(final AjaxRequestTarget target) {
        for (val listener : getChangeListeners()) {
            listener.onUpdate(target, getPopPanel());
        }
    }

    default void notifyError(final AjaxRequestTarget target) {
        for (val listener : getChangeListeners()) {
            listener.onError(target, getPopPanel());
        }
    }
