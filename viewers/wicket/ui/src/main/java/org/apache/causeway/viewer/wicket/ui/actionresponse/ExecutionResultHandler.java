...
public record ExecutionResultHandler(Either<ActionModel, PropertyModel> actionOrPropertyModel) {

    public void handle(final AjaxRequestTarget ajaxTarget, final ManagedObject result) {
        // triggers ManagedObject.getBookmarkRefreshed()
        var mediator = actionOrPropertyModel.fold(
                act->Mediator.determineAndInterpretResult(act, ajaxTarget, result),
