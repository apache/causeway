...
import org.apache.causeway.viewer.wicket.ui.components.pops.ScalarPanelFormFieldAbstract;
...
extends ScalarPanelFormFieldAbstract<T> {
    ...
    protected CausewayBlobOrClobPanelAbstract(final String id, final PopModel popModel, final Class<T> type) {
        super(id, popModel, type);
        this.unwrapped = popModel.unwrapped(type);
    }
    ...
    @Override
    protected FormComponent createFormComponent(final String id, final PopModel popModel) {
    ...
    private void addAcceptFilterTo(final Component component){
        Wkt.attributeReplace(component, "accept", popModel().getFileAccept());
    }
...
//            setEnabled(false);
//            PopModel model = CausewayBlobOrClobPanelAbstract.this.getModel();
...
//        val blob = popModel().unwrapped(Blob.class).getObject();
