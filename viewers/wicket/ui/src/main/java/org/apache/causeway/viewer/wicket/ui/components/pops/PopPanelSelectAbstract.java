...
public abstract class PopPanelSelectAbstract
extends PopPanelFormFieldAbstract<ManagedObject> {
    ...
    protected PopPanelSelectAbstract(
            final String id,
            final PopModel popModel) {
        super(id, popModel, ManagedObject.class);
        setOutputMarkupId(true);
    }

    protected final Select2 createSelect2(
            final String id,
            final Function<PopModel, ChoiceProviderAbstract> choiceProviderFactory) {
        val popModel = popModel();
        val select2 = Select2.createSelect2(id, popModel,
                choiceProviderFactory.apply(popModel),
                getPopModelChangeDispatcher());
        val settings = select2.getSettings();
        settings.setPlaceholder(popModel.getFriendlyName());

        switch(popModel.getChoiceProviderSort()) {
        case CHOICES:
            break;
        case AUTO_COMPLETE:
            settings.setMinimumInputLength(popModel.getAutoCompleteMinLength());
            break;
        case OBJECT_AUTO_COMPLETE:
            Facets.autoCompleteMinLength(popModel.getScalarTypeSpec())
    ...
    @Override
    protected final void installPopModelChangeBehavior() {
    ...
    protected final boolean isEditable() {
        ...
        return !popModel.getRenderingHint().isInTable()
                && !popModel.isViewingMode();
    }

    protected final boolean hasAnyChoices() {
        return popModel().getChoiceProviderSort().isChoicesAny();
    }
