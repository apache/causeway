 ...
 *  under the License.
 */
package org.apache.causeway.viewer.wicket.ui.components.pops.choices;
...
import org.apache.causeway.viewer.wicket.ui.components.pops.PopPanelSelectAbstract;
...
import org.apache.causeway.viewer.wicket.ui.components.pops.PopPanelSelectAbstract.ChoiceTitleHandler;
...
extends PopPanelSelectAbstract
implements ChoiceTitleHandler {
    ...
    public ObjectChoicesSelect2Panel(final String id, final PopModel popModel) {
        super(id, popModel);
        this.isCompactFormat = !popModel.getRenderingHint().isRegular();
    }
    ...
    protected Component createComponentForOutput(final String id) {
        ...
        final String name = popModel.getFriendlyName();

        this.entityLinkOutputFormat = (EntityLinkSimplePanel) getComponentFactoryRegistry()
                .createComponent(UiComponentType.ENTITY_LINK, popModel);
    ...
    @Override
    protected FormComponent<ManagedObject> createFormComponent(final String id, final PopModel popModel) {

        this.entityLink = new ChoiceFormComponent(UiComponentType.ENTITY_LINK.getId(), this);
        entityLink.setRequired(popModel.isRequired());
    ...
    private void syncWithInput() {
        ...
        .ifPresent(container->{
            val componentFactory = getComponentFactoryRegistry()
                    .findComponentFactory(UiComponentType.ENTITY_ICON_AND_TITLE, popModel);
            val iconAndTitle = componentFactory
                    .createComponent(UiComponentType.ENTITY_ICON_AND_TITLE.getId(), popModel);
            container.addOrReplace(iconAndTitle);

            val isInlinePrompt = popModel.isInlinePrompt();
            if(isInlinePrompt) {
    ...
    String getTitleForFormComponentInput() {
        val pendingElseCurrentAdapter = popModel().getObject();
    ...
    */
    void convertInput() {
        ...
        val popModel = popModel();
        val pendingValue = popModel.proposedValue().getValue();
...
//    @Override
//    public void onUpdate(final AjaxRequestTarget target, final PopPanelAbstract popPanel) {
//        super.onUpdate(target, popPanel);
