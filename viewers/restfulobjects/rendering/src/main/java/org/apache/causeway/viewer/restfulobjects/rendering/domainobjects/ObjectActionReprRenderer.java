...
extends AbstractObjectMemberReprRenderer<ObjectAction> {
    ...
    protected void followDetailsLink(final JsonRepresentation detailsLink) {
        ...
        renderer.with(ManagedAction.of(objectAdapter, objectMember, resourceContext.visibilityConstraint())).usingLinkTo(linkTo).asFollowed();
    ...
    private ObjectActionReprRenderer addParameterDetails() {
        ...
        if(objectMember.getParameterCount()>0) {
            var act = ManagedAction.of(objectAdapter, objectMember, ResourceContext.visibilityConstraint(Where.ANYWHERE));
