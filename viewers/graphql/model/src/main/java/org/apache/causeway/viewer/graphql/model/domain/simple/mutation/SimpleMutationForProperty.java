...
public class SimpleMutationForProperty extends Element {
    ...
    protected Object fetchData(final DataFetchingEnvironment dataFetchingEnvironment) {
        ...
        var visibleConsent = oneToOneAssociation.isVisible(managedObject, iConstraint);
        if (visibleConsent.isVetoed())
			throw new HiddenException(oneToOneAssociation.getFeatureIdentifier());

        var usableConsent = oneToOneAssociation.isUsable(managedObject, iConstraint);
        if (usableConsent.isVetoed())
			throw new DisabledException(oneToOneAssociation.getFeatureIdentifier());

        var validityConsent = oneToOneAssociation.isAssociationValid(managedObject, argumentManagedObject, iConstraint);
        if (validityConsent.isVetoed())
			throw new InvalidException(validityConsent);

        oneToOneAssociation.set(managedObject, argumentManagedObject, iConstraint.initiatedBy());
