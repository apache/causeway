...
@SpringJUnitWebConfig(VueSecmanHttpSecurityTest.TestConfiguration.class)
class VueSecmanHttpSecurityTest {
    ...
    void chainProtectsOnlyViewerAndGraphQlAndKeepsLoginPublic() throws Exception {
        ...
        mvc.perform(get("/vue/login.css"))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith("text/css"));
        mvc.perform(get("/vue/object/42"))
                .andExpect(status().isFound())
                .andExpect(header().string("Location", "/vue/login"));
    ...
    @Test
    void authenticationContextRequiresSessionAndReturnsOnlyBoundedNoStoreState() throws Exception {
        mvc.perform(get("/vue/authentication"))
                .andExpect(status().isUnauthorized());

        mvc.perform(get("/vue/authentication").with(user("sven")))
                .andExpect(status().isOk())
                .andExpect(header().string("Cache-Control", containsString("no-store")))
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(content().string(containsString("\"username\":\"sven\"")))
                .andExpect(content().string(containsString("\"csrfHeaderName\":\"X-CSRF-TOKEN\"")))
                .andExpect(content().string(containsString("\"loginPath\":\"/vue/login\"")))
                .andExpect(content().string(containsString("\"logoutPath\":\"/vue/logout\"")))
                .andExpect(content().string(org.hamcrest.Matchers.not(containsString("password"))))
                .andExpect(content().string(org.hamcrest.Matchers.not(containsString("roles"))));
    }

    @Test
    void loginRejectsMissingCsrfAndInvalidCredentialsGenerically() throws Exception {
        mvc.perform(post("/vue/login").param("username", "absent").param("password", "wrong"))
                .andExpect(status().isForbidden());
        mvc.perform(post("/vue/login").with(csrf().useInvalidToken())
                        .param("username", "absent").param("password", "wrong"))
                .andExpect(status().isForbidden());
        when(repository.findByUsername("absent")).thenReturn(Optional.empty());
        mvc.perform(post("/vue/login").with(csrf())
                        .param("username", "absent").param("password", "wrong"))
                .andExpect(status().isFound())
                .andExpect(redirectedUrl("/vue/login?error=true"));
        mvc.perform(get("/vue/login").param("error", "true"))
                .andExpect(status().isOk())
    ...
    void validLoginMigratesSessionAndRestoresSafeDeepLink() throws Exception {
        ...
        mvc.perform(get("/vue/object/42?tab=visits").session(session))
                .andExpect(status().isFound());

        final var result = mvc.perform(post("/vue/login").session(session).with(csrf())
                        .param("username", "sven").param("password", "correct horse"))
                .andExpect(status().isFound())
                .andExpect(redirectedUrl("/vue/object/42?tab=visits"))
                .andReturn();
    ...
    void unsafeContinueFallsBackToViewerRoot() throws Exception {
        ...
        mvc.perform(post("/vue/login").with(csrf())
                        ...
                        .param("continue", "https://attacker.example/vue"))
                .andExpect(status().isFound())
                .andExpect(redirectedUrl("/vue"));
    }
    ...
    void csrfProtectsGraphQlAndLogoutAndLogoutRemovesAuthentication() throws Exception {
        ...
        final var login = mvc.perform(post("/vue/login").with(csrf())
                        .param("username", "sven").param("password", "correct horse"))
                .andReturn();
        final var session = (MockHttpSession) login.getRequest().getSession(false);
        mvc.perform(post("/vue/logout").session(session))
                .andExpect(status().isForbidden());
        mvc.perform(post("/vue/logout").session(session).with(csrf()))
                .andExpect(status().isFound())
                .andExpect(redirectedUrl("/vue/login?logout=true"))
                .andExpect(header().string("Cache-Control", containsString("no-store")));
        mvc.perform(get("/vue"))
                .andExpect(status().isFound());
    }
    ...
    void authenticatedForbiddenOutcomeIsNotReportedAsSessionExpiry() throws Exception {
        mvc.perform(get("/vue/forbidden").with(user("sven")))
                .andExpect(status().isForbidden())
    ...
    @org.springframework.context.annotation.Import({
            ...
            VueSecmanSecurityConfiguration.class
    })
    static class TestConfiguration {

        @Bean
        VueSecmanSecurityProperties vueSecmanSecurityProperties() {
    ...
    static class TestEndpoints {
        @GetMapping({"/vue", "/vue/object/42"})
        String viewer() {
            return "viewer";
        }

        @GetMapping("/vue/forbidden")
