...
@Service
public class PetOwnerCollectionFilterService implements CollectionFilterService {

    @Override
    public boolean handles(final Class<?> domainType) {
        return PetOwner.class.isAssignableFrom(domainType);
    }

    @Override
    public <T> Function<T, Tokens> tokenizer(final Class<T> domainType) {
        return candidate -> {
            var owner = (PetOwner) candidate;
            var searchable = String.join(" ",
                    owner.getName(),
                    owner.getKnownAs() == null ? "" : owner.getKnownAs())
    ...
    public TranslatableString searchPromptPlaceholderText(final Class<?> domainType) {
        return TranslatableString.tr("Search owner names");
    }
}
