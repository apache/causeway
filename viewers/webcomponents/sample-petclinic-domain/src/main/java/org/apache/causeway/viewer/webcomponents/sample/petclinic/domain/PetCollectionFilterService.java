...
@Service
public class PetCollectionFilterService implements CollectionFilterService {

    @Override
    public boolean handles(final Class<?> domainType) {
        return Pet.class.isAssignableFrom(domainType);
    }

    @Override
    public <T> Function<T, Tokens> tokenizer(final Class<T> domainType) {
        return candidate -> {
            var pet = (Pet) candidate;
            var searchable = String.join(" ",
                    pet.getName(),
                    pet.getSpecies().name(),
                    pet.getNotes() == null ? "" : pet.getNotes())
    ...
    public TranslatableString searchPromptPlaceholderText(final Class<?> domainType) {
        return TranslatableString.tr("Search pets");
    }
}
