...
const bundlePath = resolve(directory, 'generated/assets/vaadin-reference.js');
...
if (!existsSync(bundlePath) || !existsSync(metadataPath)) throw new Error('Generated Vaadin reference assets are missing. Run npm ci --ignore-scripts and npm run build.');
...
const prohibited = ['@vaadin/grid-pro', '@vaadin/rich-text-editor', '@vaadin/charts', '@vaadin/spreadsheet'];
...
for (const marker of ['vaadin-usage-statistics-collect', 'tools.vaadin.com/usage-stats', 'Vaadin.Flow.clients']) {
