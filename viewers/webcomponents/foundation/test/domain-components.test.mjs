...
test('properties apply authored PDF options only when the standard PDF renderer wins', () => {
    const property = new CausewayPropertyElement();
    property.id = 'document';
    property.pdfRender = 'manual';
    property.pdfInitialPage = 3;
    property.pdfZoom = '125%';
    property.renderComponentState(readyState({
        descriptor: {id: 'document', value: {typeRef: {kind: 'OBJECT', name: 'Blob', ofType: null}}},
        data: {
            hidden: false,
            disabled: null,
            get: {name: 'guide.pdf', mimeType: 'application/pdf', bytes: '/guide.pdf'}
        }
    }));
    assert.equal(property.getAttribute('data-renderer'), 'pdf');
    assert.equal(property.pdfRender, 'manual');
    assert.equal(property.pdfInitialPage, 3);
    assert.equal(property.pdfZoom, 125);
    assert.match(property.innerHTML, /Preview document/);
    assert.match(property.innerHTML, /guide\.pdf/);

    property.pdfRender = 'link';
    property.renderComponentState(property.componentState ?? readyState({
        descriptor: {id: 'document'},
        data: {
            hidden: false,
            disabled: null,
            get: {name: 'guide.pdf', mimeType: 'application/pdf', bytes: '/guide.pdf'}
    ...
    assert.equal(property.getAttribute('data-renderer'), 'blob');
    assert.doesNotMatch(property.innerHTML, /data-causeway-pdf-reader/);

    property.pdfRender = 'auto';
    property.renderComponentState(readyState({
        descriptor: {id: 'document'},
        data: {hidden: false, disabled: null, get: {name: 'guide.txt', mimeType: 'text/plain', bytes: '/guide.txt'}}
    }));
    assert.equal(property.getAttribute('data-renderer'), 'blob');
    assert.doesNotMatch(property.innerHTML, /data-causeway-pdf-reader/);
});

test('invalid authored PDF options emit bounded diagnostics only for a qualifying standard PDF', () => {
    const property = new CausewayPropertyElement();
    property.id = 'document';
    property.setAttribute('pdf-render', 'surprise');
    property.setAttribute('pdf-initial-page', '0');
    property.setAttribute('pdf-zoom', '900%');
    const diagnostics = [];
    property.addEventListener('causeway-pdf-reader-diagnostic', event => diagnostics.push(event.detail));
    ...
    assert.equal(property.getAttribute('data-renderer'), 'pdf');
    assert.equal(property.pdfRender, 'auto');
    assert.equal(property.pdfInitialPage, 1);
    assert.equal(property.pdfZoom, 'page-width');
    assert.equal(diagnostics.length, 3);
    assert.ok(diagnostics.every(detail => detail.member === 'document' && detail.status === 'invalid-configuration'));
