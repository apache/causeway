...
const bundlePath = resolve(assets, 'vaadin-menubar.js');
...
await build({
  entryPoints: [resolve(directory, 'menubar-entry.mjs')],
  outfile: bundlePath,
  bundle: true,
  format: 'esm',
  platform: 'browser',
...
if (updatePolicy) {
  ...
  throw new Error(`Generated Menu Bar bundle checksum ${bundleSha256} does not match policy ${policy.bundleSha256}. Review the closure and run npm run update-policy deliberately.`);
}
const metadata = {
  bundle: {file: 'vaadin-menubar.js', rawBytes: bundle.length, gzipBytes: bundleGzipBytes, sha256: bundleSha256},
  entryPoints: ['@vaadin/menu-bar'],
