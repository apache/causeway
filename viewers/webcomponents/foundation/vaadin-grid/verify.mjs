...
const bundlePath = resolve(directory, 'generated/assets/vaadin-grid.js');
...
if (!existsSync(bundlePath) || !existsSync(metadataPath)) throw new Error('Generated Vaadin Grid assets are missing. Run npm ci --ignore-scripts and npm run build.');
...
for (const hash of policy.cspStyleHashes) {
  if (!controllerSource.includes(`\"${hash}\"`)) throw new Error(`HTMX CSP source omits the Grid policy hash ${hash}.`);
}
...
if (JSON.stringify(metadata.entryPoints) !== JSON.stringify(policy.entryPoints)) {
  throw new Error(`Unexpected Grid entry points: ${JSON.stringify(metadata.entryPoints)}.`);
}
const prohibited = [
  '@vaadin/grid-pro', '@vaadin/menu-bar', '@vaadin/rich-text-editor',
...
for (const marker of [
  ...
  'vaadin-grid-pro', 'vaadin-menu-bar', 'BinderNode', 'window.Vaadin.Flow'
]) {
