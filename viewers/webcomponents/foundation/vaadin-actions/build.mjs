...
const bundlePath = resolve(assets, 'vaadin-actions.js');
...
await build({
  entryPoints: [resolve(directory, 'action-entry.mjs')],
  outfile: bundlePath,
  bundle: true,
  format: 'esm',
  platform: 'browser',
...
if (updatePolicy) {
  ...
  throw new Error(`Generated action bundle checksum ${bundleSha256} does not match policy ${policy.bundleSha256}. Review the closure and run npm run update-policy deliberately.`);
}
const metadata = {
  bundle: {file: 'vaadin-actions.js', rawBytes: bundle.length, gzipBytes: bundleGzipBytes, sha256: bundleSha256},
  entryPoints: ['@vaadin/button'],
