...
const bundlePath = resolve(directory, 'generated/assets/vaadin-actions.js');
...
if (!existsSync(bundlePath) || !existsSync(metadataPath)) throw new Error('Generated Vaadin action assets are missing. Run npm ci --ignore-scripts and npm run build.');
...
for (const hash of policy.cspStyleHashes) {
  if (!controllerSource.includes(`\"${hash}\"`)) throw new Error(`HTMX CSP source omits the action policy hash ${hash}.`);
}
...
if (JSON.stringify(metadata.entryPoints) !== JSON.stringify(['@vaadin/button'])) {
  throw new Error(`Unexpected action entry points: ${JSON.stringify(metadata.entryPoints)}.`);
}
const prohibited = [
  '@vaadin/grid', '@vaadin/grid-pro', '@vaadin/menu-bar', '@vaadin/rich-text-editor',
...
for (const marker of [
  ...
  'vaadin-grid', 'vaadin-menu-bar', 'BinderNode', 'window.Vaadin.Flow'
]) {
