...
The auditing service works very well with implementations of xref:refguide:applib-svc:ExecutionSubscriber.adoc[`PublisherService`] that persist the ``Interaction.Execution`` objects obtained from the xref:refguide:applib-svc:InteractionContext.adoc[`InteractionContext`] service.
