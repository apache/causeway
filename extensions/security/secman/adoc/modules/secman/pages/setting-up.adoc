...
It is also possible to use _SecMan_ in conjunction to use an alternative implementaiton of `Authenticator`, using either:
...
<.> email address
<.> application tenancy
<.> local account (rather than delegated, see xref:setting-up-with-spring-oauth2.adoc[] or xref:setting-up-with-keycloak.adoc[]).
