...
class ReplayPendingBackgroundCommands {
    ...
    static final String WAIT_MESSAGE =
            "Please wait until pending background commands have executed and committed before continuing replay.";
    ...
    boolean hasPendingBackgroundCommands(final ReplayContext replayContext) {
        return Optional.ofNullable(replayContext)
                ...
                .map(repository -> repository.findBackgroundAndNotYetStarted())
                .map(commands -> !commands.isEmpty())
                .orElse(false);
    }
}
