...
@ActionLayout(
        ...
        describedAs = "Marks selected commands as excluded from export or replay")
@RequiredArgsConstructor
public class CommandManager_excludeCommands {

    public static class DomainEvent
            extends CommandManager.ActionDomainEvent<CommandManager_excludeCommands> { }
    ...
    public CommandManager act(final List<ReplayableCommand> selected) {
        if (!commandManager.replayContext().isRecordingSupportEnabled()) {
            throw new RecoverableException(recordingRequired());
        }
        final var resolved = resolve(selected);
        if (!resolved.isValid()) {
            throw new RecoverableException(resolved.failure());
        }
        resolved.entries().forEach(entry -> entry.setReplayState(ReplayState.EXCLUDED));
    ...
    public String disableAct() {
        ...
        return choicesSelected().isEmpty() ? "No commands in sequence" : null;
    }
    ...
    private CommandManagerWorkflowSupport.Selection resolve(final List<ReplayableCommand> selected) {
        return CommandManagerWorkflowSupport.resolveSelection(
                selected,
                choicesSelected(),
                "Select at least one command to exclude",
