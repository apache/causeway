...
The _Command Log_ module provides an implementation of xref:refguide:applib:index/services/publishing/spi/CommandSubscriber.adoc[] SPI that persists xref:refguide:applib:index/services/command/Command.adoc[Command]s using the xref:pjpa:ROOT:about.adoc[JPA/EclipseLink] object store.
...
In the webapp module of your application, add the following dependency for the xref:pjpa::about.adoc[] object store:
...
<.> enables command recording and replay support; it is `DISABLED` by default.
+
See xref:refguide:config:sections/causeway.extensions.adoc#causeway.extensions.command-log.recording-support[causeway.extensions.command-log.recording-support] configuration property for more details.

=== Persistent Replay-result Mappings

Command replay can observe that an object recorded with one bookmark on the source system was created with another bookmark locally.
By default these replay-result mappings are held in memory for the lifetime of the application instance.
To retain mappings across restarts, select persistent storage:
...
* `TARGET` rows represent recorded target bookmarks;
* `PARAMETER` rows represent reference-valued action parameters and include the parameter name (scalar parameters are omitted); and
* `RESULT` represents the recorded result bookmark, when present.
...
They omit resultless safe actions without deleting or changing the underlying command-log entries.
The pending-or-failed replay collection is intentionally exempt so that all imported replay work remains reviewable.
The safe Previous and Next actions navigate foreground commands in command-log order, skipping entries that are ineligible under this general rule.

The `CommandManager` view model is the forward sequence-management surface for export, import, exclusion, movement, restoration, deletion, and replay workflows.
The older `CommandExportManager` and `CommandReplayManager` view models remain available as compatibility surfaces.

=== Recording and Replay with Background Commands

Recording and replay use a global, conservative definition of pending background work: any persisted command with `ExecuteIn.BACKGROUND` and no `startedAt` timestamp is pending.
This reuses the existing command-log repository query; it is not limited by user, tenant, parent interaction, or replay-manager baseline.

When `recording-support` is `ENABLED`, the foreground command that schedules background work is allowed to complete.
While that work remains pending, an attempt to create a later recorded foreground command fails with a message instructing the user to wait until pending background commands have executed and committed.
The command can be retried after the background job commits.
The gate does not apply when recording support is disabled or command recording is paused.

Replay entry points apply the same wait condition, including an individual `ReplayableCommand`, the unified manager, and the retained legacy replay manager.
The unified manager contributes `replayOrRetryNext` and `replayOrRetryMultiple` to its `pendingOrFailed` collection.
Replay-next acts on the oldest eligible command.
Replay-multiple supports bounds of 5, 10, 20, 40, 80, 160, 320, or all, and defaults to 10.
It stops after the first replay failure or as soon as a replay schedules new pending background work, preserving completed replay states.
After the background work executes and commits, invoke replay again to continue with the remaining pending or failed commands.
The replay controls never poll for, wait on, or execute background work themselves.

This Causeway 4 behavior uses the existing JPA commandlog adapter and named query, and requires no datastore migration or additional configuration.
No commandlog JDO adapter is provided for Causeway 4.
...
public void exportInvoices(Supplier supplier) {
    backgroundService.execute(supplier)
        .acceptAsync(Supplier::exportInvoices);
}
----
...
@Action
public void exportInvoices(Supplier supplier) {
    backgroundService.executeMixin(Supplier_exportInvoices.class, supplier)
        .acceptAsync(Supplier_exportInvoices::act);
}
----
