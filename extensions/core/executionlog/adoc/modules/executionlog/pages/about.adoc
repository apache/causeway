...
+
This can be explicit positioned through the domain class' own xref:userguide:ROOT:ui.adoc#object-layout[layout file], but this is generally not necessary: it will slot into the tab group in the layout file indicated for unreferenced collections using `<tabGroup unreferencedCollections="true">`.
