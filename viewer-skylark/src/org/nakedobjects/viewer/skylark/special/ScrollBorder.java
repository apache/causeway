...
//		canvas.drawRectangle(0, 0, width - 1, height - 1, Style.SECONDARY2);
		canvas.drawLine(0, height - SCROLLBAR_WIDTH, width, height - SCROLLBAR_WIDTH, Style.SECONDARY2);
		canvas.drawLine(width - SCROLLBAR_WIDTH, 0, width - SCROLLBAR_WIDTH, height, Style.SECONDARY2);
	...
	public ViewAreaType viewAreaType(Location mouseLocation) {
		...
		if(x >= getSize().getHeight() - SCROLLBAR_WIDTH || y >= getSize().getWidth() - SCROLLBAR_WIDTH ) {
	...
	public void dragTo(InternalDrag drag) {
		...
		resize = new Size(x, y);
		Logger.getLogger(getClass()).debug("resizing sroll border " + resize);
		invalidateLayout();
		//super.dragTo(drag);
	}
	
	public Size getRequiredSize() {
		return resize == null ? super.getRequiredSize() : new Size(resize);
	}
}
