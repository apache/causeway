...
public class CausewayVegas implements HasAsciiDocDescription {
    ...
    @ObjectSupport public String title() {
        return "Vega data type";
    }

    @Action(semantics = SemanticsOf.SAFE)
    @ActionLayout(promptStyle = PromptStyle.DIALOG_MODAL)
    public CausewayVegaVm openViewModel(final Vega initialValue) {
        return new CausewayVegaVm(initialValue);
    }
    @MemberSupport public Vega default0OpenViewModel() {
    ...
    @Inject
    @XmlTransient
    ValueHolderRepository<Vega, ? extends CausewayVegaEntity> entities;

    @Inject
    @XmlTransient
    Samples<Vega> samples;
