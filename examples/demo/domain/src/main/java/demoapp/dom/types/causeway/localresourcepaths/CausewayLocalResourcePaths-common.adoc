...
A number of mixin actions contribute through the `CausewayLocalResourcePathHolder` interface, demonstrate the use of `LocalResourcePath` as a parameter and as a return type.
For example:
...
To demonstrate support for label positions using `@PropertyLayout(labelPosition=...)`, the entity and view model objects both implement the extended `CausewayLocalResourcePathHolder2` interface:
