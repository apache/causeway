...
A number of mixin actions contribute through the `CausewayCalendarEventHolder` interface, demonstrate the use of `CalendarEvent` as a parameter and as a return type.
For example:
