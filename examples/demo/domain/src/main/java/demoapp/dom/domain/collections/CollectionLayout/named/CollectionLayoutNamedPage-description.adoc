...
This inferred name can be however be overridden using either the link:https://causeway.apache.org/refguide/2.0.0-RC1/applib/index/annotation/CollectionLayout.html#named[@CollectionLayout#named] attribute, or using the link:https://causeway.apache.org/userguide/2.0.0-RC1/fun/ui.html#by-example[.layout.xml] layout file.

This is useful for example if the desired name is a Java keyword (such as "package").
...
* the `children` property has its name specified using the `@CollectionLayout#named()` annotation:
+
...
* meanwhile, the `moreChildren` property has its name specified using the `Xxx.layout.xml` file:
+
