The framework has built-in support for all of the Wrapper data types, including `AsciiDoc`.
...
These objects both implement a common interface, `IsisAsciiDocHolder`:
...
There is also an extension of this interface, `IsisAsciiDocHolder2`, with default methods demonstrating the use of label positions:
...
A number of mixin actions contribute through this interface, demonstrate the use of `AsciiDoc` as a parameter and as a return type.
For example:
