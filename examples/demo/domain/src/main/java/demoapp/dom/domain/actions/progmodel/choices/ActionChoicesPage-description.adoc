...
The list of choices is expected to be small, and should (if possible) only proffer candidates that would also pass any validation rules (otherwise it would make for a frustrating user experience).
...
** The preceding N-1 parameters are supplied to the Nth "choices" supporting method.
