...
The link:https://causeway.apache.org/refguide/2.0.0-RC1/applib/index/value/semantics/ValueSemanticsProvider.html[ValueSemanticsProvider] SPI provides a mechanism to customise the way in which the framework interacts with value types, both simple scalar value types (consisting of a single value) and composite value types (multiple values).
...
* the page's property is anontated with the `@Percentage` meta-annotation:
+
[source,java,indent=0]
----
