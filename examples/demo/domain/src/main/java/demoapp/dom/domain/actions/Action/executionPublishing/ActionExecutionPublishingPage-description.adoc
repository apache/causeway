...
The link:https://causeway.apache.org/userguide/2.0.0-RC1/executionoutbox/about.html[executionOutbox] extension (not part of this demo) is very similar, also persisting executions.
...
* the `changeNameWrapped` action has execution publishing disabled, but it uses the link:https://causeway.apache.org/refguide/2.0.0-RC1/applib/index/services/wrapper/WrapperFactory.html[WrapperFactory] to invoke the "changeNamePublished" action (above), which does.
This results in an executions being published, with a corresponding link:https://causeway.apache.org/refguide/2.0.0-RC1/extensions/index/executionlog/applib/dom/ExecutionLogEntry.html[ExecutionLogEntry].
+
WARNING: no such execution log entry is currently created; tracking with link:https://issues.apache.org/jira/browse/CAUSEWAY-3476[CAUSEWAY-3476].
