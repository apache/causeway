...
A number of mixin actions contribute through the `JavaSqlTimestampHolder` interface, demonstrate the use of `java.sql.Timestamp` as a parameter and as a return type.
For example:
...
To demonstrate support for label positions using `@PropertyLayout(labelPosition=...)`, the entity and view model objects both implement the extended `JavaSqlTimestampHolder2` interface:
...
To demonstrate support for `@PropertyLayout(renderDay=...)` -- to simplify the modelling "[inclusive, exclusive)" date ranges -- the entity and view model objects both implement the extended `JavaSqlTimestampHolder3` interface.
All methods have a default implementation:

[source,java]
----
include::holder/JavaSqlTimestampHolder3.java[tags=class]
----
