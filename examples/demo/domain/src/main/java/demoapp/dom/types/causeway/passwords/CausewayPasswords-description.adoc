...
The framework has built-in support for strings that are automatically masked as passwords, using the `Password` data type.
...
On the left hand side is a collection of entities that uses the `Password` datatype.
There is also an action that opens a view model; this view model also uses the `Password` datatype.
