...
A number of mixin actions contribute through the `JavaUtilDateHolder` interface, demonstrate the use of `java.util.Date` as a parameter and as a return type.
For example:
...
To demonstrate support for label positions using `@PropertyLayout(labelPosition=...)`, the entity and view model objects both implement the extended `JavaUtilDateHolder2` interface:
...
To demonstrate support for `@PropertyLayout(renderDay=...)` -- to simplify the modelling "[inclusive, exclusive)" date ranges -- the entity and view model objects both implement the extended `JavaUtilDateHolder3` interface.
All methods have a default implementation:

[source,java]
----
include::holder/JavaUtilDateHolder3.java[tags=class]
----
