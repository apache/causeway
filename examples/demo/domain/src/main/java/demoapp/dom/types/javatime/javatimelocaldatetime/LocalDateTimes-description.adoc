...
* The `LocalDateTimeHolder` interface is used to contribute a number of mixins to both types:
+
[source,java]
----
include::holder/LocalDateTimeHolder.java[tags=class]
----

* The `LocalDateTimeHolder2` interface is used to demonstrate support for label positions using `@PropertyLayout(labelPosition=...)`.
