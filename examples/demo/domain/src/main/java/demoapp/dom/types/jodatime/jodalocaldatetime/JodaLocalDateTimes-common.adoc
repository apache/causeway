...
A number of mixin actions contribute through the `JodaLocalDateTimeHolder` interface, demonstrate the use of `LocalDateTime` as a parameter and as a return type.
For example:
