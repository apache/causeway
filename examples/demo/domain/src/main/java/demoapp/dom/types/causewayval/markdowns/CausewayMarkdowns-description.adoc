...
The framework has built-in support for HTML markup, written in Markdown, using the `Markdown` data type.
...
On the left hand side is a collection of entities that uses the `Markdown` datatype.
There is also an action that opens a view model; this view model also uses the `Markdown` datatype.
