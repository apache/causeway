...
Every _intention_ to edit a propertyis reified into a link:https://causeway.apache.org/refguide/2.0.0-SNAPSHOT/applib/index/services/command/Command.html[Command] which captures the target and the proposed property value, as well as the "who" and the "when" (identity of the user performing the action and a timestamp).
