...
The `renderDay` attribute applies to date fields, and indicates that the date rendered should be one day before the day actually held in the property.
...
The attribute is supported only for properties and parameters representing dates/times.
...
For properties (not parameters), the `renderDay` attribute can be specified using the `Xxx.layout.xml` file:
