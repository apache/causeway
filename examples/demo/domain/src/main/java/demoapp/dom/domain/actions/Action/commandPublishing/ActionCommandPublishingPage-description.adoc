...
Every action invocation is reified into a link:https://causeway.apache.org/refguide/2.0.0-SNAPSHOT/applib/index/services/command/Command.html[Command] which captures the target and action arguments, as well as the "who" and the "when" (identity of the user performing the action and a timestamp).

The link:https://causeway.apache.org/refguide/2.0.0-SNAPSHOT/applib/index/annotation/Action.html#commandpublishing[@Action#commandPublishing] determines whether subscribers are notified of the action being executed.
...
Invoking the published action will result in a command representing the action invocation from being persisted; these are listed in the collection at the bottom of the page.
