Normally the name of a property _and_ also a parameter is inferred from its name in code.
This inferred name can be however be overridden using various mechanisms with an explicit value.
This is useful for example if the desired name is a Java keyword (such as "default", or "package") and therefore cannot be used as a variable name.
...
The name can be specified using explicitly using an annotation.
...
----
<.> explicitly specifies name of the property
...
----
<.> explicitly specifies name of the parameter
...
----
<.> `named` attribute is _not_ present.
