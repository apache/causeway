The "multiLine" facet is a hint to indicates that a multiline text field (a textbox) should be used to render a property or parameter:
...
The facet is currently supported only for ``String``s.

== Annotation

The multiline facet can be specified explicitly using an annotation:
...
For properties (not parameters), the multiline facet can be specified using the `Xxx.layout.xml` file:
...
----
<.> `multiLine` attribute is _not_ present.
...
The multiline facet can also be specified using a custom meta-annotation:

* the meta-annotation is defined as:
+
[source,java]
----
include::MultiLineMetaAnnotation.java[tags=class]
----
...
----
<.> property's multiLine facet is inherited from the meta-annotation
