...
=== JavaTimeLocalDateHolder

The `JavaTimeLocalDateHolder` interface is used to contribute a number of mixins to both types:
...
The `JavaTimeLocalDateHolder2` interface is used to demonstrate support for label positions using `@PropertyLayout(labelPosition=...)`.
