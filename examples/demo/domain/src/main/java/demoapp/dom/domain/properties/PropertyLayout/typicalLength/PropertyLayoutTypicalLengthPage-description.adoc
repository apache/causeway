...
* for parameters, the attribute can be specified using either the `@ParameterLayout` annotation.
...
For properties (not parameters), the `typicalLength` attribute can be specified using the `Xxx.layout.xml` file:
...
The `typicalLength` attribute can also be specified using a custom meta-annotation:
