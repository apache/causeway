...
The `publishing` element specifies that the editing of an object property should be published to registered implementation(s) of link:https://isis.apache.org/refguide/2.0.0-M3/applib-svc/PublisherService.html#spi[`PublisherService` SPI].
...
The `publisher` element can be specified explicitly using an annotation through `@Property#publishing()`:
