...
=== JavaTimeOffsetTimeHolder

The `JavaTimeOffsetTimeHolder` interface is used to contribute a number of mixins to both types:
...
The `JavaTimeOffsetTimeHolder2` interface is used to demonstrate support for label positions using `@PropertyLayout(labelPosition=...)`.
