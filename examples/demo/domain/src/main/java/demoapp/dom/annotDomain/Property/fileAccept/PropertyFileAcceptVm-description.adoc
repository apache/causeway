...
The `fileAccept` element specifies the file extension allowed when browsing for a file to upload:
...
The `fileAccept` element can be specified explicitly using an annotation (on a `Blob` property):
