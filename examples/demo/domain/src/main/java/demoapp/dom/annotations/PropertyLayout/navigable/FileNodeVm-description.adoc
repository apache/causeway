...
But first, let's discuss the `navigable` hint as a means to construct breadcrumbs.
...
The framework constructs the breadcrumbs using the value returned by the property with the `navigable` hint.
