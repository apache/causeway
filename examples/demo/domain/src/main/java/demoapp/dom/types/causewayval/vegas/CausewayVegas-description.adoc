...
The framework has built-in support for a visualization grammar (to render interactive graphs, maps and other designs), using the `Vega` data type.
These can be used either in entities or view models.

NOTE: This is an Apache Causeway specific data type.

WARNING: vega types are currently not being rendered correctly as graphics; being tracked under https://issues.apache.org/jira/browse/CAUSEWAY-3488[CAUSEWAY-3488].

== How this demo works

On the left hand side is a collection of entities that uses the `Vega` datatype.
There is also an action that opens a view model; this view model also uses the `Vega` datatype.
