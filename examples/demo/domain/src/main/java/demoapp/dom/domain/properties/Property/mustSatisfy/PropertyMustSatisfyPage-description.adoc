...
The `mustSatisfy` attribute specifies an arbitrary constraint that the value entered into the property must satisfy:
...
The `mustSatisfy` attribute can also be specified using a custom meta-annotation:
