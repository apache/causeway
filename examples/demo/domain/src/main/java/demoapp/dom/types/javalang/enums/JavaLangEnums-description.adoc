...
The framework has built-in support for  the `DemoEnum` data type.
These can be used either in entities or view models.

== How this demo works

On the left hand side is a collection of entities that uses the `DemoEnum` datatype.
There is also an action that opens a view model; this view model also uses the `DemoEnum` datatype.
