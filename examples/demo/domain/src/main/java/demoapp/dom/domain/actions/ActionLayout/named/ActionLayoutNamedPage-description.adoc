...
The link:https://causeway.apache.org/refguide/2.0.0-SNAPSHOT/applib/index/annotation/ActionLayout.html#named[@ActionLayout#named] attribute allows you to override the name (as used for rendering) from that derived from the action's method name).

This can also be used if you want the name to include characters such as brackets `()[]` or quotation marks `' "`.
...
Instead of using `@ActionLayout#named`, it is also possible to specify a name in the link:https://causeway.apache.org/userguide/2.0.0-RC1/fun/ui.html#by-example[.layout.xml] file:
