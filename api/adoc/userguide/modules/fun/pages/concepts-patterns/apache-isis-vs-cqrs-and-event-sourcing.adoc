...
To synchronise the write- and read- model, the events emitted by xref:refguide:applib-svc:ExecutionSubscriber.adoc[`PublisherService`] could be used.
...
Also, in Apache Isis every business action (and indeed, property and collection) emits domain events through the xref:refguide:applib-svc:EventBusService.adoc[`EventBusService`], and can optionally also be published through the xref:refguide:applib-svc:ExecutionSubscriber.adoc[`PublisherService`].
