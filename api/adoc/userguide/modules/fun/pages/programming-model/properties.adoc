...
A property is an instance variable of a domain object, of a scalar type, that holds some state in either a xref:userguide:fun:programming-model.adoc#domain-entities[domain entity] or a xref:userguide:fun:programming-model.adoc#view-models[view model].
...
It also provides a xref:refguide:applib-ant:PropertyLayout.adoc[`@PropertyLayout`] for UI hints (though the information in this annotation may instead be provided by a supplementary xref:vw:ROOT:layout.adoc[`.layout.xml`] file)
...
It's ok for a xref:userguide:fun:programming-model.adoc#view-models[view model] to reference other view models and also to reference domain entities.
...
It's also possible to have non-persisted but still editable properties.
The setter would not write to a field, but would mutate the object in some other way.
In such a case you will need a getter and a setter, but with the field/property annotated for the ORM as non-persistent.
