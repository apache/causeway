...
It provides an implementation of the xref:refguide:applib-svc:ExecutionSubscriber.adoc[`PublisherService`], but which simply publishes instances of xref:refguide:schema:ixn.adoc[`InteractionDto`] to an ActiveMQ queue.
