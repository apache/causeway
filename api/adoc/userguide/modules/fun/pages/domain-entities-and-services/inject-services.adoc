...
For example, the framework defines the xref:refguide:applib-svc:ExecutionSubscriber.adoc[`PublisherService`] SPI, which is used to publish xref:refguide:schema:ixn.adoc[Interaction Execution]s to external systems.
