...
Apache Isis defines its own set of annotations, and recognises some annotations from the ORM layer (eg JDO/DataNucleus) and infers domain semantics from these annotations.
...
xref:userguide:fun:programming-model.adoc#domain-entities[here].
====
...
[TIP]
====
Details of how to consume such DTOs, eg within an ESB such as link:http://camel.apache.org[Apache Camel(TM)], can be found xref:userguide:fun:programming-model.adoc#dto-consumers[here].
====
...
There's a lot more discussion on this topic in xref:building-blocks.adoc#modules[modules], below.
