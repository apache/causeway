...
This is then executed either at the end of the transaction, or if a  xref:userguide:fun:crud/finding.adoc[query is run], or if the internal queue is manually flushed using xref:refguide:applib-svc:TransactionService.adoc[`TransactionService`]'s `flush()` method.
