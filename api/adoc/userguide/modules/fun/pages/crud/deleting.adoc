...
This is then executed either at the end of the transaction, or if a  xref:userguide:fun:crud.adoc#finding[query is run], or if the internal queue is manually flushed using xref:refguide:applib-svc:TransactionService.adoc[`TransactionService`]'s `flush()` method.
