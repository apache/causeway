...
To support the business domain being split into separate modules, the framework provides various features, the most important of which are the dependency injection of xref:building-blocks.adoc#domain-services[domain services], xref:building-blocks.adoc#mixins[mixins], and in-memory xref:building-blocks.adoc#events[events].
