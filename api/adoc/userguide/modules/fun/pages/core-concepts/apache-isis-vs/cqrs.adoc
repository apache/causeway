...
Or, the CQRS architecture can be more fully implemented with Apache Isis by introducing a separate read model, synchronized using the xref:refguide:applib-svc:PublisherService.adoc[`PublisherService`], or using subscribers on the xref:refguide:applib-svc:EventBusService.adoc[`EventBusService`].
