...
* if running in prototype mode xref:refguide:config:about.adoc#deployment-types[deployment type] or during integration tests, then the service runs in *write* mode, in which case it records all translations into the `.pot` file.
The `.pot` file is written out when the system is shutdown.
* if running in server (production) mode axref:refguide:config:about.adoc#deployment-types[deployment type], then the service runs in *read* mode.
It is also possible to set a configuration setting in `application.properties` to force read mode even if running in prototype mode (useful to manually test/demo the translations).
...
For example (based on similar code in the xref:docs:starters:simpleapp.adoc[SimpleApp] starter app):
...
----
<.> concatenated together to form the context for the `.pot` file.

=== `TranslatableException`

Another mechanism by which messages can be rendered to the user are as the result of exception messages thrown and recognized by an xref:refguide:applib-svc:ExceptionRecognizerService.adoc[`ExceptionRecognizer`].
...
These are simply passed through to the xref:refguide:applib-svc:ExceptionRecognizerService.adoc[`ExceptionRecognizerService`]s so that any messages are recorded as requiring translation.
====
...
For this to work, the `namedEscaped` attribute must be specified using either the xref:userguide:fun:ui.adoc#object-layout[layout file], or using an annotation such as xref:refguide:applib-ant:PropertyLayout.adoc[`@PropertyLayout`] or xref:refguide:applib-ant:ParameterLayout.adoc[`@ParameterLayout`].
...
To configure the `TranslationServicePo` to write to out the `translations.pot` file, add the following to the _integtests_ `log4j-test.xml` file:
...
WARNING: TODO - v2 - need to verify this.
...
WARNING: TODO - v2 - need to figure out how to configure Spring Boot so that these resources can be read from an external location.
...
To force read mode (ie use translations) even if in prototype mode, add the xref:refguide:config:sections/isis.core.runtime-services.adoc#isis.core.runtime-services.translation.po.mode[`isis.core.runtime-services.translation.po.mode`] configuration property:

[source,ini]
.application.properties
----
isis.core.runtime-services.translation.po.mode=read
----

It's also possible to disable the service completely.
This can sometimes be useful in integration tests.

[source,ini]
.application.properties
----
isis.core.runtime-services.translation.po.mode=disabled
----
...
The xref:refguide:applib-svc:LocaleProvider.adoc[`LocaleProvider`] API is used by the `TranslationServicePo` implementation to obtain the locale of the "current user".

An implementation is provided by the Wicket viewer.

[CAUTION]
====
There is no equivalent implementation of `LocaleProvider` for Restful Objects viewer; requests through Restful Objects are not translated.
====
