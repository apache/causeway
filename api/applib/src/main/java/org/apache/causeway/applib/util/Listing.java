...
public class Listing<T> {
	...
	final @NonNull LineAdapter<T> lineAdapter;
	final @NonNull Can<Line> lines;
	...
	@RequiredArgsConstructor @Getter @Accessors(fluent = true)
    private static class SimpleLineAdapter<T> implements LineAdapter<T> {
    	
        final @NonNull Class<T> objectType;
        final @NonNull Function<T, String> stringifier;
        final @NonNull Function<String, T> destringifier;
        final @NonNull Function<T, ?> keyExtractor;
     ...
     */
    @RequiredArgsConstructor @Getter @Accessors(fluent = true)
    public static class LineEnabled<T> implements MappedLine<T> {
    	
        final @NonNull T object;
        final @NonNull String objectStringified;
     ...
     */
    @RequiredArgsConstructor @Getter @Accessors(fluent = true)
    public static class LineDisabled<T> implements MappedLine<T> {
     ...
     */
    @RequiredArgsConstructor @Getter @Accessors(fluent = true)
    public static class LineRemoved<T> implements MappedLine<T> {
    	final @NonNull T object;
    	final @NonNull String objectStringified;
     ...
     */
    @RequiredArgsConstructor @Getter @Accessors(fluent = true)
    public static class LineComment implements Line {
    ...
    public <K> Listing<T> merge(@NonNull final MergePolicy policy,
        ...
        .forEach((final Line line)->{
            ...
            var key = (line instanceof MappedLine)
                    ? lineAdapter().extractKey((T) ((MappedLine<T>)line).object())
                    : null;
            final boolean incomingContainsKey = key!=null
                    ? incomingByKey.remove(key)!=null
                    : false;

            if(line instanceof LineComment) {
                mergedLines.add(((LineComment)line)); // identity operation
            } else if(line instanceof LineEnabled) {
            	var lineEnabled = (LineEnabled<?>)line;
            ...
            } else if(line instanceof LineDisabled) {
            	var lineDisabled = (LineDisabled<?>)line;
                if(incomingContainsKey) {
                    mergedLines.add(lineDisabled); // identity operation
                } else {
                    mergedLines.add(lineDisabled.toRemoved()); // mark REMOVED
                }
            } else if(line instanceof LineRemoved) {
            	var lineRemoved = (LineRemoved<?>)line;
                if(incomingContainsKey) {
