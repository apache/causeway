...
The domain services can be categories as API or SPI, and relating to a particular architectural layers:
...
* Also at the end of each transaction, details of all changed properties are passed to any registered xref:refguide:applib-svc:AuditerService.adoc[`AuditerService`](s) by way of the (internal) xref:core:runtime-services:AuditingServiceInternal.adoc[`AuditingServiceInternal`] domain service.
