...
[TIP]
====
As an alternative, consider using DataNucleus' link:http://www.datanucleus.org/products/accessplatform_4_0/jdo/jdoql_typesafe.html[type-safe JDO query API], discussed xref:refguide:applib-svc:.IsisJdoSupport.adoc#type-safe-query-api[below].
====
...
When writing xref:testing:integtestsupport:about.adoc[integration tests] you'll usually need to tear down some/all mutable transactional data before each test.  One way to do that is to use the `executeUpdate(...)` method described xref:refguide:applib-svc:.IsisJdoSupport.adoc#executing-sql[above].
