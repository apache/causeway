...
[source,java]
----
include::refguide:applib-svc:example$services/queryresultscache/QueryResultsCache.java[tags="refguide"]
----
<.> Execute the callable if not already cached for the supplied calling class, method and keys.

There are also overloaded generics versions of  `execute(...)` that simplify the calling of this domain service.
