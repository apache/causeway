...
As noted elsewhere, the default implementation of this service uses xref:refguide:applib-svc:.EmailService.adoc[`EmailService`].  This service has no specific configuration properties but does require that the `EmailService` has been configured.
