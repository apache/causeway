...
Document `WrappingObject`, document `AsyncWrap`
...
----
<.> enumerates how the wrapper returned by `wrap(...)` interacts with the underlying domain object.

<.> wraps the underlying domain object with a wrapper as specified by the execution mode.
+
...
<.> wraps the underlying domain object with a wrapper that will validate the business rules, and just return `null` if any fail.
