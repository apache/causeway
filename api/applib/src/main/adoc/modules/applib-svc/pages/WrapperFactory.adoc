...
Similarly if an action has a xref:refguide:applib-cm:methods/prefixes/validate.adoc[`validateXxx()`] method and the supplied arguments are invalid then an `InvalidException` will be thrown.
...
In addition, any xref:refguide:applib-cm:methods.adoc#default[`default...()`], xref:refguide:applib-cm:methods.adoc#choices[`choices...()`] or xref:refguide:applib-cm:methods.adoc#autoComplete[`autoComplete...()`] methods can be called.
