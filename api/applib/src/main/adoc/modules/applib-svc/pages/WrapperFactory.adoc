...
+
The `SyncControl` modifies the way in which the wrapper interacts with the underlying domain object, discussed xref:#synccontrol-and-asynccontrol[below].
...
Again `SyncControl` modifies the way in which the wrapper interacts with the underlying domain object, discussed xref:#synccontrol-and-asynccontrol[below]..
...
=== `SyncControl` and `AsyncControl`

The `SyncControl` and `AsyncControl` classes control the way that the wrapper works, for both synchronous calls using `wrap(...)`, and asynchronous calls using `asyncWrap`.
