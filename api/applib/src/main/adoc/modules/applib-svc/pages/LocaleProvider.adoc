...
This is notionally request-scoped, returning the `Locale` of the current user; _not_ that of the server.  (Note that the implementation is not required to actually be xref:refguide:applib-ant:IsisSessionScope.adoc[`@IsisSessionScope`], however).
