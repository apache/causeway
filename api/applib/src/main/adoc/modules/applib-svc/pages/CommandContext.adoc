...
<.> human-friendly description of the arguments with which the action was invoked
<.> formal (XML or similar) specification of the action to invoke/being invoked
...
----
<.>  date/time at which this action started (same as `timestamp` property for foreground commands)
<.> date/time at which this action completed.
<.> for actions created through the `BackgroundService`, captures the parent action
<.> exception stack trace, if action threw exception
<.> bookmark to object returned by action, if any
<.> the policy controlling whether this command should ultimately be persisted:
+
[source,java,indent=0]
----
...
The xref:refguide:applib-svc:CommandContext.adoc[`CommandContext`] service is very similar in nature to the xref:refguide:applib-svc:InteractionContext.adoc[`InteractionContext`], in that the xref:refguide:applib-svc:CommandContext.adoc#command[`Command`] object accessed through it is very similar to the xref:refguide:applib-svc:InteractionContext.adoc#interaction[`Interaction`] object obtained from the xref:refguide:applib-svc:InteractionContext.adoc[`InteractionContext`].
The principle distinction is that while xref:refguide:applib-svc:CommandContext.adoc#command[`Command`] represents the __intention__ to invoke an action or edit a property, the xref:refguide:applib-svc:InteractionContext.adoc#interaction[`Interaction`] (and contained ``Execution``s) represents the actual execution.
...
xref:refguide:applib-svc:CommandContext.adoc#command[`Command`]s
