...
[NOTE]
====
If the domain object defines its own xref:refguide:applib-cm:methods.adoc#iconName[`iconName()`] supporting method,
or if it has the
xref:refguide:applib-ant:DomainObjectLayout#cssClassFa.adoc[`@DomainObjectLayout#cssClassFa()`] attribute, then
