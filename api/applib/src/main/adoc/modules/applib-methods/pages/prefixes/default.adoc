...
The method is _also_ called for properties in the case when an object is newly instantiated using xref:refguide:applib-svc:RepositoryService.adoc[`RepositoryService#detachedEntity(...)`] or xref:refguide:applib-svc:FactoryService.adoc[`FactoryService#create(...)`].
...
The first is to use the xref:refguide:applib-methods:lifecycle.adoc#created[`created()`] callback, called by the framework when xref:refguide:applib-svc:RepositoryService.adoc[`RepositoryService#detachedEntity(...)`] or xref:refguide:applib-svc:FactoryService.adoc[`FactoryService#create(...)`] is called.
