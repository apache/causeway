...
The `@ViewModelLayout` annotation is identical to the xref:refguide:applib-ant:DomainObjectLayout.adoc[`@DomainObjectLayout`], but is provided for symmetry with domain objects that have been annotated using xref:refguide:applib-ant:ViewModel.adoc[`@ViewModel`] (rather than xref:refguide:applib-ant:DomainObject.adoc#nature[`@DomainObject(nature=VIEW_MODEL)`]).

[source,java]
----
include::refguide:applib-ant:example$annotation/ViewModelLayout.java[tags="refguide"]
----
