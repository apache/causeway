...
The `mustSatisfy` attribute allows arbitrary validation to be applied to parameters using an (implementation of a) `org.apache.isis.applib.spec.Specification` object.  The attribute is also supported on xref:refguide:applib-ant:Property.adoc#mustSatisfy[properties].
