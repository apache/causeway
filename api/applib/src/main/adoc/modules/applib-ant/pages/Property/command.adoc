...
//* xref:refguide:applib-svc:application-layer-spi/_BackgroundCommandService.adoc[`BackgroundCommandService`]


Each property edit is automatically reified by the xref:refguide:applib-svc:application-layer-api/CommandContext.adoc[`CommandContext`] service into a `Command` object, capturing details of the target object, the property, the proposed new value fo the property, the user, a timestamp and so on.
...
//Background execution means that the command is not executed immediately, but is available for a configured xref:refguide:applib-svc:application-layer-spi/_BackgroundCommandService.adoc[`BackgroundCommandService`] to execute, eg by way of an in-memory scheduler such as Quartz.
