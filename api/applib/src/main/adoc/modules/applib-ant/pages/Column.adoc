...
Isis also provides xref:refguide:applib-ant:Property.adoc#maxLength[`@Property#maxLength``] attribute.  If both are specified, Apache Isis will check when it initializes for any contradictions, and will fail-fast with an appropriate error message in the log if there are.
