...
In fact, it actually defines a call-graph of such executions for those cases where the xref:system:generated:index/applib/services/wrapper/WrapperFactory.adoc[WrapperFactory] is used to execute sub-actions/property edits.
...
<10> if any sub-actions or sub-edits were performed via the xref:system:generated:index/applib/services/wrapper/WrapperFactory.adoc[WrapperFactory], then these are captured in the `childExecutions` element.
