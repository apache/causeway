...
<2> uses complex types defined in the xref:refguide:schema:common.adoc["common" schema] and also the xref:refguide:schema:cmd.adoc["cmd" schema]
...
The transaction Id is used to correlate back to the xref:refguide:schema:cmd.adoc[command] that represented the intention to perform this execution, as well as to any xref:refguide:schema:chg.adoc[changes] to domain objects that occur as a side-effect of the interaction.
