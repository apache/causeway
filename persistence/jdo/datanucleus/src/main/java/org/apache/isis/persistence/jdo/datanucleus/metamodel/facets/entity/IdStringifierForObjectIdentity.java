...
public class IdStringifierForObjectIdentity extends IdStringifier.Abstract<ObjectIdentity> {
    ...
    @Override
    public ObjectIdentity parse(String stringified, Class<?> owningEntityType) {
        if (idStringifierForUuid.recognizes(stringified)) {
            UUID uuid = idStringifierForUuid.parse(stringified, null);
            return new ObjectIdentity(owningEntityType, uuid);
        }
        return new ObjectIdentity(owningEntityType, stringified);
    }
}
