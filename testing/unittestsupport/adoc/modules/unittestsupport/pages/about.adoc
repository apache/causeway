...
If using an RDBMS for persistence that supports `SortedSet` then we strongly recommend that you implement this test.

TIP: The xref:refguide:applib:index/util/ObjectContracts.adoc[ObjectContracts]  utility class substantially simplifies the task of implementing `Comparable` in your domain classes.
