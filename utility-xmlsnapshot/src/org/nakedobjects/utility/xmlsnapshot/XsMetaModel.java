...
public final class XsMetaModel {
	 ...
	 * <li>the <code>xs:import</code> element referencing the NOF namespace is added
	 *     as a child
	 * </ul>
	 */
	Element createXsSchemaElement(final Document xsdDoc) {
		...
		Element xsSchemaElement = createXsElement(xsdDoc, "schema");
		...
		Element xsImportElement = createXsElement(xsdDoc, "import");
	 ...
	 */
	Element createXsElement(final Document xsdDoc, String localName) {

		Element element = xsdDoc.createElementNS(XsMetaModel.W3_ORG_XS_URI, XsMetaModel.W3_ORG_XS_PREFIX+":" + localName);
...
//	}
	...
	Element addXsNofFeatureAttributeElements(final Element parentXsElement, final String feature) {
		Element xsNofFeatureAttributeElement = createXsElement(helper.docFor(parentXsElement), "attribute");
	...
	Element choiceOrSequenceFor(final Element parentXsElement) {
		NodeList choiceNodeList = parentXsElement.getElementsByTagNameNS(XsMetaModel.W3_ORG_XS_URI, "choice");
		if (choiceNodeList.getLength() > 0) {
			return (Element)choiceNodeList.item(0);
		}
		NodeList sequenceNodeList = parentXsElement.getElementsByTagNameNS(XsMetaModel.W3_ORG_XS_URI, "sequence");
	 ...
	 * element, creating and appending if nec; also sets the <code>base</code> attribute.
	 * 
	 * The supplied element is presumed to be one for which <code>xs:extension</code>
	 * is valid as a child (eg <code>xs:complexType</code>).
	 */
	Element extensionFor(final Element parentXsElement, final String base) {
		Element childXsElement = childXsElement(parentXsElement, "extension");
	...
	Element childXsElement(final Element parentXsElement, final String localName) {
		NodeList nodeList = parentXsElement.getElementsByTagNameNS(XsMetaModel.W3_ORG_XS_URI, localName);
		if (nodeList.getLength() > 0) {
			return (Element)nodeList.item(0);
		}

		Element childXsElement = createXsElement(helper.docFor(parentXsElement), localName);
	 ...
	 */
	Element setXsCardinality(final Element xsElement, int minOccurs, int maxOccurs) {
		if (maxOccurs >= 0) {
