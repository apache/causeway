...
public class MockUpdateNotifier implements UpdateNotifier {
	...
	public void broadcastAdd(Object collectionOid, Object elementOid, NakedObjectManager objectStore) {
		LOG.debug("update (add) received " + elementOid);
		expectedBroadcastAdd.addActual(collectionOid);
		expectedBroadcastAdd.addActual(elementOid);
	}
	...
	public void broadcastRemove(Object collectionOid, Object elementOid, NakedObjectManager objectStore) {
		LOG.debug("update (remove) received " + elementOid);
		expectedBroadcastRemove.addActual(collectionOid);
		expectedBroadcastRemove.addActual(elementOid);
	}
	...
	public void verify() {
		...
		if(expected.size() != actual.size()) {throw new AssertionFailedError("Expected and actual sizes differ: " + 
				expected.size() +":" +actual.size());}
		for (int i = 0; i < expected.size(); i++) {
			if(expected.elementAt(i) != actual.elementAt(i)) {throw new AssertionFailedError("Expected " + 
					expected.elementAt(i) +" but found " +actual.elementAt(i));}
