...
public abstract class NakedClassManager implements DebugInfo {
    ...
    public static NakedClassManager getInstance() {
        if (instance == null) { throw new IllegalStateException("No existing class manager"); }
    ...
    public NakedClass getNakedClass(String name) {
        ...
        if (classes.containsKey(name)) {
            ...
            if (cls instanceof String) { throw new NakedObjectRuntimeException(
                    "Unwanted recursion - already creating NakedClass for " + name); }
            return (NakedClass) cls;

        } else {
            classes.put(name, name); // used to ensure no looping occurs
