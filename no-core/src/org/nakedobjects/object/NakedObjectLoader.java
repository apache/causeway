...
/**
 * The NakedObjectLoader is responsible for managing the adapters and identities
 * for each and every POJO that is being used by the NOF. It provides a
 * consistent set of adapters in memory, providing adapter for the POJOs that
 * are in use by the NOF and ensuring that the same object is not loaded twice
 * into memory.
 * 
 * Each POJO is given an adapted so that the NOF can work with the POJOs even
 * though it does not understand their types. Each POJO maps to an adapter and
 * these are reused
 * 
 * Loading of an object refers to the initializing of state within each object
 * as it is restored for persistent storage.
 */
public interface NakedObjectLoader extends NakedObjectsComponent, DebugInfo {
    ...
    /**
     * Creates an adpater for the specified value object. Values are not cached
     * so each call to this will always create new adapter.
     */
    ...
    /**
     * Creates transient instance of the type declared by the specification, and
     * creates an adapter for it.
     * 
    ...
    /**
     * Creates value instance of the type declared by the specification, and
     * creates an adapter for it.
     * 
    ...
    /**
     * Retrieves an existing adapter, from the Pojo-adapter map, for the
     * specified object. If the object is not in the map then null is returned.
     */
    NakedObject getAdapterFor(Object object);

    /**
     * Retrieves an existing adapter, from the identity-adapter map, for the
     * specified object. If the OID is not in the map then null is returned.
     */
    NakedObject getAdapterFor(Oid oid);

    /**
     * Retrieves an existing adapter, from the Pojo-adapter map, for the
     * specified object. If the object is not in the map then a new adapter is
     * returned.
     * 
     * @see #createAdapterForTransient(Object)
     */
    ...
    /**
     * Returns true if the object for the specified OID exists, ie it is already
     * loaded.
     */
     ...
     * transient) and sets the OID on the adapter. The adapter is added to the
     * identity-adapter map.
     */
     ...
     * business object is created and an adapter is created for it. The adapter
     * will then be in the state UNRESOLVED.
     */
    ...
    /**
     * Unloads the specified object from both the identity-adapter map, and the pojo-adapter map.  This indicates that the 
