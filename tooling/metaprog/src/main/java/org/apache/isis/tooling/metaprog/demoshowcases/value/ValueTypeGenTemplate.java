...
import org.apache.isis.commons.io.FileUtils;
...
public class ValueTypeGenTemplate {
    ...
    public static class Config {
         ...
         */
        final boolean isisSpecific;

        final boolean frameworkSupportForJpa;
        /**
         * Implied by {@link #isIsisSpecific()}, if present then indicates that the framework has added support for JPA persistence for certain data types.
         */
        private boolean isFrameworkSupportForJpa() {
            return isisSpecific || frameworkSupportForJpa;
        }
        public String getJpaTypeSupportNotice() {
            return isFrameworkSupportForJpa()
                        ? "Apache Isis provides its own implementation of the relevant JPA extension points for the `" + getShowcaseValueSimpleType() + "` value type, meaning that JPA can persist properties of this value type without further configuration."
        ...
        /**
         * Implied by {@link #isIsisSpecific()}, if present then indicates that the framework has added support for JDO persistence for certain data types.
         */
        private boolean isFrameworkSupportForJdo() {
            return isisSpecific || frameworkSupportForJdo;
        }
        public String getJdoTypeSupportNotice() {
            return isFrameworkSupportForJdo()
                        ? "Apache Isis provides its own implementation of the relevant JDO extension points for the `" + getShowcaseValueSimpleType() + "` value type, meaning that JDO can persist properties of this value type without further configuration."
        ...
        /**
         * Implied by {@link #isIsisSpecific()} or a non-null {@link #getJaxbAdapter()}, if present then indicates that the framework has added support for JAXB serialization for certain data types.
         */
        private boolean isFrameworkSupportForJaxb() {
            return isisSpecific || frameworkSupportForJaxb || jaxbAdapter != null;
        }
        public String getJaxbTypeSupportNotice() {
            return isFrameworkSupportForJaxb()
                    ? "Apache Isis provides its own implementation of `@XmlJavaTypeAdapter` for the `" + getShowcaseValueSimpleType() + "` value type, meaning that JAXB can serialize properties of this value type without further configuration."
    ...
    public void generate(final Consumer<File> onSourceGenerated) {
        ...
        for(var template: config.getTemplates()) {
            ...
            templateVars.put("showcase-note-if-isis-specific", (config.isIsisSpecific() ? "NOTE: This is an Apache Isis specific data type.\n\n": ""));
