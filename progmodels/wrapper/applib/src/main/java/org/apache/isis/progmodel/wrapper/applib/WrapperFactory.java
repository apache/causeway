...
/**
 * Provides the ability to &quot;wrap&quot; of a domain object such that it can be interacted
 * with while enforcing the hide/disable/validate rules implies by the Isis programming model.
 *
 * <p>
 * The &quot;wrap&quot; is a CGLib proxy that wraps the underlying domain object.  The wrapper can then be
...
public interface WrapperFactory {
     ...
     *
     * <p>
     * If the object has (see {@link #isWrapper(Object)} already been wrapped), then should just return the
    ...
    /**
     * Same as {@link #wrap(Object)}, except the actual execution occurs only if the <tt>execute</tt>
     * parameter indicates.
     *
     * <p>
     ...
     *
     * <p>
     * This is retrospective: the listener will be notified of interactions even on wrappers created before the
     * listener was installed. (From an implementation perspective this is because the wrappers delegate back to
     ...
     * wrappers, not just on those wrappers created subsequently. (From an implementation perspective this is
     * because the wrappers delegate back to the container to fire the events).
     *
     * @param listener
     * @return
     */
