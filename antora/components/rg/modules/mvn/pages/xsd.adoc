...
generate XSD schemas from any JAXB-annotated xref:ug:fun:building-blocks/types-of-domain-objects/view-models.adoc[view model/DTOs].
...
objects into ``OidDto``s (as defined by the Apache Isis xref:rg:cms:schema/common.adoc[common schema]).
...
* `jaxbClasses` - a list of xref:ug:fun:programming-model/view-models/jaxb.adoc[JAXB-annotated view model] classes;
...
+
Defaults to `false`.  Most DTO classes will reference one another or the xref:rg:cms:schema/common.adoc[common schema].  Normally it's fine to merge all these XSDs together.  This property, if set, results in each a separate directory for each generation of its XSD or XSDs.
...
<4> catalog file indicates the location of the referenced xref:rg:cms:schema/common.adoc[common schema] XSDs.
...
reference for the Apache Isis xref:rg:cms:schema/common.adoc[common schema] is likely to be needed:
