...
If the `schema()` attribute is specified (and if xref:rg:ant:rgant.adoc#\_rgant-Discriminator[`@Discriminator`] __hasn't__ been specified), is also used to derive the object type, as used in `Bookmark`s, URLs for xref:ug:vro:about.adoc[RestfulObjects viewer] and elsewhere.
