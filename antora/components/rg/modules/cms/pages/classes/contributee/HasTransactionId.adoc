...
Here the (inherited) `getTransactionId()` is intended to be interpreted as an interaction (cf xref:rg:cms:schema/ixn.adoc[`InteractionDto`]) that has at least one member execution (cf xref:rg:cms:schema/ixn/memberExecutionDto.adoc[`memberExecutionDto`]).
