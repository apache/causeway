...
The transaction Id is used to correlate to the xref:rg:cms:schema/interaction.adoc[interaction] that executes the command, and to any xref:rg:cms:schema/changes.adoc[changes] to domain objects occurring as a side-effect of that interaction.
...
xref:rg:cms:schema/common.adoc["common"] schema.
