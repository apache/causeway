...
<2> uses complex types defined in the xref:rg:cms:schema/common.adoc["common" schema] and also the xref:rg:cms:schema/cmd.adoc["cmd" schema]
...
The transaction Id is used to correlate back to the xref:rg:cms:schema/command.adoc[command] that represented the intention to perform this execution, as well as to any xref:rg:cms:schema/changes.adoc[changes] to domain objects that occur as a side-effect of the interaction.
...
The xref:rg:cms:schema/changes.adoc[changes] schema also provides metrics on the number of objects loaded/changed, but relates to the entire interaction rather than just one (sub)execution of an interaction.