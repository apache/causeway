...
<2> uses complex types defined in the xref:rg:cms:schema/common.adoc["common" schema].
...
The transaction Id is used to correlate back to the xref:rg:cms:schema/command.adoc[command] that represented the intention to perform this execution, as well as to the xref:rg:cms:schema/interaction.adoc[interaction] that executes said command.
...
The xref:rg:cms:schema/ixn.adoc[interaction] schema also provides metrics on the number of objects loaded/changed, but is more granular, each figure relating to a single (sub-)execution within an interaction.
