...
A number of other xref:rg:cms:rgcms.adoc#_rgcms_classes_mixins_Object[mixins] also contribute properties or actions to the "Metadata" fieldset.
...
----
<1> returns the XML text wrapped up in a xref:rg:cms:rgcms.adoc#_rgcms_classes_value-types_Clob[`Clob`].



=== Related Services

The mixin delegates to the xref:rg:svc:rgsvc.adoc#_rgsvc_persistence-layer-api_IsisJdoSupport[`IsisJdoSupport`] service to obtain a reference to the JDO `PersistenceManagerFactory`.
