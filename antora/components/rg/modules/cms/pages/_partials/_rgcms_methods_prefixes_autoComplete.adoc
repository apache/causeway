...
[TIP]
====
If the number of candidate objects is comparatively small, then use xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_choices[`choices...()`] supporting method instead.
====
...
<2> the xref:rg:ant:rgant.adoc#_rgant-MinLength[`@MinLength`] annotation specifies the minimum number of characters that must be entered before a search is performed for matching objects
...
----
<1> the xref:rg:ant:rgant.adoc#_rgant-MinLength[`@MinLength`] annotation specifies the minimum number of characters that must be entered before a search is performed for matching objects
