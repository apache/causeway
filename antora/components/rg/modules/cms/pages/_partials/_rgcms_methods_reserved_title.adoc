...
Note that Apache Isis' xref:ug:btb:ugbtb.adoc#_ugbtb_i18n[i18n support] extends this so that titles can also be internationalized.
...
----
<1> simple xref:rg:cms:rgcms.adoc#_rgcms_classes_utility_TitleBuffer[utility class] to help construct the title string
<2> imperative conditional logic
<3> using the xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_WrapperFactory[`WrapperFactory`] to determine if the `dueBy` field is visible for this user ...
<4> ... but ignore if not

As the example above shows, the implementation can be as complex as you like.

In many cases, though, you may be able to use the xref:rg:ant:rgant.adoc#_rgant-Title[`@Title`] annotation.
