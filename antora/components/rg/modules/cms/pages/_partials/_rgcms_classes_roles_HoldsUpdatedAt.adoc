...
The current time is obtained from the xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_ClockService[`ClockService`].

Entities that implement this interface often also implement xref:rg:cms:rgcms.adoc#_rgcms_classes_roles_HoldsUpdatedBy[`HoldsUpdatedBy`] role interface; as a convenience the xref:rg:cms:rgcms.adoc#_rgcms_classes_roles_Timestampable[`Timestampable`] interface combines the two roles.
