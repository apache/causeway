...
Here the (inherited) `getTransactionId()` is intended to be interpreted as an interaction (cf xref:../../rgcms/rgcms.adoc#_rgcms_schema-ixn[`InteractionDto`]) that has at least one member execution (cf xref:rg:cms:rgcms.adoc#__rgcms_schema-ixn_memberExecutionDto[`memberExecutionDto`]).
