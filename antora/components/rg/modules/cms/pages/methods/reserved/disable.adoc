...
It would be painful to have to write a separate xref:rg:cms:methods/prefixes/disable.adoc[`disable...()`] method for each and every member, so instead Isis allows a single `disable...(...)` method to be implemented that is applied to all members.
...
See also the similar methods to xref:rg:cms:methods/reserved/hide.adoc[`hide()`] object members en-masse.
