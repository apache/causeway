...
[NOTE]
====
Directly mutable collections are not currently supported by the xref:ug:vw:about.adoc[Wicket viewer]; they are always implicitly disabled.
====
...
However, Apache Isis' xref:ug:btb:i18n.adoc[i18n support] extends this so that reasons can be internationalized.
