...
Note that enabling this configuration property in effect means that xref:ug:fun:building-blocks/types-of-domain-objects/mixins.adoc[mixins] must be used instead of xref:ug:fun:programming-model/domain-services/contributions.adoc[contributed services] (because contributed actions are the one case where the value of a parameter to a supporting method may be non-null).
====
