...
|`isis.services`
|`FQCN`,`FQCN2`,...
|NO LONGER REQUIRED; replaced by xref:rg:cms:classes/super/AppManifest.adoc[`AppManifest`].
...
|`true`,`false` +
(`false`)
| When searching for  xref:rg:cms:methods/prefixes/disable.adoc[`disableXxx()`] or xref:rg:cms:methods/prefixes/hide.adoc[`hideXxx()`] methods, whether to search only for the no-param version (or also for supporting methods that match the parameter types of the action). +
...
See xref:rg:cfg:configuring-core.adoc#filterVisibility[section below] for further discussion.
