...
Filtering is supported by the xref:ug:vw:ugvw.adoc#[Wicket viewer] and the xref:ug:vro:about.adoc[Restful Objects viewer], and also by the xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_WrapperFactory[`WrapperFactory`] domain service (provided the wrapper's execution mode is __not__ "skip rules").
