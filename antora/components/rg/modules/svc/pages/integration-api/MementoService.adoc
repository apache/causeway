...
The `MementoService` was originally introduced to simplify the implementation of xref:ug:fun:building-blocks/types-of-domain-objects/view-models.adoc[ViewModel]s which are required by the framework to return string representation of all of their backing state, moreover which is safe for use within a URL.
This usage is deprecated; use xref:ug:fun:programming-model/view-models/jaxb.adoc[JAXB view models] instead.
