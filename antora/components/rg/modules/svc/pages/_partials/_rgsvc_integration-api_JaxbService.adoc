...
xref:rg:cms:rgcms.adoc#_rgcms_schema[common Isis schemas] (for example the `OidDto` class that represents a reference to
...
To use an alternative implementation, implement `JaxbService` interface and use xref:rg:ant:rgant.adoc#_rgant-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] (as explained in the xref:rg:svc:rgsvc.adoc#__rgsvc_intro_overriding-the-services[introduction] to this guide).
...
xref:rg:ant:rgant.adoc#_rgant-XmlRootElement[`@XmlRootElement`]-annotated
xref:ug:fun:ugfun.adoc#_ugfun_building-blocks_types-of-domain-objects_view-models[view models].  The functionality to download XML and XSD schemas is also
exposed in the UI through mixins to xref:rg:cms:rgcms.adoc#_rgcms_classes_mixins_Dto[`Dto`] interface.
