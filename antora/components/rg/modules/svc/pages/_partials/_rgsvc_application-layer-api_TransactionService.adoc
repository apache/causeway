...
xref:rg:svc:rgsvc.adoc#_rgsvc_persistence-layer-api_RepositoryService[`RepositoryService`]'s `persist()` or `delete()` methods), then the framework actually queues up the work and only performs the persistence command either at the end of the transaction or immediately prior to the next query.
...
It is also used by the xref:ug:vw:ugvw.adoc#[Wicket viewer]'s support for bulk actions; each action is invoked in its own transaction.
...
xref:rg:cms:rgcms.adoc#_rgcms_classes_mixins_HasTransactionId[`HasTransactionId`] mixin.
