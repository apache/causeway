[[_rgsvc_core-domain-api_ConfigurationService]]
...
The `ConfigurationService` allows domain objects to read the configuration properties aggregated from the various xref:rg:cfg:rgcfg.adoc#_rgcfg_configuration-files[configuration files].
...
[TIP]
====
If you _do_ have a domain service that needs to access Isis properties, then an alternative is to define a xref:rg:ant:rgant.adoc#_rgant-PostConstruct[`@PostConstruct`] method and pass in a `Map<String,String>` of properties.
