[[_rgsvc_integration-api_XmlSnapshotService]]
= `XmlSnapshotService`
...
The xref:rg:svc:rgsvc.adoc#_rgsvc_integration-api_BookmarkService[`BookmarkService`] provides a mechanism for obtaining a string representations of a single domain object.

The xref:rg:svc:rgsvc.adoc#_rgsvc_integration-api_MementoService[`MementoService`] also provides a mechanism for generating string representations of domain objects.

The xref:rg:svc:rgsvc.adoc#_rgsvc_integration-api_JaxbService[`JaxbService`] is a simple wrapper around
...
xref:rg:ant:rgant.adoc#_rgant-XmlRootElement[`@XmlRootElement`]) to be used as view models.
