...
This service is used by the xref:rg:cms:rgcms.adoc#_rgcms_classes_super_FixtureScripts[`FixtureScripts`] service to automatically locate any xref:rg:cms:rgcms.adoc#_rgcms_classes_super_FixtureScript[`FixtureScript`] implementations.
...
[NOTE]
====
This implementation is also used to discover domain services annotated with xref:rg:ant:rgant.adoc#_rgant-DomainService[`@DomainService`].
Currently this logic uses the implementation directly, so is not pluggable.
====

To use an alternative implementation, implement the `ClassDiscoveryService` interface and use xref:rg:ant:rgant.adoc#_rgant-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] (as explained in the xref:rg:svc:rgsvc.adoc#__rgsvc_intro_overriding-the-services[introduction] to this guide).
