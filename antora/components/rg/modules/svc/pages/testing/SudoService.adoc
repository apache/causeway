...
When `sudo(...)` is called the "effective user" is reported by both xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_UserService[`UserService`] and by xref:rg:fis:application-layer/AuthenticationSessionProvider.adoc[`AuthenticationSessionProvider`], but does not propagate through to the xref:ug:sec:about.adoc[Shiro security mechanism].
