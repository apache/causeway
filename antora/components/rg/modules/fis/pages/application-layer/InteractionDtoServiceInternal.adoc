...
<2> called by the framework when editing a property, to create a DTO (for the xref:rg:cms:rgcms.adoc#_rgcms_schema_ixn["ixn" schema]) capturing the details of the property edit (target, new value etc).
...
(from the xref:rg:cms:rgcms.adoc#_rgcms_schema_cmd["cmd" schema]).
