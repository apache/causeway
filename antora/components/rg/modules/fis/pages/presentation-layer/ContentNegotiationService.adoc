...
The default `ContentNegotiationServiceXRoDomainType` implementation provides a xref:rg:cfg:configuring-core.adoc[configuration property] which controls whether a mapped domain object is pretty-printed (formatted, indented) or not:
...
If the property is not set, then the default depends on the xref:rg:cfg:deployment-types.adoc[deployment type]; production mode will disable pretty printing, while prototyping mode will enable it.
