...
Apache Causeway integrates with a variety of authentication mechanisms, including link:https://spring.io/projects/spring-security[Spring Security] and link:https://www.keycloak.org/[Keycloak].

The xref:security:ROOT:about.adoc[SecMan extension] provides a well-featured subdomain of users, roles and permissions against features derived from the Apache Causeway metamodel.
...
The xref:vw:tabular:about.adoc[Tabular Download] component is rather simple: it simply allows the table to be downloaded as a spreadsheet:
