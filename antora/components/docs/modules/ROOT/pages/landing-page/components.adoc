...
The framework allows xref:userguide:fun:building-blocks.adoc#domain-entities[domain entities] to be persisted using the xref:pjdo:ROOT:about.adoc[JDO/DataNucleus object store], for use with the JDO API.

Alternatively, if you restrict yourself to xref:userguide:fun:building-blocks.adoc#view-models[view models] then these can be persisted with any data persistence technology that is supported by Spring Boot.
