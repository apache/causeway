...
Analogous to xref:applib/services/wrapper/events/PropertyModifyEvent[PropertyModifyEvent] , however the _#getReason()_ will always be `null` . (If access is not allowed then a xref:applib/services/wrapper/events/PropertyVisibilityEvent[PropertyVisibilityEvent] would have been fired).
