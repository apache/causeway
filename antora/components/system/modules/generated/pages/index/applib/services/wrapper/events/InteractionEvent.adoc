...
--
<.> `[teal]#*getIdentifier*#()` : `xref:applib/Identifier[Identifier]`
+
--
The xref:applib/Identifier[Identifier] of the feature of the object being interacted with.

Will be consistent with the subclass of xref:applib/services/wrapper/events/InteractionEvent[InteractionEvent] . So for example a xref:applib/services/wrapper/events/PropertyModifyEvent[PropertyModifyEvent] will have an xref:applib/Identifier[Identifier] that identifies the property being modified.
--
...
* for xref:applib/services/wrapper/events/VisibilityEvent[VisibilityEvent] , a veto means that the feature (property, collection, action) is hidden
* for xref:applib/services/wrapper/events/UsabilityEvent[UsabilityEvent] , a veto means that the feature is disabled
* for xref:applib/services/wrapper/events/ValidityEvent[ValidityEvent] , a veto means that the proposed modification (property value, object added/removed, action argument) is invalid
--
