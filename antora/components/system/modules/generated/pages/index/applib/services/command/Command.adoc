...
It also captures details of the corresponding action invocation (or property edit), specifically when that action/edit _Command#getStartedAt() started_ or _Command#getCompletedAt() completed_ , and its result, either a _Command#getResult() return value_ or an _Command#getException() exception_ . Also captures a stack of xref:applib/events/domain/ActionDomainEvent[ActionDomainEvent] s.

Note that when invoking an action, other actions may be invoked courtesy of the xref:applib/services/wrapper/WrapperFactory[WrapperFactory] . These "sub-actions" do _not_ modify the contents of the current command object; in other words think of the command object as representing the outer-most originating action.

That said, if the sub-action is invoked asynchronously (using _WrapperFactory#asyncWrap(Object, AsyncControl)_ or _WrapperFactory#asyncWrapMixin(Class, Object, AsyncControl)_ ), then a separate xref:applib/services/command/Command[Command] object is created, and the originating xref:applib/services/command/Command[Command] is set to be its _Command#getParent() parent_ .
...
--
<.> `[teal]#*getTarget*#()` : `xref:applib/services/bookmark/Bookmark[Bookmark]`
+
--
Derived from _#getCommandDto()_ , is the xref:applib/services/bookmark/Bookmark[Bookmark] of the target object (entity or service) on which this action/edit was performed.
--
