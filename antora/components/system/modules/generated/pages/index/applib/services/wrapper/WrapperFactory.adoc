...
Calling any of the above methods may result in a (subclass of) _InteractionException_ if the object disallows it. For example, if a property is annotated as hidden then a xref:applib/services/wrapper/HiddenException[HiddenException] will be thrown. Similarly if an action has a `validate` method and the supplied arguments are invalid then a xref:applib/services/wrapper/InvalidException[InvalidException] will be thrown.
...
<.> `[teal]#*wrap*#<T>(T domainObject, xref:applib/services/wrapper/control/SyncControl[SyncControl] syncControl)` : `T`
+
--
Provides the "wrapper" of a domain object against which to invoke the action.

The provided xref:applib/services/wrapper/control/SyncControl[SyncControl] determines whether business rules are checked first, and conversely whether the action is executed. There are therefore three typical cases:
...
--
<.> `[teal]#*wrapMixin*#<T>(Class<T> mixinClass, Object mixedIn, xref:applib/services/wrapper/control/SyncControl[SyncControl] syncControl)` : `T`
+
--
Provides the wrapper for a _FactoryService#mixin(Class, Object) mixin_ , against which to invoke the action.

The provided xref:applib/services/wrapper/control/SyncControl[SyncControl] determines whether business rules are checked first, and conversely whether the action is executed. See _#wrap(Object, SyncControl)_ for more details on this.
--
...
--
<.> `[teal]#*asyncWrap*#<T, R>(T domainObject, xref:applib/services/wrapper/control/AsyncControl[AsyncControl]<R> asyncControl)` : `T`
+
--
Returns a proxy object for the provided _domainObject_ , through which can execute the action asynchronously.
--
<.> `[teal]#*asyncWrapMixin*#<T, R>(Class<T> mixinClass, Object mixedIn, xref:applib/services/wrapper/control/AsyncControl[AsyncControl]<R> asyncControl)` : `T`
+
--
Returns a proxy object for the provided _mixinClass_ , through which can execute the action asynchronously.
--
<.> `[teal]#*getListeners*#()` : `List<xref:applib/services/wrapper/listeners/InteractionListener[InteractionListener]>`
+
--
All xref:applib/services/wrapper/listeners/InteractionListener[InteractionListener] s that have been registered using _#addInteractionListener(InteractionListener)_ .
--
<.> `[teal]#*addInteractionListener*#(xref:applib/services/wrapper/listeners/InteractionListener[InteractionListener] listener)` : `boolean`
+
--
Registers an xref:applib/services/wrapper/listeners/InteractionListener[InteractionListener] , to be notified of interactions on all wrappers.
...
--
<.> `[teal]#*removeInteractionListener*#(xref:applib/services/wrapper/listeners/InteractionListener[InteractionListener] listener)` : `boolean`
+
--
Remove an xref:applib/services/wrapper/listeners/InteractionListener[InteractionListener] , to no longer be notified of interactions on wrappers.
