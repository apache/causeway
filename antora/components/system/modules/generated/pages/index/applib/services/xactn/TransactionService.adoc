...
<.> `[teal]#*currentTransactionId*#()` : `xref:applib/services/xactn/TransactionId[TransactionId]`
+
--
When called within an existing transactional boundary returns the unique identifier to the transaction, _null_ otherwise.
--
<.> `[teal]#*currentTransactionState*#()` : `xref:applib/services/xactn/TransactionState[TransactionState]`
