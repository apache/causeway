...
* it may follow the equal-by-content contract (as per xref:applib/annotation/EqualByContent[EqualByContent] ), and by default is presumed that it does.
+
--
Note also that though a value is conceptually not shared, if it is also immutable then it is in fact safe to share objects (as in the flyweight pattern). In addition, the xref:applib/annotation/EqualByContent[EqualByContent] semantic means that we needn't care whether value types are being shared or not.
--
...
+
--
The fully qualified name of a class that implements the xref:applib/adapters/ValueSemanticsProvider[ValueSemanticsProvider] interface.
...
Implementation note: the default value provided here is simply the xref:applib/annotation/Value[Value] 's own class, because `null` is not a valid default.
--
