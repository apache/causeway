...
For custom-written (as opposed to third-party) value types, the ability for the xref:applib/adapters/Parser[Parser] to provide a title responsibilities overlap with other conventions for domain objects. Specifically, normally we write a `title()` method to return a title. In such cases a typical implementation of xref:applib/adapters/Parser[Parser] would just delegate to the value type itself to obtain the title (ie invoking the `title()` method directly rather than having the framework do this).
