...
This is done using the `content.sources.url[].tags` and `content.sources.url[].branches` properties.
We use tags for all cases - note that the tag appears in the generated UI.
If there are patches to the documentation, we move the tag.
...
We also have to modify the home page `index.html`:

* Update the home page of the website, `antora/supplemental-ui/index.html`

** starter app instructions for the helloworld app:
+
...
** starter app instructions for the simpleapp:
+
[source,html,subs="attributes+"]
----
...
** any mention of `docs/latest` -> `docs/{isisrel}`
...
* Now tag.
+
We use "docs/isis-{isisrel}" as the tag name.
This mirrors the "rel/isis-{isisrel}" used for the formal (immutable) release tag, but allows us to move it if necessary.
+
[source,bash,subs="attributes+"]
----
git tag docs/$ISISART-$ISISREL
git push origin refs/tags/$ISISREL:refs/tags/$ISISREL
----

* Now, immediately revert the last commit.
+
[source,bash,subs="attributes+"]
----
git revert HEAD
...
* Finally, revert the changes to `site.yml`:
+
[source,bash,subs="attributes+"]
----
git revert HEAD
