...
* We create the `{isisrel}` branch.
+
This mirrors the "rel/isis-{isisrel}" used for the formal (immutable) release tag, but is a branch because it allows us to move it, and must have this simplified name as it is used in the "edit page" link of the site template.
+
[source,bash,subs="attributes+"]
----
git branch {isisrel}
git push origin {isisrel}
----

Finally, revert the last commit (backing changes to `antora.yml` files and `index.html`):
...
+
//** `url: .` -> `url: https://github.com/apache/isis.git`
** `branches: HEAD` -> `branches: {isisrel}`
