...
[source,bash,subs="attributes+"]
----
find . -name antora.yml -exec sed -i 's/latest/{page-causewayreleaseversion}/g' {} \;
----
...
+
Bump the `BASELINE` variable in link:https://github.com/apache/causeway/blob/main/.github/workflows/ci-build-artifacts-no-push-maven.yml#L20[ci-build-artifacts-no-push-maven.yml] (workflow file).
