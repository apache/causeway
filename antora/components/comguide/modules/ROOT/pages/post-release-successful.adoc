...
This is done in the release branch (`$CAUSEWAYBRANCH`), _not_ in the documentation branch for the current release ({page-causewayrel}).
...
* Update with the documentation branch for the current release ({page-causewayrel})
+
[source,bash,subs="attributes+"]
----
git merge {page-causewayrel} --ff-only  #<.>
----
...
git push origin release-{page-causewayrel}-$RC --delete
----
