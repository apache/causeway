...
We use Antora to generate the site, not only the version being release but also any previous versions listed in `site.yml`; each of these are tags.
The most recent version is also tagged with the `latest` branch (updated each release); this means that neither the `antora.yml` files nor top-level `index.html` file needs to be updated.
The `antora/playbooks/site.yml` _does_ need to be updated, but the `site-ALL.yml` file (alongside it) is used as a way to quickly make the necessary edits.
...
* update the new `site.yml`:

** for each documentation component, add in a reference to a new branch `{page-causewayrel}` (we'll create it in the next step).
Also make sure that the `latest` tag is also in the list; eg:
+
...
content:
  sources:
    - url: .
      ...
      branches: [2.0.0-M5, ...  2.0.0-RC3, latest]
      # ...
    - url: .
...
* also check that the attributes are correct.
+
The value of the `page-causewayprev` attribute needs to be (the current) `{page-causewayrel}`, and `page-causewayrel` then set to the next likely release version:
+
[source,yaml,subs="attributes+"]
.site.yml
----
...
Now we ensure that the `{page-causewayrel}` branch and the `latest` tag refer to the current location:

* create the `{page-causewayrel}` branch.
+
This mirrors the "rel/causeway-{page-causewayrel}" used for the formal (immutable) release tag.
+
[source,bash,subs="attributes+"]
----
...
This name is used in the "edit page" link of the site template.
+
[source,bash,subs="attributes+"]
----
