...
+
image::release-process-post/update-antora-latest.png[width="600px",link="{imagesdir}/release-process-post/update-antora-latest.png"]
...
----
git add .
git commit -m "$ISISJIRA: bumps antora.yml version to $ISISREL"
...
[source,yml,subs="attributes+"]
----
- url: https://github.com/apache/isis.git
  start_path: antora/components/docs # docs
...
eg using the IDE:
+
image::release-process-post/update-site-yml-tags.png[width="600px",link="{imagesdir}/release-process-post/update-site-yml-tags.png"]
...
+
That is, so that each content source specifies a local URL and only the HEAD branch:
+
.site.yml
[source,yml,subs="attributes+"]
----
...
git pull origin/asf-site --ff-only
----
