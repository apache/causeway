...
Apache Causeway itself is released in one go, everything is ultimately a child of `parent/pom.xml` (relative to the link:https://github.com/apache/causeway[source code root]).
...
=== Update top-level pom

The top level `pom.xml` inherits from `org.apache:apache` pom.
Update if there has been a new version of this parent.
...
+
Is are intended to reference the _most recently released_ version (demonstrating persistence using EclipseLink JPA).
This branch is referenced from the home page and getting started pages of the website.
