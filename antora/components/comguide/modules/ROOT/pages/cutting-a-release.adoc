...
----
<properties>
    <revision>{page-isisrel}</revision>
    ...
</properties>
----
...
[NOTE]
====
The `apache-release` profile is set to only check the modules actually to be released (ignores the incubator modules).

Partly that's because there's no need to check the inubator modules, it's also because the incubator modules require Java 11 (Java 8 is used for everything else)
====
