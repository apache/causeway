...
[NOTE]
====
The `released system property activates a profile that only runs the modules actually to be released (ignores the incubator modules).

That's because the incubator modules require Java 11, whereas (for now at least) the non-incubator module are built and released by Java 8.
====
