...
[IMPORTANT]
====
The branch name is intentionally *not* the same as the eventual tag names (eg `causeway-{page-causewayrelv2}`).
====
...
IMPORTANT: make sure you have switched back to Java 11 to perform the build.
...
* `v2-jpa` and `v2-jdo` ; and also `v3-jpa` and `v3-jdo`
+
...
* `v2-jpa-SNAPSHOT` and `v2-jdo-SNAPSHOT` ; and also `v3-jpa-SNAPSHOT` and `v3-jdo-SNAPSHOT`
+
These reference the most current snapshot nightly build.

The general idea is that a release will fast-forward `v{2,3}-jpa` to `v{2,3}-jpa-SNAPSHOT` and similarly moves `v{2,3}-jdo` up to `v{2,3}-jdo-SNAPSHOT`, bumping to the newly released version of the framework in the process.
...
[source,bash,subs="attributes+"]
----
git checkout v3-jpa-SNAPSHOT    # or: git checkout v2-jpa-SNAPSHOT
git pull --ff-only

git checkout -b $CAUSEWAYBRANCH-jpa

mvnd versions:update-parent -DparentVersion=[$CAUSEWAYREL,$CAUSEWAYREL] -Dskip.nightly  # <.>
mvnd versions:set -DnewVersion=$CAUSEWAYREL
...
[source,bash,subs="attributes+"]
----
git checkout v3-jdo-SNAPSHOT    # or: git checkout v2-jdo-SNAPSHOT
git pull --ff-only
...
+
This might be easiest to just cherry-pick commits made on the `v3-jpa` branch.

** Merge in the `v3-jpa` branch, discarding any conflicts.
+
[source,bash,subs="attributes+"]
----
...
----
[VOTE] Apache Causeway Core release {page-causewayrelv2} RC1
----
...
In the source code repo the code has been tagged as causeway-{page-causewayrelv2}-RC1;
...
* the version number (`{page-causewayrelv2}` or whatever)
