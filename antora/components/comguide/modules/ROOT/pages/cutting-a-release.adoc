...
[CAUTION]
====
Be aware of any changes to the configuration property pages, in particular for default values going missing.

It seems that building from the command line using `mvn` does not preserve all the information in the `core/config/target/classes/META-INF/spring-configuration-metadata.json` file that is parsed in order to create these pages.

If you observe this, then rebuild the project using an IDE (IntelliJ) and re-run the steps to create the config properties:
...
Partly that's because there's no need to check the incubator modules, it's also because the incubator modules might require new than Java 11 (Java 11 is used for everything else)
====
