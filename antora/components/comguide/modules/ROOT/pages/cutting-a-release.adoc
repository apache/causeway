...
I hit issues running on Windows under git-bash, as I was picking up git's version of gpg in `/usr/bin/gpg`.
I sorted the issue eventually by running under Powershell, to pick up my gpg4Win.
====
...
When prompted, enter your GPG passphrase (unless configured in `~/mvn/settings.xml` as the `gpg.passphrase` property)
