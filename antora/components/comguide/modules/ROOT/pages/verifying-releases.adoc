...
Whenever a release manager announces a vote on a release (as per the xref:comguide:ROOT:cutting-a-release.adoc#_start_voting_thread_on_dev_mailing_list[release process]) on the xref:docs:support:mailing-list.adoc#mailing-lists[dev mailing list], it is the responsibility of the project's PMC to cast their vote on the release.
