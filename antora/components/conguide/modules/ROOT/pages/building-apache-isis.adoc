...
Apache Isis v2 is compatible with Java 8 and above, and releases are xref:comguide:ROOT:cutting-a-release.adoc[cut] using Java 8, leveraging the link:http://maven.apache.org/plugins/maven-toolchains-plugin/[Maven toolchains plugin]).

Some of the tooling used to generate documentation uses Java 11.

Therefore install either/both of Java 8 JDK and Java 11 JDK.
...
link:http://maven.apache.org/plugins/maven-toolchains-plugin/[toolchains] plugin so that releases can be built using Java 8.
