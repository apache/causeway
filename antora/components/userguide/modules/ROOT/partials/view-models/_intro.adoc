...
Sometimes we may have domain objects that conceptually are entities - long-lived persistent and shared instances - but where the state is not stored in an accessible RDBMS.

For example:

* The entity might be externally-managed, and reside in some other system, for example accessible only through a SOAP web service.

* The entity's state might reside in a datastore not supported by xref:pjpa:ROOT:about.adoc[JPA/Eclipselink], such as a graph database like neo4j.

* the entity's state might be held only in-memory, for example read-only configuration data that is read from a config file (eg log4j appender definitions).

Because such entities are responsible for their own state management, they should be implemented within Causeway as view models, for example by implementing the xref:refguide:applib:index/ViewModel.adoc[] interface.
