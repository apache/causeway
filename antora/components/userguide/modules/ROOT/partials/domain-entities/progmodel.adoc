...
In this section we explain the basics, using annotations.
...
In particular, its strongly recommended that the `logicalTypeName` (which acts like an alias to the concrete domain class) is specified; note that it corresponds to the schema/table for EcilpseLink's `@Table` annotation.
