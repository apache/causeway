...
Mixins are somewhat similar to traits or extension methods in other programming languages.
A mixin class is an implementation of behaviour - actions, (derived) properties and (derived) collections - that is contributed to another class, either a domain entity or view model.
...
In fact, the mixin class is bound to the contributee through a Java type, which could be a concrete class, an abstract class, an interface or -- even -- `java.lang.Object`.
...
There's a lot more discussion on this topic in xref:modules.adoc[modules], below.
...
Reenskaug was the inventor of the MVC pattern (and also the external examiner for Richard Pawson's link:https://causeway.apache.org/docs/2.0.0-RC1/_attachments/Pawson-Naked-Objects-thesis.pdf[PhD thesis] on Naked Objects), while Coplien has a long history in object-orientation, C++ and patterns.
