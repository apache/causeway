...
This section shows the idioms for creating, reading, updating and deleting xref:userguide:ROOT:domain-entities.adoc[domain entities] using domain services.
...
When the framework instantiates the object, all services are injected into the framework, and an `ObjectCreatedEvent` xref:userguide:ROOT:overview.adoc#lifecycle-events[lifecycle event] will also be emitted.
...
When an object is persisted the framework will emit `ObjectPersistingEvent` and `ObjectPersistedEvent` xref:userguide:ROOT:overview.adoc#lifecycle-events[lifecycle events].
...
When an object is loaded from the database the framework will emit `ObjectLoadedEvent` xref:userguide:ROOT:overview.adoc#lifecycle-events[lifecycle event].
...
When an object is updated the framework will emit `ObjectUpdatingEvent` and `ObjectUpdatedEvent` xref:userguide:ROOT:overview.adoc#lifecycle-events[lifecycle events].
...
When an object is deleted the framework will emit `ObjectRemovingEvent` xref:userguide:ROOT:overview.adoc#lifecycle-events[lifecycle event].
