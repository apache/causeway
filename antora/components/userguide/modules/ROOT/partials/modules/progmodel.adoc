...
We have Maven modules, Java 9 (Jigsaw) modules and we also have Spring modules (a class annotated with link:https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/Configuration.html[@Configuration]).

In the context of Apache Causeway, a module is actually implemented as the last of these, a Spring `@Configuration` module.
