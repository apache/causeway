...
[NOTE]
====
You could instead choose to have multiple Spring modules per Maven module, but if so you will need to watch out for cycles.
...
<.> recommended if using xref:pjpa::about.adoc[JPA/EclipseLink].
