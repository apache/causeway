...
For this, we recommend that you ensure that all such methods are tested through an xref:testing:integ-test-support:about.adoc[integration test] (not unit test).
...
For this to work, the `namedEscaped()` attribute must be specified using either the xref:vw:ROOT:layout/file-based.adoc[layout file], or using an annotation such as xref:refguide:applib-ant:PropertyLayout.adoc[`@PropertyLayout`] or xref:refguide:applib-ant:ParameterLayout.adoc[`@ParameterLayout`].
