...
xref:refguide:applib-svc:persistence-layer-api/IsisJdoSupport.adoc[IsisJdoSupport] domain service, then you are likely to confuse the
...
The framework provides the xref:refguide:applib-svc:application-layer-api/MessageService.adoc[`MessageService`] as a means to return an out-of-band
...
To handle this the xref:refguide:applib-svc:presentation-layer-spi/ExceptionRecognizer.adoc[`ExceptionRecognizer`] SPI can be used.  The
