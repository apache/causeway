...
In practical terms, this means that view models should be instantiated using a constructor, and then injecting services (if required) using the xref:refguide:applib-svc:metadata-api/ServiceRegistry.adoc[`ServiceRegistry`] service:
...
What will most likely *fail* is to use the xref:refguide:applib-svc:core-domain-api/FactoryService.adoc[`FactoryService`]:
