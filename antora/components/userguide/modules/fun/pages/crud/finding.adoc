...
When an object is loaded from the database the framework will emit `ObjectLoadedEvent` xref:ug:fun:building-blocks.adoc#lifecycle-events[lifecycle event].
