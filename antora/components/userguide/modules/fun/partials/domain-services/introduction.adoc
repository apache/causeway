...
xref:userguide:fun:domain-services.adoc#domain-services[here].
====

=== Hexagonal Architecture

It's worth extending the xref:userguide:fun:background-context-and-theory.adoc#hexagonal-architecture[Hexagonal Architecture] to show where domain services fit in:
...
Although these can be organised using annotations, it's usually easier to use a xref:userguide:fun:ui.adoc#file-based-menus[file-based layout file] (`menubars.layout.xml`).
...
There is further discussion of service injection xref:userguide:fun:domain-services.adoc#injecting-services[below].
