...
image::powered-by/gesconsultor-grc/fig-4-managing-projects.png[width="px",link="{imagesdir}powered-by/gesconsultor-grc/fig-4-managing-projects.png"]
...
Even so, while GRC does (as the figures above show) include custom views, the majority of the views exposed by GRC are automatically generated from the underlying domain entities; the GRC viewer is a proprietary version of Isis' own xref:ug:vw:ugvw.adoc#[Wicket viewer].
...
At the Domain level, we also extensively use the xref:ug:fun:ugfun.adoc#_ugfun_building-blocks_events_domain-events[domain events] functionality available in Apache Isis, along with many of the (non-ASF) link:http://platform.incode.org[Incode Platform^] (such as audit, security and excel modules).

The GRC platform currently has more than 5600 automated tests, about 2000 being xref:ug:tst:ugtst.adoc#_ugtst_bdd-spec-support[BDD tests] and 3600 xref:ug:tst:ugtst.adoc#_ugtst_integ-test-support[integration tests], all leveraging Apache Isis' extensive testing capabilities.
