...
Set up xref:support:dg:dg.adoc#_dg_ide[an IDE] and import the project to be able to run and debug the app.

Then set up a launch configuration and check that you can:

* Run the app from within the IDE
* Run the app in debug mode
* Run with different deploymentTypes; note whether prototype actions (those annotated xref:rg:ant:rgant.adoc#_rgant-Action_restrictTo[`@Action(restrictTo=PROTOTYPING`]) are available or not:
...
* specify a xref:ug:fun:ugfun.adoc#_ugfun_ui-hints_object-titles-and-icons[title]
* specify an xref:ug:fun:ugfun.adoc#_ugfun_ui-hints_object-titles-and-icons[icon]
* make the entity bookmarkable by adding the xref:rg:ant:rgant.adoc#_rgant-DomainObjectLayout_bookmarking[`@DomainObjectLayout#bookmarking()`] attribute.
...
** as per the docs describing xref:ug:fun:ugfun.adoc#_ugfun_crud[how to create or delete objects]
* rename if you wish
** eg `newPet(...)` or `addPet(...)`
* review `listAll` action (acting as a repository)
* as per the docs describing xref:ug:fun:ugfun.adoc#_ugfun_crud[how to write a custom repository]
* note the annotations on the corresponding domain class (originally called `SimpleObject`, though renamed by now, eg to `Pet`)
* rename if you wish
** eg `listPets()`
* note the xref:rg:ant:rgant.adoc#_rgant-DomainService[`@DomainService`] annotation
...
* use the injected xref:rg:svc:rgsvc.adoc#_rgsvc_persistence-layer-api_IsisJdoSupport[`IsisJdoSupport`] domain service.
...
* use the xref:rg:ant:rgant.adoc#_rgant-ParameterLayout_named[`@ParameterLayout(named=...)`] annotation to specify the name of action parameters
* use the xref:rg:ant:rgant.adoc#_rgant-Action_semantics[`@Action(semanticsOf=...)`]  annotation to indicate the semantics of the action (safe/query-only, idempotent or non-idempotent)
* annotate safe action as bookmarkable using xref:rg:ant:rgant.adoc#_rgant-ActionLayout_bookmarking[`@ActionLayout(bookmarking=...)`]
* confirm is available from bookmark panel (top-left of Wicket UI)
...
* experiment changing xref:rg:ant:rgant.adoc#_rgant-Action_semantics[`@Action(semantics=...)`] on actions
...
* add some xref:ug:fun:ugfun.adoc#_ugfun_programming-model_properties[value properties]; also:
* for string properties
** use the xref:rg:ant:rgant.adoc#_rgant-PropertyLayout_multiLine[`@PropertyLayout(multiLine=...)`] annotation to render a text area instead of a text box
** use the xref:rg:ant:rgant.adoc#_rgant-Property_maxLength[`@Property(maxLength=...)`] annotation to specify the maximum number of characters allowable
** use joda date/time properties, bigdecimals and blob/clob properties
* use the xref:rg:ant:rgant.adoc#_rgant-Property_optionality[`@Column(allowsNull=...)`] annotation specify whether a property is optional or mandatory
...
* revisit the title, consider whether to use the xref:rg:ant:rgant.adoc#_rgant-Title[`@Title`] annotation
** rather than the xref:rg:cms:rgcms.adoc#_rgcms_methods_reserved_title[`title()`] method
* order the properties using the xref:rg:ant:rgant.adoc#_rgant-MemberOrder[`@MemberOrder`], also `@MemberGroupLayout`
** see also the docs on xref:ug:vw:ugvw.adoc#_ugvw_layout_annotation-based[static layouts]
* use the xref:rg:ant:rgant.adoc#_rgant-PropertyLayout[`@PropertyLayout`] annotation to position property/action parameter labels either to the LEFT, TOP or NONE
** do the same for parameters using xref:rg:ant:rgant.adoc#_rgant-ParameterLayout[`@ParameterLayout`]
...
* add some xref:ug:fun:ugfun.adoc#_ugfun_programming-model_properties[reference properties]
...
** use the xref:rg:ant:rgant.adoc#_rgant-DomainObject_bounding[`@DomainObjectLayout(bounding=...)`] annotation on the referenced type if there are only a small number (bounded) of instances
** use a xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_choices[`choices...()`] supporting method
*** on a property
*** on an action parameter
** use a xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_autoComplete[`autoComplete...()`] supporting method
*** on a property
*** on an action parameter
...
* Add xref:ug:fun:ugfun.adoc#_ugfun_drop-downs-and-defaults[defaults] for action parameters
...
** use the xref:rg:cms:rgcms.adoc#_rgcms_classes_utility_ObjectContracts[`ObjectContracts`] utility class to help implement `Comparable`
*** you can also implement `equals()`, `hashCode()`, `toString()`
* Add a xref:ug:fun:ugfun.adoc#_ugfun_programming-model_collections[collection] to one of the entities
** Use `SortedSet` as the class
** Use the xref:rg:ant:rgant.adoc#_rgant-CollectionLayout_render[`@CollectionLayout(render=...)`] annotation to indicate if the collection should be visible or hidden by default
* optional: use the xref:rg:ant:rgant.adoc#_rgant-CollectionLayout_sortedBy[`@CollectionLayout(sortedBy=...)`] annotation to specify a different comparator than the natural ordering
...
* to create objects, xref:ug:fun:ugfun.adoc#_ugfun_programming-model_inject-services[inject] associated domain service
** generally we recommend using the xref:rg:ant:rgant.adoc#_rgant-Inject[`@Inject`] annotation with either private or default visibility
* the service itself should use xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_RepositoryService[`RepositoryService`]
* use the xref:rg:ant:rgant.adoc#_rgant-MemberOrder[`@MemberOrder(name=...)`] annotation to associate an action with a property or with a collection
...
* the bootstrap "btn" CSS classes can be used using the xref:rg:ant:rgant.adoc#_rgant-ActionLayout_cssClass[`@ActionLayout(cssClass=...)`] annotation

*  the http://fortawesome.github.io/Font-Awesome/icons/[Font Awesome] icons can be used using the xref:rg:ant:rgant.adoc#_rgant-ActionLayout_cssClassFa[`@ActionLayout(cssClassFa=...)`]

It's also possible to use Font Awesome icons for the xref:ug:fun:ugfun.adoc#_ugfun_ui-hints_object-titles-and-icons[domain object icon].
...
* Delete the various hint annotations and instead specify layout hints using a  xref:ug:vw:ugvw.adoc#_ugvw_layout_file-based[.layout.xml] file.
...
* Use the xref:rg:ant:rgant.adoc#_rgant-Property_hidden[`Property(hidden=...)`] annotation to make properties invisible
** likewise xref:rg:ant:rgant.adoc#_rgant-Collection_hidden[`@Collection(hidden=...)`] for collections
** the xref:rg:ant:rgant.adoc#_rgant-Programmatic[`@Programmatic`]  annotation can also be used and in many cases is to be preferred; the difference is that the latter means the member is not part of the Apache Isis metamodel.
* Use the xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_hide[`hide...()`] supporting method on properties, collections and actions to make a property/collection/action invisible according to some imperative rule


=== Use it!

* Use the xref:rg:ant:rgant.adoc#_rgant-Property_editing[`Property(editing=...)`] annotation to make property read-only
** likewise xref:rg:ant:rgant.adoc#_rgant-Collection_editing[`@Collection(editing=...)`] for collections
** alternatively, use xref:rg:ant:rgant.adoc#_rgant-DomainObject_editing[`@DomainObject(editing=...)`] to disable editing for all properties/collections
* Use the xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_disable[`disable...()`] supporting method on properties and actions to make a property/action disabled according to some imperative rule


=== Do it!

* use the xref:rg:ant:rgant.adoc#_rgant-Property_regexPattern[`@Property(regexPattern=...)`] annotation to specify a regex pattern for properties, and use xref:rg:ant:rgant.adoc#_rgant-Parameter_regexPattern[`@Parameter(regexPattern=...)`] for parameters
* use the xref:rg:ant:rgant.adoc#_rgant-Property_maxLength[`@Property(maxLength=...)`] annotation to indicate a maxmum number of characters, and xref:rg:ant:rgant.adoc#_rgant-Parameter_maxLength[`@Parameter(maxLength=...)`] for parameters
* Use the xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_validate[`validate...()`] supporting method on properties or action parameter
* optional: for any data type:
** use the xref:rg:ant:rgant.adoc#_rgant-Property_mustSatisfy[`Property(mustSatisfy=...)`] and xref:rg:ant:rgant.adoc#_rgant-Parameter_mustSatisfy[`Parameter(mustSatisfy=...)`] annotations to specify arbitrary constraints on properties and parameters
...
* Add the xref:rg:ant:rgant.adoc#_rgant-HomePage[`@HomePage`] annotation to one (no more) of the domain services' no-arg actions
...
* inject xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_ClockService[`ClockService`]
...
** annotate with xref:rg:ant:rgant.adoc#_rgant-DomainService_nature[`@DomainService(nature=NatureOfService.VIEW_CONTRIBUTIONS_ONLY)`]
...
* For this action:
** add the xref:rg:ant:rgant.adoc#_rgant-ActionLayout_contributedAs[`@ActionLayout(contributedAs=ASSOCIATION)`] annotation
...
By default the events generated are `ActionDomainEvent.Default` (for actions) and `PropertyDomainEvent.Default` (for properties). Subclasses of these can be specified using the xref:rg:ant:rgant.adoc#_rgant-Action_domainEvent[`@Action(domainEvent=...)`] or xref:rg:ant:rgant.adoc#_rgant-Property_domainEvent[`Property(domainEvent=...)`] for properties.


Using the guidance in the docs for the xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_EventBusService[`EventBusService`]:
...
Actions are specified as being bulk actions using the xref:rg:ant:rgant.adoc#_rgant-Action_invokeOn[`@action(invokeOn=OBJECT_AND_COLLECTION)`] annotation.
...
* Inject the xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_ActionInvocationContext[`ActionInteractionContext`] (request-scoped) service
...
The similar xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_Scratchpad[`Scratchpad`] (request-scoped) domain service is a good way to share information between bulk action invocations:
...
The xref:rg:svc:rgsvc.adoc#_rgsvc_persistence-layer-api_QueryResultsCache[`QueryResultsCache`] (request-scoped) domain service allows arbitrary objects to be cached for the duration of a request.
...
Each element in the Wicket viewer (entity form, properties, collections, action button etc) is a component, each created by a internal API (`ComponentFactory`, described xref:ug:vw:ugvw.adoc#_ugvw_extending[here]).
...
To support these use cases, Apache Isis therefore allows you to write a view model, either by annotating the class with xref:rg:ant:rgant.adoc#_rgant-ViewModel[`@ViewModel`] or (for more control) by implementing the xref:rg:cms:rgcms.adoc#_rgcms_classes_super_AbstractViewModel[`ViewModel`] interface.
...
https://code.google.com/p/mockito/[Mockito] seems to be the current favourite among Java developers for mocking libraries, but if you use JMock then you'll find we provide a `JUnitRuleMockery2` class and a number of other utility classes, documented xref:ug:tst:ugtst.adoc#_ugtst_unit-test-support[here].
...
Isis has great support for writing xref:ug:tst:ugtst.adoc#_ugtst_integ-test-support[integration tests]; well-written integration tests should leverage fixture scripts and use the xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_WrapperFactory[`@WrapperFactory`] domain service.
...
* as per xref:ug:vro:ugvro.adoc#__ugvro_simplified-representations_configuration-properties[the documentation], configure the Restful Objects viewer to generate a simplified object representation: +
+
[source,ini]
----
