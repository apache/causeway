...
** see also the docs on xref:ug:vw:layout/annotation-based.adoc[static layouts]
...
* Delete the various hint annotations and instead specify layout hints using a  xref:ug:vw:layout/file-based.adoc[.layout.xml] file.
...
Each element in the Wicket viewer (entity form, properties, collections, action button etc) is a component, each created by a internal API (`ComponentFactory`, described xref:ug:vw:extending.adoc[here]).
