...
* Run with different deploymentTypes; note whether prototype actions (those annotated xref:rg:ant:Action/restrictTo.adoc[`@Action(restrictTo=PROTOTYPING`]) are available or not:
...
* make the entity bookmarkable by adding the xref:rg:ant:DomainObjectLayout/bookmarking.adoc[`@DomainObjectLayout#bookmarking()`] attribute.
...
* rename if you wish
** eg `listPets()`
* note the xref:rg:ant:DomainService.adoc[`@DomainService`] annotation
...
* use the xref:rg:ant:ParameterLayout/named.adoc[`@ParameterLayout(named=...)`] annotation to specify the name of action parameters
* use the xref:rg:ant:Action/semantics.adoc[`@Action(semanticsOf=...)`]  annotation to indicate the semantics of the action (safe/query-only, idempotent or non-idempotent)
* annotate safe action as bookmarkable using xref:rg:ant:ActionLayout/bookmarking.adoc[`@ActionLayout(bookmarking=...)`]
* confirm is available from bookmark panel (top-left of Wicket UI)
...
* experiment changing xref:rg:ant:Action/semantics.adoc[`@Action(semantics=...)`] on actions
...
** use the xref:rg:ant:PropertyLayout/multiLine.adoc[`@PropertyLayout(multiLine=...)`] annotation to render a text area instead of a text box
** use the xref:rg:ant:Property/maxLength.adoc[`@Property(maxLength=...)`] annotation to specify the maximum number of characters allowable
** use joda date/time properties, bigdecimals and blob/clob properties
* use the xref:rg:ant:Property/optionality.adoc[`@Column(allowsNull=...)`] annotation specify whether a property is optional or mandatory
...
* revisit the title, consider whether to use the xref:rg:ant:Title.adoc[`@Title`] annotation
** rather than the xref:rg:cms:rgcms.adoc#_rgcms_methods_reserved_title[`title()`] method
* order the properties using the xref:rg:ant:MemberOrder.adoc[`@MemberOrder`], also `@MemberGroupLayout`
** see also the docs on xref:ug:vw:ugvw.adoc#_ugvw_layout_annotation-based[static layouts]
* use the xref:rg:ant:PropertyLayout.adoc[`@PropertyLayout`] annotation to position property/action parameter labels either to the LEFT, TOP or NONE
** do the same for parameters using xref:rg:ant:ParameterLayout.adoc[`@ParameterLayout`]
...
** use the xref:rg:ant:DomainObject/bounding.adoc[`@DomainObjectLayout(bounding=...)`] annotation on the referenced type if there are only a small number (bounded) of instances
...
** Use the xref:rg:ant:CollectionLayout/render.adoc[`@CollectionLayout(render=...)`] annotation to indicate if the collection should be visible or hidden by default
* optional: use the xref:rg:ant:CollectionLayout/sortedBy.adoc[`@CollectionLayout(sortedBy=...)`] annotation to specify a different comparator than the natural ordering
...
** generally we recommend using the xref:rg:ant:Inject.adoc[`@Inject`] annotation with either private or default visibility
* the service itself should use xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_RepositoryService[`RepositoryService`]
* use the xref:rg:ant:MemberOrder.adoc[`@MemberOrder(name=...)`] annotation to associate an action with a property or with a collection
...
* the bootstrap "btn" CSS classes can be used using the xref:rg:ant:ActionLayout/cssClass.adoc[`@ActionLayout(cssClass=...)`] annotation

*  the http://fortawesome.github.io/Font-Awesome/icons/[Font Awesome] icons can be used using the xref:rg:ant:ActionLayout/cssClassFa.adoc[`@ActionLayout(cssClassFa=...)`]
...
* Use the xref:rg:ant:Property/hidden.adoc[`Property(hidden=...)`] annotation to make properties invisible
** likewise xref:rg:ant:Collection/hidden.adoc[`@Collection(hidden=...)`] for collections
** the xref:rg:ant:Programmatic.adoc[`@Programmatic`]  annotation can also be used and in many cases is to be preferred; the difference is that the latter means the member is not part of the Apache Isis metamodel.
...
* Use the xref:rg:ant:Property/editing.adoc[`Property(editing=...)`] annotation to make property read-only
** likewise xref:rg:ant:Collection/editing.adoc[`@Collection(editing=...)`] for collections
** alternatively, use xref:rg:ant:DomainObject/editing.adoc[`@DomainObject(editing=...)`] to disable editing for all properties/collections
...
* use the xref:rg:ant:Property/regexPattern.adoc[`@Property(regexPattern=...)`] annotation to specify a regex pattern for properties, and use xref:rg:ant:Parameter/regexPattern.adoc[`@Parameter(regexPattern=...)`] for parameters
* use the xref:rg:ant:Property/maxLength.adoc[`@Property(maxLength=...)`] annotation to indicate a maxmum number of characters, and xref:rg:ant:Parameter/maxLength.adoc[`@Parameter(maxLength=...)`] for parameters
...
** use the xref:rg:ant:Property/mustSatisfy.adoc[`Property(mustSatisfy=...)`] and xref:rg:ant:Parameter/mustSatisfy.adoc[`Parameter(mustSatisfy=...)`] annotations to specify arbitrary constraints on properties and parameters
...
* Add the xref:rg:ant:HomePage.adoc[`@HomePage`] annotation to one (no more) of the domain services' no-arg actions
...
** annotate with xref:rg:ant:DomainService/nature.adoc[`@DomainService(nature=NatureOfService.VIEW_CONTRIBUTIONS_ONLY)`]
...
* For this action:
** add the xref:rg:ant:ActionLayout/contributedAs.adoc[`@ActionLayout(contributedAs=ASSOCIATION)`] annotation
...
By default the events generated are `ActionDomainEvent.Default` (for actions) and `PropertyDomainEvent.Default` (for properties). Subclasses of these can be specified using the xref:rg:ant:Action/domainEvent.adoc[`@Action(domainEvent=...)`] or xref:rg:ant:Property/domainEvent.adoc[`Property(domainEvent=...)`] for properties.
...
Actions are specified as being bulk actions using the xref:rg:ant:Action/invokeOn.adoc[`@action(invokeOn=OBJECT_AND_COLLECTION)`] annotation.
...
To support these use cases, Apache Isis therefore allows you to write a view model, either by annotating the class with xref:rg:ant:ViewModel.adoc[`@ViewModel`] or (for more control) by implementing the xref:rg:cms:rgcms.adoc#_rgcms_classes_super_AbstractViewModel[`ViewModel`] interface.
