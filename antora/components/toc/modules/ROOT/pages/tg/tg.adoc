...
* use the injected xref:rg:svc:persistence-layer-api/IsisJdoSupport.adoc[`IsisJdoSupport`] domain service.
...
* the service itself should use xref:rg:svc:core-domain-api/RepositoryService.adoc[`RepositoryService`]
...
* inject xref:rg:svc:core-domain-api/ClockService.adoc[`ClockService`]
...
Using the guidance in the docs for the xref:rg:svc:core-domain-api/EventBusService.adoc[`EventBusService`]:
...
* Inject the xref:rg:svc:application-layer-api/ActionInvocationContext.adoc[`ActionInteractionContext`] (request-scoped) service
...
The similar xref:rg:svc:core-domain-api/Scratchpad.adoc[`Scratchpad`] (request-scoped) domain service is a good way to share information between bulk action invocations:
...
The xref:rg:svc:persistence-layer-api/QueryResultsCache.adoc[`QueryResultsCache`] (request-scoped) domain service allows arbitrary objects to be cached for the duration of a request.
...
Isis has great support for writing xref:ug:tst:integ-test-support.adoc[integration tests]; well-written integration tests should leverage fixture scripts and use the xref:rg:svc:application-layer-api/WrapperFactory.adoc[`@WrapperFactory`] domain service.
