...
https://code.google.com/p/mockito/[Mockito] seems to be the current favourite among Java developers for mocking libraries, but if you use JMock then you'll find we provide a `JUnitRuleMockery2` class and a number of other utility classes, documented xref:ug:tst:unit-test-support.adoc[here].
...
Isis has great support for writing xref:ug:tst:integ-test-support.adoc[integration tests]; well-written integration tests should leverage fixture scripts and use the xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_WrapperFactory[`@WrapperFactory`] domain service.
