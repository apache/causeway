...
Apache Isis' has a pluggable architecture allowing different user interface technologies.
 The principal implementation (as configured by the xref:ug:fun:ugfun.adoc#_ugfun_getting-started_simpleapp-archetype[SimpleApp archetype]) is the xref:ug:vw:ugvw.adoc#[Wicket viewer].
...
Apache Isis' xref:ug:vro:ugvro.adoc#[RestfulObjects viewer] is an implementation of that spec, making any Apache Isis domain object automatically available via REST.
