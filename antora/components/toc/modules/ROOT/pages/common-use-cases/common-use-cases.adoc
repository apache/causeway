...
Apache Isis' has a pluggable architecture allowing different user interface technologies.
 The principal implementation (as configured by the xref:ug:fun:getting-started/simpleapp-archetype.adoc[SimpleApp archetype]) is the xref:ug:vw:about.adoc[Wicket viewer].
