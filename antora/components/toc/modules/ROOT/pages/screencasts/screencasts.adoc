...
Implementing business logic using a (no-arg) action, and associating with a property using either the xref:rg:ant:MemberOrder.adoc[`@MemberOrder`] annotations or using xref:ug:vw:ugvw.adoc#_ugvw_layout_file-based[dynamic (XML) layouts].
|x||||||||x||



|link:https://www.youtube.com/watch?v=0naoVsWppuQ[013^] +
Invoking (no-arg) action on multiple objects at once (using xref:rg:ant:Action/invokeOn.adoc[`@Action#invokeOn()`], and using the xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_ActionInvocationContext[`ActionInvocationContext`] domain service for a smoother end-user experience.
|||x||||||x||
...
Moving the responsibility to specify the icon for a domain object out and into a subscriber, using the xref:rg:cms:rgcms.adoc#_rgcms_classes_uievent_IconUiEvent[`IconUiEvent`] as per the xref:rg:ant:DomainObjectLayout/iconUiEvent.adoc[`@DomainObjectLayout#iconUiEvent()`] annotation
||||||x|||||
...
Using the xref:rg:ant:Title.adoc[`@Title`] annotation (instead of the xref:rg:cms:rgcms.adoc#_rgcms_methods_reserved_title[`title()`] reserved method) to obtain the title of a domain object, so that the end-user can distinguish one object from another.
|x||||||||x||
...
How to write an xref:ug:tst:ugtst.adoc#_ugtst_integ-test-support[integration test] for an xref:rg:ant:Mixin.adoc[mixin].
||x|||||||||
