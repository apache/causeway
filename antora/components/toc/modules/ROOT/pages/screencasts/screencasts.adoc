...
Shows how to refactor a domain object to move an action implementation out of the domain object itself, and instead implement as a xref:ug:fun:building-blocks/types-of-domain-objects/mixins.adoc[mixin] (useful for decoupling).
|x|||||x|||||



|link:https://www.youtube.com/watch?v=m633OEBpWqQ[023^] +
Shows how to refactor a domain object to move (derived) collections out of the domain object and reimplement as a xref:ug:fun:building-blocks/types-of-domain-objects/mixins.adoc[mixin].
||||||x|||||



|link:https://www.youtube.com/watch?v=-AQJb9GtIqI[024^] +
Using a domain event xref:rg:cms:rgcms.adoc#_rgcms_classes_super_AbstractSubscriber[subscriber] to xref:ug:fun:building-blocks/events/domain-events.adoc[decouple] and abstract business rules (xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_validate[validation]).
||||||x|||||
...
How to write a xref:ug:fun:programming-model/view-models/jaxb.adoc[(JAXB) view model], holding references to two domain objects.
|x||||||||x||


|link:https://www.youtube.com/watch?v=tdBkmA2CCZY[053^] +
Updating the title, icon and layout of a xref:ug:fun:programming-model/view-models/jaxb.adoc[(JAXB) view model].
||||x|||||||


|link:https://www.youtube.com/watch?v=TKF0FB_od00[054^] +
How to expose information from underlying domain entities referenced by a xref:ug:fun:programming-model/view-models/jaxb.adoc[(JAXB) view model] using derived properties
||||||||||x|


|link:https://www.youtube.com/watch?v=TMMeVhRNi8A[055^] +
Adding an action to a xref:ug:fun:programming-model/view-models/jaxb.adoc[(JAXB) view model], and adjusting the layout using custom CSS.
||||x|||||||
