...
Walking through the main classes that make up the SimpleApp: `SimpleObject` persistent entity, `HomePageViewModel` xref:ug:fun:view-models.adoc[view model] and `SimpleObjects` domain service (repository).
||||x|||||||
