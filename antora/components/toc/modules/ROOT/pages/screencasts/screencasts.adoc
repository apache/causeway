...
Using the xref:rg:cms:methods/reserved/title.adoc[`title()`], xref:rg:cms:methods/reserved/iconName.adoc[`iconName()`] and xref:rg:cms:methods/reserved/cssClass.adoc[`cssClass()`] so that end-users can distinguish domain objects within the UI.
|x||x||||||x||
...
Using a domain event xref:rg:cms:classes/super/AbstractSubscriber.adoc[subscriber] to xref:ug:fun:building-blocks/events/domain-events.adoc[decouple] and abstract business rules (xref:rg:cms:methods/prefixes/validate.adoc[validation]).
||||||x|||||



|link:https://www.youtube.com/watch?v=6GjLW0hlrm4[025^] +
Using a domain event xref:rg:cms:classes/super/AbstractSubscriber.adoc[subscriber] to hide functionality, in this
...
Using the xref:rg:ant:Title.adoc[`@Title`] annotation (instead of the xref:rg:cms:methods/reserved/title.adoc[`title()`] reserved method) to obtain the title of a domain object, so that the end-user can distinguish one object from another.
|x||||||||x||
...
How to validate action parameters using a supporting xref:rg:cms:methods/prefixes/validate.adoc[`validateNXxx()`] method.
|x||||||||x||
...
How to provide a set of xref:rg:cms:methods/prefixes/choices.adoc[choices] (a drop-down list) when editing a property.
|x||||||||x||



|link:https://www.youtube.com/watch?v=afEnYKljBQs[034^] +
How to provide a set of xref:rg:cms:methods/prefixes/choices.adoc[choices] (a drop-down list) when invoking an action.
|||||||||x||
...
Using the xref:rg:cms:classes/super/FixtureScript.adoc[`FixtureScript`] `defaultParam(...)` method to reflectively default parameters to fixture scripts that have not been set by the caller.
||x|||||||||
...
How to use the supporting xref:rg:cms:methods/prefixes/default.adoc[`defaultXxx(...)`] supporting method to provide a default argument value for action parameters.
|x||||||||x||
...
How to use the xref:rg:cms:methods/prefixes/choices.adoc[`choicesXxx(...)`] supporting method to provide a drop-down list for parameters to actions that are for reference types (domain entities or view models).
|||||||||x||


|link:https://www.youtube.com/watch?v=K36IJQ_hDfs[046^] +
How to use the xref:rg:cms:methods/prefixes/autoComplete.adoc[`autoCompleteXxx(...)`] supporting method to provide a drop-down list for parameters to actions that are for reference types (domain entities or view models).
|||||||||x||
