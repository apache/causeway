...
Using the xref:rg:cms:methods/reserved.adoc#title[`title()`], xref:rg:cms:methods/reserved.adoc#iconName[`iconName()`] and xref:rg:cms:methods/reserved.adoc#cssClass[`cssClass()`] so that end-users can distinguish domain objects within the UI.
|x||x||||||x||
...
Using a domain event xref:rg:cms:classes/super.adoc#AbstractSubscriber[subscriber] to xref:ug:fun:building-blocks.adoc#domain-events[decouple] and abstract business rules (xref:rg:cms:methods/prefixes.adoc#validate[validation]).
||||||x|||||
...
Using the xref:rg:ant:Title.adoc[`@Title`] annotation (instead of the xref:rg:cms:methods/reserved.adoc#title[`title()`] reserved method) to obtain the title of a domain object, so that the end-user can distinguish one object from another.
|x||||||||x||
...
How to validate action parameters using a supporting xref:rg:cms:methods/prefixes.adoc#validate[`validateNXxx()`] method.
|x||||||||x||
...
How to provide a set of xref:rg:cms:methods/prefixes.adoc#choices[choices] (a drop-down list) when editing a property.
|x||||||||x||



|link:https://www.youtube.com/watch?v=afEnYKljBQs[034^] +
How to provide a set of xref:rg:cms:methods/prefixes.adoc#choices[choices] (a drop-down list) when invoking an action.
|||||||||x||



|link:https://www.youtube.com/watch?v=fKo6aTPK-gk[035^] +
How to use the xref:rg:cms:methods/prefixes.adoc#choices[choices] supporting methods as a source for default values within a xref:testing:ROOT:fixture-scripts.adoc[fixture script].
||x|||||||x||
...
How to use the supporting xref:rg:cms:methods/prefixes.adoc#default[`defaultXxx(...)`] supporting method to provide a default argument value for action parameters.
|x||||||||x||
...
How to use the xref:rg:cms:methods/prefixes.adoc#choices[`choicesXxx(...)`] supporting method to provide a drop-down list for parameters to actions that are for reference types (domain entities or view models).
|||||||||x||


|link:https://www.youtube.com/watch?v=K36IJQ_hDfs[046^] +
How to use the xref:rg:cms:methods/prefixes.adoc#autoComplete[`autoCompleteXxx(...)`] supporting method to provide a drop-down list for parameters to actions that are for reference types (domain entities or view models).
|||||||||x||
