...
Shows how to refactor a domain object to move an action implementation out of the domain object itself, and instead implement as a xref:ug:fun:building-blocks.adoc#mixins[mixin] (useful for decoupling).
|x|||||x|||||



|link:https://www.youtube.com/watch?v=m633OEBpWqQ[023^] +
Shows how to refactor a domain object to move (derived) collections out of the domain object and reimplement as a xref:ug:fun:building-blocks.adoc#mixins[mixin].
||||||x|||||



|link:https://www.youtube.com/watch?v=-AQJb9GtIqI[024^] +
Using a domain event xref:rg:cms:classes/super/AbstractSubscriber.adoc[subscriber] to xref:ug:fun:building-blocks.adoc#domain-events[decouple] and abstract business rules (xref:rg:cms:methods/prefixes/validate.adoc[validation]).
||||||x|||||
