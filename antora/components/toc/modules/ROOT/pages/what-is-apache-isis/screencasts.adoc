...
Importing the generated app into an xref:toc:devguide:ide.adoc[IDE] +
...
How to write a xref:ug:fun:programming-model.adoc#jaxb[(JAXB) view model], holding references to two domain objects.
|x||||||||x||


|link:https://www.youtube.com/watch?v=tdBkmA2CCZY[053^] +
Updating the title, icon and layout of a xref:ug:fun:programming-model.adoc#jaxb[(JAXB) view model].
||||x|||||||


|link:https://www.youtube.com/watch?v=TKF0FB_od00[054^] +
How to expose information from underlying domain entities referenced by a xref:ug:fun:programming-model.adoc#jaxb[(JAXB) view model] using derived properties
||||||||||x|


|link:https://www.youtube.com/watch?v=TMMeVhRNi8A[055^] +
Adding an action to a xref:ug:fun:programming-model.adoc#jaxb[(JAXB) view model], and adjusting the layout using custom CSS.
||||x|||||||
