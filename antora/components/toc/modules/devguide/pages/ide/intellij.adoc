...
then make sure you have correctly configured the xref:toc:devguide:ide/intellij.adoc#other-settings-compiler[annotation processor]
...
Running an Apache Isis application requires that the DataNucleus enhancer runs on the compiled bytecode.  As described xref:toc:devguide:ide/intellij.adoc#running_the-app[above], the recommended way to do this with IntelliJ is to use a Run configuration that runs the enhancer goal prior to launch.
