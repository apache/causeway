...
We provide a set of editor templates.  These are used to add new methods to existing classes.  (These are equivalent to the xref:toc:devguide:ide/intellij.adoc#live-templates[IntelliJ live templates]):
...
For details of setting up DCEVM, see the xref:toc:devguide:ide/intellij.adoc#advanced_dcevm[corresponding section] in the IntelliJ documentation.
