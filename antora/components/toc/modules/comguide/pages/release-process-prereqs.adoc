...
This section (appendix) describes the prerequisites for the xref:toc:comguide:about.adoc#release-process[release process].
...
Ensure that Java 7 is installed and the toolchains plugin is configured, as described in the xref:toc:devguide:building-apache-isis.adoc#configure-maven-toolchains-plugin[developers' guide].
...
See the page on xref:toc:comguide:about.adoc#key-generation[key generation] for more details.
