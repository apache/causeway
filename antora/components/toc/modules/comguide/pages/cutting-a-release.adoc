...
* the release manager performing post-release tasks, for either a xref:toc:comguide:post-release-successful.adoc[successful] or an xref:toc:comguide:post-release-unsuccessful.adoc[unsuccessful] vote.
...
The subsequent sections describe how other committers can xref:toc:comguide:verifying-releases.adoc[verify a release] and how the release manager can then perform xref:toc:comguide:post-release.adoc[post-release] activities and set up for the next development iteration.

If you've not performed a release before, then note that there are some configuration xref:toc:comguide:release-process-prereqs.adoc[prerequisites] that must be configured first.
...
This should be in line with our xref:toc:comguide:versioning-policy.adoc[semantic versioning policy].
...
Obviously, alter ``$ISISREL`` as required, and bump ``$ISISRC`` for re-releasing following an xref:toc:comguide:post-release-unsuccessful.adoc[unsuccessful] release.
...
[NOTE]
====
Unfortunately, Nexus does not seem to allow subkeys to be used for signing. See xref:toc:comguide:key-generation.adoc[Key Generation] for more details.
====
...
Note that the email also references the procedure for other committers to xref:toc:comguide:verifying-releases.adoc[verify the release].
