...
The file templates are provided as a settings JAR file, namely *link:intellij/isis-settings-file-templates.jar[isis-settings-file-templates.jar]*.  Download this file.
...
//* `ad` : for Asciidoc documentation; a full list can be found in the xref:support:dg:asciidoc-templates.adoc[appendix].
...
provided as a settings file, namely *link:intellij/isis-settings-code-style.jar[isis-settings-code-style.jar]*.  Download and import (as for the above settings JAR files).
...
for frameworks such as xref:support:dg:project-lombok.adoc[Project Lombok].
...
then make sure you have correctly configured the xref:support:dg:ide/intellij.adoc#other-settings-compiler[annotation processor]
...
Running an Apache Isis application requires that the DataNucleus enhancer runs on the compiled bytecode.  As described xref:support:dg:ide/intellij.adoc#running_the-app[above], the recommended way to do this with IntelliJ is to use a Run configuration that runs the enhancer goal prior to launch.
