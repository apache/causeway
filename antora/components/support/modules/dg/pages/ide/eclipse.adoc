...
We provide a set of editor templates.  These are used to add new methods to existing classes.  (These are equivalent to the xref:support:dg:ide/intellij.adoc#live-templates[IntelliJ live templates]):
...
The xref:ug:fun:getting-started/simpleapp-archetype.adoc[SimpleApp archetype] uses link:http://projectlombok.org[Project Lombok] annotations (`@Getter` and `@Setter` and so on) to reduce the boilerplate.
...
See the section in xref:support:dg:hints-and-tips/datanucleus-enhancer.adoc[DataNucleus enhancer] for details of the contents of the `pom.xml`.
Chances are it is already set up from running the xref:ug:fun:getting-started/helloworld-archetype.adoc[HelloWorld] or the xref:ug:fun:getting-started/simpleapp-archetype.adoc[SimpleApp] archetype.
...
For details of setting up DCEVM, see the xref:support:dg:ide/intellij.adoc#advanced_dcevm[corresponding section] in the IntelliJ documentation.
