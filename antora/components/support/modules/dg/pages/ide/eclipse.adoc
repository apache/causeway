...
The xref:arch:simpleapp:about.adoc[SimpleApp archetype] uses link:http://projectlombok.org[Project Lombok] annotations (`@Getter` and `@Setter` and so on) to reduce the boilerplate.
...
Chances are it is already set up from running the xref:helloworld:ROOT:about.adoc[HelloWorld] or the xref:arch:simpleapp:about.adoc[SimpleApp] archetype.
