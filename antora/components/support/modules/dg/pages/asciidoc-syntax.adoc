...
We have now switched to using https://antora.org[Antora], which is a static doc site generator that builds upon Asciidoctor.
Antora has its own directory/file conventions, which supercede the file name conventions described here.
===
...
|....
xref:support:dg:asciidoc/links-and-xrefs#.adoc[background]
....
|xref:support:dg:asciidoc/links-and-xrefs.adoc[background]
...
For example, this xref:support:dg:about.adoc[developers' guide] document is broken into several files:
