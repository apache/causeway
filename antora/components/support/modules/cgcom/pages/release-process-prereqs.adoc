...
This section (appendix) describes the prerequisites for the xref:support:cgcom:release-process.adoc[release process].
...
Ensure that Java 7 is installed and the toolchains plugin is configured, as described in the xref:support:dg:building-apache-isis.adoc#configure-maven-toolchains-plugin[developers' guide].
...
See the page on xref:support:cgcom:key-generation.adoc[key generation] for more details.
