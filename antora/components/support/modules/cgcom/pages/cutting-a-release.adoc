...
* Members of the Apache Isis PMC xref:support:cgcom:verifying-releases.adoc[verifying] and voting on the release
...
The subsequent sections describe how other committers can xref:support:cgcom:verifying-releases.adoc[verify a release] and how the release manager can then perform xref:support:cgcom:post-release.adoc[post-release] activities and set up for the next development iteration.

If you've not performed a release before, then note that there are some configuration xref:support:cgcom:release-process-prereqs.adoc[prerequisites] that must be configured first.
...
This should be in line with our xref:support:cgcom:versioning-policy.adoc[semantic versioning policy].
...
(One should exist already, xref:support:cgcom:post-release-successful.adoc#create-new-jira[created at] the beginning of the development cycle now completing).
<3> adjust as required (Tomcat is used to smoke test the simpleapp archetype)

Obviously, alter ``$ISISREL`` as required, and bump ``$ISISRC`` for re-releasing following an xref:support:cgcom:post-release-unsuccessful.adoc[unsuccessful] release.
...
After release, there is a xref:support:cgcom:cutting-a-release.adoc#reset-revision[similar step at the end] to reset back to `1.0.0-SNAPSHOT`.
...
[NOTE]
====
Unfortunately, Nexus does not seem to allow subkeys to be used for signing. See xref:support:cgcom:key-generation.adoc[Key Generation] for more details.
====
...
Note that the email also references the procedure for other committers to xref:support:cgcom:verifying-releases.adoc[verify the release].
