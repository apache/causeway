...
You can verify the binary releases by configuring your local Maven install to point to the Maven staging repository (or repositories) and then using them, eg to run the xref:arch:helloworld:about.adoc[HelloWorld archetype] or the xref:arch:simpleapp:about.adoc[SimpleApp archetype] and running the resultant app.
