...
* Members of the Apache Isis PMC xref:support:cgcom:cgcom.adoc#_cgcom_verifying-releases[verifying] and voting on the release
* the release manager performing post-release tasks, for either a xref:support:cgcom:cgcom.adoc#_cgcom_post-release-successful[successful] or an xref:support:cgcom:cgcom.adoc#_cgcom_post-release-unsuccessful[unsuccessful] vote.
...
The subsequent sections describe how other committers can xref:support:cgcom:cgcom.adoc#_cgcom_verifying-releases[verify a release] and how the release manager can then perform xref:support:cgcom:cgcom.adoc#_cgcom_post-release[post-release] activities and set up for the next development iteration.

If you've not performed a release before, then note that there are some configuration xref:support:cgcom:cgcom.adoc#_cgcom_release-process-prereqs[prerequisites] that must be configured first.
...
This should be in line with our xref:support:cgcom:cgcom.adoc#_cgcom_versioning-policy[semantic versioning policy].
...
(One should exist already, xref:support:cgcom:cgcom.adoc#__cgcom_post-release-successful_update-jira_create-new-jira[created at] the beginning of the development cycle now completing).
<3> adjust as required (Tomcat is used to smoke test the simpleapp archetype)

Obviously, alter ``$ISISREL`` as required, and bump ``$ISISRC`` for re-releasing following an xref:support:cgcom:cgcom.adoc#_cgcom_post-release-unsuccessful[unsuccessful] release.
...
[NOTE]
====
Unfortunately, Nexus does not seem to allow subkeys to be used for signing. See xref:support:cgcom:cgcom.adoc#_cgcom_key-generation[Key Generation] for more details.
====
...
Note that the email also references the procedure for other committers to xref:support:cgcom:cgcom.adoc#_cgcom_verifying-releases[verify the release].
