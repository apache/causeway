...
Whereas xref:support:cgcom:cutting-a-release.adoc[formal release]s and xref:support:cgcom:release-process-for-snapshots.adoc[snapshot release]s are public (released through the Maven repository maintained by Apache Software Foundation), interim releases are non-public and rely on infrastructure provided by a developer team.
