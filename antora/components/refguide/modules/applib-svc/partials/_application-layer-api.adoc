...
|Access to the current xref:refguide:applib:index/services/iactnlayer/InteractionLayer.adoc[InteractionLayer], where the lowest layer is in effect a short-lived session connected to the database, with a xref:refguide:applib:index/services/xactn/TransactionService.adoc#currentTransactionId__[transaction] started automatically.
