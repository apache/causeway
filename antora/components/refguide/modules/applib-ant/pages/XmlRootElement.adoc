...
The `@XmlRootElement` annotation provides an alternative way to define a xref:userguide:ROOT:overview.adoc#view-models[view model], in particular one intended to act as a DTO for use within xref:vro:ROOT:about.adoc[RestfulObjects viewer], or which contains arbitrarily complex state.
...
Behaviour can therefore be provided using xref:userguide:ROOT:overview.adoc#mixins[mixins] (annotated with xref:refguide:applib:index/annotation/DomainObject.adoc[@DomainObject(nature=MIXIN)]), while xref:applib-classes:events.adoc#ui-event-classes[UI events] can be used to obtain title, icons and so on.

For a more complete discussion of writing JAXB view models/DTOs, see xref:userguide:ROOT:overview.adoc#view-models[this topic] in the user guide.
