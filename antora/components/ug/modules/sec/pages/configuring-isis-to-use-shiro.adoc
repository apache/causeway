...
[TIP]
====
Both the xref:arch:helloworld:about.adoc[HelloWorld] and xref:arch:simpleapp:about.adoc[SimpleApp] archetypes are pre-configured to use Apache Shiro, so much of what follows may well have been set up already.
====
...
For example, the xref:arch:helloworld:about.adoc[HelloWorld archetype] bootstraps using:
