...
By and large the security mechanisms within Isis are transparent to the rest of the framework (the xref:ug:vw:about.adoc[Wicket Viewer] and xref:ug:vro:about.adoc[Restful Objects viewer], and the overall xref:rg:cfg:rgcfg.adoc#_rgcfg[runtime]).
...
The xref:ug:vw:about.adoc[Wicket viewer] defines a relatively small number of pages (by which we mean subclasses of link:http://ci.apache.org/projects/wicket/apidocs/6.0.x/org/apache/wicket/markup/html/WebPage.html[`org.apache.wicket.markup.html.WebPage`]):
