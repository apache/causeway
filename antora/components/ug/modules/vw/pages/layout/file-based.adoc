...
It is also possible to download an initial `.layout.xml` - capturing any existing layout metadata - using the xref:rg:svc:rgsvc.adoc#_rgsvc_metadata-api_LayoutService[`LayoutService`] (exposed on the prototyping menu) or using a xref:rg:cms:classes/mixins/Object.adoc[mixin action] contributed to every domain object.
...
One way in which the domain object can specify an alternate layout is through its xref:rg:cms:methods/reserved/layout.adoc[`layout()`] method.
...
Alternatively the layout XML for a single domain object can be downloaded using the xref:rg:cms:classes/mixins/Object.adoc[mixin action] (contributed to every domain object).
...
* xref:rg:svc:rgsvc.adoc#_rgsvc_metadata-api_LayoutService[`LayoutService`] (whose functionality is exposed on the prototyping menu as an action) and lso the a xref:rg:cms:classes/mixins/Object.adoc[mixin action]
