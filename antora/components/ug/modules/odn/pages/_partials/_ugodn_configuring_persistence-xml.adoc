...
Here's the one provided by the xref:ug:fun:ugfun.adoc#_ugfun_getting-started_simpleapp-archetype[SimpleApp archetype]:
...
[TIP]
====
If you use Eclipse IDE on Windows then xref:support:dg:dg.adoc#__dg_ide_eclipse_workaround-for-path-limits[note the importance] of the `persistence.xml` file to make DataNucleus enhancer work correctly.
====
