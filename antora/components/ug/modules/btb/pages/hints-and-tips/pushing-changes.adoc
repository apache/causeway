...
[NOTE]
====
This technique is much less powerful than using xref:ug:fun/building-blocks/events/domain-events.adoc[the event bus] or an SPI service.
...
<3> the xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_addTo[`addTo...()`] method increments the derived properties
<4> the xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_removeFrom[`removeFrom...()`] method similarly decrements the derived properties
