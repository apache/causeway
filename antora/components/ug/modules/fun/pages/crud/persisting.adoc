...
When an object is persisted the framework will emit `ObjectPersistingEvent` and `ObjectPersistedEvent` xref:ug:fun:building-blocks/events/lifecycle-events.adoc[lifecycle events].
