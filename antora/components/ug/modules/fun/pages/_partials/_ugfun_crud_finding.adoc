...
This repository calls the framework-provided xref:../rgsvc/rgsvc.adoc#_rgsvc_persistence-layer-api_RepositoryService[`RepositoryService`] to actually submit the query.
...
----
<1>	The xref:../rgsvc/rgsvc.adoc#_rgsvc_persistence-layer-api_RepositoryService[`RepositoryService`] is a generic facade over the JDO/DataNucleus API.
...
DataNucleus also supports type-safe queries; see xref:../rgsvc/rgsvc.adoc#__rgsvc_persistence-layer-api_IsisJdoSupport_type-safe-jdoql-queries[here] for further details.
