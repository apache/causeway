...
One decoupled approach is using xref:ug:fun:building-blocks/events/domain-events.adoc[domain events]; the checks above correspond to the phases of the domain event.
...
Properties are similar (note that collections are implicitly read-only in the xref:ug:vw:about.adoc[Wicket viewer]:
