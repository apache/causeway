...
MetaWidget has a number of ideas in common with Apache Isis (we compare Apache Isis' with MetaWidget xref:ug:fun:core-concepts/principles/apache-isis-vs/metawidget.adoc[here]), in particular the runtime generation of a UI for domain objects.
