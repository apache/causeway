...
Here, the presentation layer (xref:ug:vw:ugvw.adoc#[Wicket UI] or xref:ug:vro:about.adoc[REST API]) is handled by the framework, while the developer focusses on just the domain layer.
