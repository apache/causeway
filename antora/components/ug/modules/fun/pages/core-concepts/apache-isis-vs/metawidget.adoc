...
MetaWidget (mentioned xref:ug:fun:core-concepts/philosophy/naked-objects-pattern/object-interface-mapping.adoc[earlier] has a number of ideas in common with Apache Isis, specifically the runtime generation of a UI for domain objects.
...
In contrast, Apache Isis is full-stack and does generate a complete UI; we then allow you to customize or extend this UI (as per the various (non-ASF) link:http://platform.incode.org[Incode Platform^] modules), and we also provide a full REST API through the xref:ug:vro:about.adoc[Restful Objects viewer]
