...
More sophisticated UIs can be built either by xref:ug:vw:extending.adoc[extending Apache Isis' Wicket viewer] or by writing a bespoke UI leveraging the REST (hypermedia) API automatically exposed by xref:ug:vro:ugvro.adoc#[Isis' Restful Objects viewer].
