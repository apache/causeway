...
In Apache Isis this same separation of business logic from the underlying data can be accomplished most straightforwardly using xref:ug:fun:building-blocks/types-of-domain-objects/mixins.adoc[mixins] or xref:ug:fun:how-tos/contributed-members.adoc[contributions].
In the UI (surfaced by the xref:ug:vw:ugvw.adoc#[Wicket viewer]) or in the REST API (surfaced by the xref:ug:vro:about.adoc[RestfulObjects viewer]) the behaviour appears to reside on the domain object; however the behaviour actually resides on separate classes and is mixed in (like a trait) only at runtime.
