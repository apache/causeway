...
Or, the CQRS architecture can be more fully implemented with Apache Isis by introducing a separate read model, synchronized using the xref:rg:svc:rgsvc.adoc#_rgsvc_spi_PublisherService[`PublisherService`], or using xref:rg:cms:classes/super/AbstractSubscriber.adoc[subscribers]  on the xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_EventBusService[`EventBusService`].
