...
The easiest way to register domain services with the framework is to use an xref:rg:cms:rgcms.adoc#_rgcms_classes_AppManifest-bootstrapping[`AppManifest`].
This specifies the modules which contain xref:rg:ant:rgant.adoc#_rgant-DomainService[`@DomainService`]-annotated classes.
...
Examples of framework-provided services (as defined in the applib) include clock, auditing, publishing, exception handling, view model support, snapshots/mementos, and user/application settings management; see the xref:rg:svc:rgsvc.adoc#[domain services] reference guide for further details.
