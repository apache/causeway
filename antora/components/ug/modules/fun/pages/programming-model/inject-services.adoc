...
When there are multiple service implementations of a given type, the framework will inject the service with highest priority, as defined through xref:rg:ant:DomainService/menuOrder.adoc[`@DomainService#menuOrder()`] (even for domain services that are not menus), lowest first.
