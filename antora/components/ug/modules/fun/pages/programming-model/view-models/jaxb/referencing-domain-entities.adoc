...
For example, the `CompareCustomers` view model described xref:ug:fun:programming-model/view-models/jaxb.adoc[above] actually references two underlying ``Customer`` entities.

It wouldn't make sense to serialize out the state of a persistent entity.
However, the identity of the underlying entity is well defined; Apache Isis defines the xref:rg:cms:schema/common.adoc[common schema] which defines the `<oid-dto>` element (and corresponding `OidDto` class): the object's type and its identifier.
