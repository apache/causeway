...
In the section xref:ug:fun:ugfun.adoc#_ugfun_programming-model_view-models_jaxb_referencing-domain-entities[above] it was explained how a view model DTO can transparent reference any "backing" entities; these references are converted to internal object identifiers.
...
The xref:rg:svc:rgsvc.adoc#_rgsvc_integration-api_BookmarkService[`BookmarkService`] can be used to obtain the object identifiers
...
The `isis-xsd` profile generates the XSD using the xref:rg:mvn:rgmvn.adoc#_rgmvn_xsd[`xsd` goal] of Isis' maven plugin:
