...
A property is an instance variable of a domain object, of a scalar type, that holds some state about either a xref:ug:fun:programming-model/class-definition/entities.adoc[domain entity] or a xref:ug:fun:programming-model/class-definition/view-models.adoc[view model].
...
It's ok for a xref:ug:fun:programming-model/class-definition/entities.adoc[domain entity] to reference another domain entity, and for a xref:ug:fun:programming-model/class-definition/view-models.adoc[view model] to reference both view model and domain entities.
...
By default Apache Isis will automatically render all properties in the xref:ug:vw:ugvw.adoc#[Wicket UI] or in the xref:ug:vro:about.adoc[REST API].
...
As for xref:ug:fun:programming-model/properties/datatypes/joda-dates.adoc[Joda dates], this requires the `@javax.jdo.annotations.Persistent` annotation.
...
Mapping `Clob`s works in a very similar way to xref:ug:fun:programming-model/properties/datatypes/blobs.adoc[``Blob``]s, but the `jdbcType` and `sqlType` attributes will, respectively, be `CLOB` and `LONGVARCHAR`:
