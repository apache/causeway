...
Unlike xref:ug:fun:programming-model/properties.adoc[properties], the framework (at least, the xref:ug:vw:about.adoc[Wicket viewer]) does not allow collections to be "edited".
...
If you want to visualize an array of value types in Apache Isis, then one option is to wrap value in a view model, as explained xref:ug:btb:hints-and-tips/simulating-collections-of-values.adoc[elsewhere].
