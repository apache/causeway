...
Unlike xref:ug:fun:programming-model/properties.adoc[properties], the framework (at least, the xref:ug:vw:ugvw.adoc#[Wicket viewer]) does not allow collections to be "edited".
Instead, xref:ug:fun:programming-model/actions.adoc[action]s can be written that will modify the contents of the collection as a side-effect.
