...
For example, the xref:arch:helloworld:about.adoc[Hello World archetype] has the following layout:
...
For example, in the xref:arch:helloworld:about.adoc[Hello World archetype] the `HelloWorld` domain object has a layout defined by `HelloWorld.layout.xml`.
