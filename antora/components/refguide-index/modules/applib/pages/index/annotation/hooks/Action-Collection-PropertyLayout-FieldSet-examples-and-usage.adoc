...
It corresponds to a `<fieldset>` if using xref:userguide:ROOT:ui.adoc#object-layout[layout files].
...
TIP: It's possible to combine `@PropertyLayout` with xref:userguide:ROOT:ui.adoc#object-layout[layout file].
