...
For example, a `Customer` object with an xref:refguide:applib:index/annotation/DomainObject.adoc#logicalTypeName[logical type name] of "custmgmt.Customer" and an id of 123 would correspond to a xref:BookmarkService.adoc#bookmark[Bookmark] with a string representation of "custmgmt.Customer|123".
