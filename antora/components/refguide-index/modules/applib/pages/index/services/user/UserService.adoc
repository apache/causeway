...
+
--
Returns the details about the current user, either the "effective" user (if being _#impersonateUser(String, List, String) impersonated_ ) otherwise the "real" user (as obtained from the xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] of the current thread).
--
<.> xref:#isCurrentUserWithSudoAccessAllRole_[isCurrentUserWithSudoAccessAllRole()]
+
--
Whether the current user is the _system user_ (as obtained from the xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] of the current thread).
--
