...
interface TableColumnOrderService {
  List<String> orderParented(Class<?> parentType, String collectionId, Class<?> elementType, List<String> associationIds)     // <.>
  List<String> orderStandalone(Class<?> domainType, List<String> associationIds)     // <.>
}
----

<.> xref:#orderParented_Class_String_Class_List[orderParented(Class, String, Class, List)]
+
--
For the parented collection owned by the specified parent and collection Id, return a list of association ids to be rendered as columns, in a particular order; those not included will be hidden.
--
...
[#orderParented_Class_String_Class_List]
=== orderParented(Class, String, Class, List)
