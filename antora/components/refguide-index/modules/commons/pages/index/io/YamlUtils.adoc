...
class YamlUtils {
  ...
  Try<List<T>> tryReadAsListCustomized(Class<T> elementType, DataSource source, YamlLoadCustomizer loadCustomizer, JsonUtils.JacksonCustomizer... customizers)     // <.>
  Try<Stream<String>> tryReadMultiDoc(DataSource source)     // <.>
  ...
  String writeMultiDoc(Iterable<String> yamlDocuments)     // <.>
  String writeMultiDoc(Stream<String> yamlDocumentStream)     // <.>
...
+
--
Tries to deserialize YAML content from given xref:refguide:commons:index/io/DataSource.adoc[DataSource] into a _List_ with given _elementType_ .
--
...
Tries to deserialize YAML content from given xref:refguide:commons:index/io/DataSource.adoc[DataSource] into an instance of given _requiredType_ .
--
<.> xref:#tryReadAsListCustomized_Class_DataSource_YamlLoadCustomizer_JsonUtils_JacksonCustomizer[tryReadAsListCustomized(Class, DataSource, YamlLoadCustomizer, JsonUtils_JacksonCustomizer)]
+
--
Tries to deserialize YAML content from given xref:refguide:commons:index/io/DataSource.adoc[DataSource] into a _List_ with given _elementType_ .
--
<.> xref:#tryReadMultiDoc_DataSource[tryReadMultiDoc(DataSource)]
+
--
Returns a _Stream_ of (arbitrary) YAML Document junks from provided xref:refguide:commons:index/io/DataSource.adoc[DataSource] , where each junk typically needs further parsing individually.
--
...
Converts given _pojo_ to an UTF8 encoded _String_ .
--
<.> xref:#writeMultiDoc_Iterable[writeMultiDoc(Iterable)]
+
--
Concatenates given _Iterable_ into a multi-doc YAML.
--
<.> xref:#writeMultiDoc_Stream[writeMultiDoc(Stream)]
+
--
Concatenates given _Stream_ into a multi-doc YAML.
--
...
[#tryReadAsListCustomized_Class_DataSource_YamlLoadCustomizer_JsonUtils_JacksonCustomizer]
=== tryReadAsListCustomized(Class, DataSource, YamlLoadCustomizer, JsonUtils_JacksonCustomizer)

Tries to deserialize YAML content from given xref:refguide:commons:index/io/DataSource.adoc[DataSource] into a _List_ with given _elementType_ .

Either parses (regular) YAML-list format or multi-doc YAML format.

[#tryReadMultiDoc_DataSource]
=== tryReadMultiDoc(DataSource)
...
[#writeMultiDoc_Iterable]
=== writeMultiDoc(Iterable)

Concatenates given _Iterable_ into a multi-doc YAML.

[#writeMultiDoc_Stream]
=== writeMultiDoc(Stream)
