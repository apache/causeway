...
Implements the HTTP Basic Auth protocol; does not bind the xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] onto the _HttpSession_ .
...
+
--
This implementation is stateless and so does not support binding the xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] (aka authentication) into a store (eg a session); instead each request is authenticated afresh.
--
