...
Checks that an already-present xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] (obtained from the _HttpSession_ ) is still _org.apache.causeway.core.security.authentication.manager.AuthenticationManager#isSessionValid(InteractionContext) valid_ , and re-binds the xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] onto the _HttpSession_ .

Note that this implementation is not particularly "restful"; normally REST APIs are expected to be stateless whereas this implementation requires a session to obtain the xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] . Typically it would be combined with Shiro, whose default behaviour (not suppressed by this filter) is indeed to store the xref:refguide:applib:index/services/iactn/InteractionContext.adoc[InteractionContext] on the session.
