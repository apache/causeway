...
--
<.> xref:#supportsInput_[supportsInput()]
+
--
...
The default is _false_ : registering output coercion never implicitly grants input capability. Reversible marshallers must opt in explicitly so unsupported inputs are rejected before domain invocation.
