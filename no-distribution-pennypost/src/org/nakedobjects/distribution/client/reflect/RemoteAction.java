...
public final class RemoteAction implements Action {
    ...
    public RemoteAction(final Action local, final SessionId securityToken, final RemoteObjectFactory factory, final DistributionInterface connection, final LoadedObjects loadedObjects) {
    	this.local = local;
    ...
    public NakedObject execute(NakedObject object, Naked[] parameters) {
        if (isPersistent(object)) {
            ObjectReference objectReference = factory.createObjectReference(object);
            ParameterSet parameterSet = factory.createParameterSet(parameters);
            ActionType type = factory.createActionType(getType());
            ObjectData graph = connection.executeAction(securityToken, objectReference, type, getName(), parameterSet);
            return graph.recreateObject(loadedObjects, object.getContext());
        } else {
    ...
    public About getAbout(Session session, NakedObject object, Naked[] parameters) {
        if (isPersistent(object) && fullProxy) {
            ...
            ParameterSet parameterSet = factory.createParameterSet(parameters);
            ActionType type = factory.createActionType(getType());
            AboutData aboutData = connection.aboutAction(securityToken, objectReference, type, getName(), parameterSet);
            return aboutData.recreateAbout();
        } else {
        	return local.getAbout(session, object, parameters);
        }
    }
