...
The `xsd` goal of the `isis-maven-plugin` uses the xref:refguide:applib-svc:JaxbService.adoc[`JaxbService`] to
...
+
Defaults to `false`; if set then the call to xref:refguide:applib-svc:JaxbService.adoc[`JaxbService`] will set `IsisSchemas.INCLUDE` flag.
