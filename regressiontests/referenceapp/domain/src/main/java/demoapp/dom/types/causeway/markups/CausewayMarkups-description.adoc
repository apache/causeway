...
The framework has built-in support for arbitrary HTML markup, using the `Markup` data type.
...
On the left hand side is a collection of entities that uses the `Markup` datatype.
There is also an action that opens a view model; this view model also uses the `Markup` datatype.
