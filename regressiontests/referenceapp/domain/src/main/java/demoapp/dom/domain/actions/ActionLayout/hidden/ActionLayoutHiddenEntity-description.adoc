...
attribute allows for rendering in Tables (specifically STANDALON and/or PARENTED depending on context). 
...
----
<.> action should _NOT_ be visible for this entity in object forms, but in tables.

What happens after Action invocation (where to redirect to), is governed by the Action's return type:

. `void` -> stay on current Page
. if return type is `T` and `T` is also the object the Action is declared on or mixed in to -> stay on current Page
