...
public class MarkupReaderService {
    ...
    public Markup readFor(final Class<?> aClass) {
        var markupResourceName = String.format("%s.html", aClass.getSimpleName());
        var html = resourceReaderService.readResource(aClass, markupResourceName);
        return new Markup(html);
    }

    public Markup readFor(final Class<?> aClass, final String member) {
        var markupResourceName = String.format("%s-%s.html", aClass.getSimpleName(), member);
        var html = resourceReaderService.readResource(aClass, markupResourceName);
        return new Markup(html);
    }
