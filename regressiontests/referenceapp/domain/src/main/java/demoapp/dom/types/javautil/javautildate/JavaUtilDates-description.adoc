...
Rather than use this data type, consider using a more modern class such as `java.time.LocalDate`, `java.time.LocalDateTime` or `java.time.OffsetDateTime`.

== How this demo works

On the left hand side is a collection of entities that uses the `Date` datatype.
There is also an action that opens a view model; this view model also uses the `Date` datatype.
