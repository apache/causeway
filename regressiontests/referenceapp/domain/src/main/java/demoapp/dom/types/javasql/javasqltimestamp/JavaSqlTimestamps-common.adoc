...
A number of mixin actions contribute through the `JavaSqlTimestampHolder` interface, demonstrate the use of `Timestamp` as a parameter and as a return type.
For example:
