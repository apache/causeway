...
On the left hand side is a collection of entities that uses the `LocalDate` datatype.
There is also an action that opens a view model; this view model also uses the `LocalDate` datatype.
...
* The `LocalDateHolder` interface is used to contribute a number of mixins to both types:
+
[source,java]
----
include::holder/LocalDateHolder.java[tags=class]
----

* The `LocalDateHolder2` interface is used to demonstrate support for label positions using `@PropertyLayout(labelPosition=...)`.
