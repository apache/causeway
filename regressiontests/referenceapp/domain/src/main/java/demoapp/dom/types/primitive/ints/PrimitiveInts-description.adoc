...
On the left hand side is a collection of entities that uses the `int` datatype.
There is also an action that opens a view model; this view model also uses the `int` datatype.
