...
Apache Causeway provides its own implementation of `@XmlJavaTypeAdapter` for the `Date` value type, meaning that JAXB can serialize properties of this value type without further configuration.

[source,java]
----
include::JavaUtilDateVm.java[tags=class]
----
